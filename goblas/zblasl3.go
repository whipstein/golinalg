...
func Zgemm(transa, transb mat.MatTrans, m, n, k *int, alpha *complex128, a *mat.CMatrix, lda *int, b *mat.CMatrix, ldb *int, beta *complex128, c *mat.CMatrix, ldc *int) {
		...
		if nota {
			...
			for j = 1; j <= (*n); j++ {
				for i = 1; i <= (*m); i++ {
					temp = zero
					for l = 1; l <= (*k); l++ {
						temp = temp + a.GetConj(l-1, i-1)*b.Get(l-1, j-1)
					}
					if (*beta) == zero {
			...
			for j = 1; j <= (*n); j++ {
				for i = 1; i <= (*m); i++ {
					temp = zero
					for l = 1; l <= (*k); l++ {
						temp = temp + a.Get(l-1, i-1)*b.Get(l-1, j-1)
					}
					if (*beta) == zero {
						c.Set(i-1, j-1, (*alpha)*temp)
					} else {
		...
		if conjb {
			...
			for j = 1; j <= (*n); j++ {
				for i = 1; i <= (*m); i++ {
					temp = zero
					for l = 1; l <= (*k); l++ {
						temp = temp + a.Get(l-1, i-1)*b.GetConj(j-1, l-1)
					}
					if (*beta) == zero {
						c.Set(i-1, j-1, (*alpha)*temp)
					} else {
			...
			for j = 1; j <= (*n); j++ {
				for i = 1; i <= (*m); i++ {
					temp = zero
					for l = 1; l <= (*k); l++ {
						temp = temp + a.Get(l-1, i-1)*b.Get(j-1, l-1)
					}
					if (*beta) == zero {
						c.Set(i-1, j-1, (*alpha)*temp)
					} else {
...
func Zhemm(side mat.MatSide, uplo mat.MatUplo, m, n *int, alpha *complex128, a *mat.CMatrix, lda *int, b *mat.CMatrix, ldb *int, beta *complex128, c *mat.CMatrix, ldc *int) {
	var upper bool
			...
			for j = 1; j <= (*n); j++ {
				for i = 1; i <= (*m); i++ {
					...
					for k = 1; k <= i-1; k++ {
						...
						temp2 = temp2 + b.Get(k-1, j-1)*a.GetConj(k-1, i-1)
					}
					if (*beta) == zero {
...
// C are m by n matrices.
func Zsymm(side mat.MatSide, uplo mat.MatUplo, m, n *int, alpha *complex128, a *mat.CMatrix, lda *int, b *mat.CMatrix, ldb *int, beta *complex128, c *mat.CMatrix, ldc *int) {
	var upper bool
	var one, temp1, temp2, zero complex128
	var i, info, j, k, nrowa int
		...
		if upper {
			for j = 1; j <= (*n); j++ {
				for i = 1; i <= (*m); i++ {
					...
					for k = 1; k <= i-1; k++ {
						c.Set(k-1, j-1, c.Get(k-1, j-1)+temp1*a.Get(k-1, i-1))
						temp2 = temp2 + b.Get(k-1, j-1)*a.Get(k-1, i-1)
					}
					if (*beta) == zero {
...
func Ztrmm(side mat.MatSide, uplo mat.MatUplo, transa mat.MatTrans, diag mat.MatDiag, m, n *int, alpha *complex128, a *mat.CMatrix, lda *int, b *mat.CMatrix, ldb *int) {
...
func Ztrsm(side mat.MatSide, uplo mat.MatUplo, transa mat.MatTrans, diag mat.MatDiag, m, n *int, alpha *complex128, a *mat.CMatrix, lda *int, b *mat.CMatrix, ldb *int) {
	var lside, noconj, nounit, upper bool
	var one, temp, zero complex128
	var i, info, j, k, nrowa int
...
func Zherk(uplo mat.MatUplo, trans mat.MatTrans, n, k *int, alpha *float64, a *mat.CMatrix, lda *int, beta *float64, c *mat.CMatrix, ldc *int) {
	var upper bool
	var temp complex128
		...
		if upper {
			for j = 1; j <= (*n); j++ {
				for i = 1; i <= j-1; i++ {
					temp = complex(zero, 0)
					for l = 1; l <= (*k); l++ {
						temp = temp + a.GetConj(l-1, i-1)*a.Get(l-1, j-1)
					}
					if (*beta) == zero {
				...
				rtemp = zero
				for l = 1; l <= (*k); l++ {
					rtemp = rtemp + a.GetConjProd(l-1, j-1)
				}
				if (*beta) == zero {
...
// in the second case.
func Zsyrk(uplo mat.MatUplo, trans mat.MatTrans, n, k *int, alpha *complex128, a *mat.CMatrix, lda *int, beta *complex128, c *mat.CMatrix, ldc *int) {
	var upper bool
	var one, temp, zero complex128
	var i, info, j, l, nrowa int
		...
		if upper {
			for j = 1; j <= (*n); j++ {
				for i = 1; i <= j; i++ {
					temp = zero
					for l = 1; l <= (*k); l++ {
						temp = temp + a.Get(l-1, i-1)*a.Get(l-1, j-1)
					}
					if (*beta) == zero {
						c.Set(i-1, j-1, (*alpha)*temp)
					} else {
...
func Zher2k(uplo mat.MatUplo, trans mat.MatTrans, n, k *int, alpha *complex128, a *mat.CMatrix, lda *int, b *mat.CMatrix, ldb *int, beta *float64, c *mat.CMatrix, ldc *int) {
	var upper bool
...
func Zsyr2k(uplo mat.MatUplo, trans mat.MatTrans, n, k *int, alpha *complex128, a *mat.CMatrix, lda *int, b *mat.CMatrix, ldb *int, beta *complex128, c *mat.CMatrix, ldc *int) {
	var upper bool
	var one, temp1, temp2, zero complex128
	var i, info, j, l, nrowa int
