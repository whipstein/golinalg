...
func Dorbdb4(m, p, q *int, x11 *mat.Matrix, ldx11 *int, x21 *mat.Matrix, ldx21 *int, theta, phi, taup1, taup2, tauq1, phantom, work *mat.Vector, lwork, info *int) {
	...
	for i = 1; i <= (*m)-(*q); i++ {
		...
		if i == 1 {
			...
			Dorbdb5(p, toPtr((*m)-(*p)), q, phantom, func() *int { y := 1; return &y }(), phantom.Off((*p)), func() *int { y := 1; return &y }(), x11, ldx11, x21, ldx21, work.Off(iorbdb5-1), &lorbdb5, &childinfo)
			goblas.Dscal(*p, negone, phantom.Off(0, 1))
			Dlarfgp(p, phantom.GetPtr(0), phantom.Off(1), func() *int { y := 1; return &y }(), taup1.GetPtr(0))
			Dlarfgp(toPtr((*m)-(*p)), phantom.GetPtr((*p)), phantom.Off((*p)+2-1), func() *int { y := 1; return &y }(), taup2.GetPtr(0))
			...
			Dlarf('L', toPtr((*m)-(*p)), q, phantom.Off((*p)), func() *int { y := 1; return &y }(), taup2.GetPtr(0), x21, ldx21, work.Off(ilarf-1))
		} else {
			...
			Dlarfgp(toPtr((*p)-i+1), x11.GetPtr(i-1, i-1-1), x11.Vector(i, i-1-1), func() *int { y := 1; return &y }(), taup1.GetPtr(i-1))
			Dlarfgp(toPtr((*m)-(*p)-i+1), x21.GetPtr(i-1, i-1-1), x21.Vector(i, i-1-1), func() *int { y := 1; return &y }(), taup2.GetPtr(i-1))
		...
		c = x21.Get(i-1, i-1)
		x21.Set(i-1, i-1, one)
		Dlarf('R', toPtr((*p)-i), toPtr((*q)-i+1), x21.Vector(i-1, i-1), ldx21, tauq1.GetPtr(i-1), x11.Off(i, i-1), ldx11, work.Off(ilarf-1))
	...
	for i = (*m) - (*q) + 1; i <= (*p); i++ {
		Dlarfgp(toPtr((*q)-i+1), x11.GetPtr(i-1, i-1), x11.Vector(i-1, i), ldx11, tauq1.GetPtr(i-1))
		x11.Set(i-1, i-1, one)
		Dlarf('R', toPtr((*p)-i), toPtr((*q)-i+1), x11.Vector(i-1, i-1), ldx11, tauq1.GetPtr(i-1), x11.Off(i, i-1), ldx11, work.Off(ilarf-1))
		Dlarf('R', toPtr((*q)-(*p)), toPtr((*q)-i+1), x11.Vector(i-1, i-1), ldx11, tauq1.GetPtr(i-1), x21.Off((*m)-(*q), i-1), ldx21, work.Off(ilarf-1))
	}

	//     Reduce the bottom-right portion of X21 to [ 0 I ]
	for i = (*p) + 1; i <= (*q); i++ {
		Dlarfgp(toPtr((*q)-i+1), x21.GetPtr((*m)-(*q)+i-(*p)-1, i-1), x21.Vector((*m)-(*q)+i-(*p)-1, i), ldx21, tauq1.GetPtr(i-1))
		x21.Set((*m)-(*q)+i-(*p)-1, i-1, one)
		Dlarf('R', toPtr((*q)-i), toPtr((*q)-i+1), x21.Vector((*m)-(*q)+i-(*p)-1, i-1), ldx21, tauq1.GetPtr(i-1), x21.Off((*m)-(*q)+i-(*p), i-1), ldx21, work.Off(ilarf-1))
	}
}
