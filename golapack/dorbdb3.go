...
func Dorbdb3(m, p, q *int, x11 *mat.Matrix, ldx11 *int, x21 *mat.Matrix, ldx21 *int, theta, phi, taup1, taup2, tauq1, work *mat.Vector, lwork, info *int) {
	...
	for i = 1; i <= (*m)-(*p); i++ {
		...
		if i > 1 {
			goblas.Drot(toPtr((*q)-i+1), x11.Vector(i-1-1, i-1), ldx11, x21.Vector(i-1, i-1), ldx11, &c, &s)
		}
		...
		c = math.Sqrt(math.Pow(goblas.Dnrm2(toPtr((*p)-i+1), x11.Vector(i-1, i-1), func() *int { y := 1; return &y }()), 2) + math.Pow(goblas.Dnrm2(toPtr((*m)-(*p)-i), x21.Vector(i+1-1, i-1), func() *int { y := 1; return &y }()), 2))
		theta.Set(i-1, math.Atan2(s, c))

		Dorbdb5(toPtr((*p)-i+1), toPtr((*m)-(*p)-i), toPtr((*q)-i), x11.Vector(i-1, i-1), func() *int { y := 1; return &y }(), x21.Vector(i+1-1, i-1), func() *int { y := 1; return &y }(), x11.Off(i-1, i+1-1), ldx11, x21.Off(i+1-1, i+1-1), ldx21, work.Off(iorbdb5-1), &lorbdb5, &childinfo)
		Dlarfgp(toPtr((*p)-i+1), x11.GetPtr(i-1, i-1), x11.Vector(i+1-1, i-1), func() *int { y := 1; return &y }(), taup1.GetPtr(i-1))
		if i < (*m)-(*p) {
			Dlarfgp(toPtr((*m)-(*p)-i), x21.GetPtr(i+1-1, i-1), x21.Vector(i+2-1, i-1), func() *int { y := 1; return &y }(), taup2.GetPtr(i-1))
			phi.Set(i-1, math.Atan2(x21.Get(i+1-1, i-1), x11.Get(i-1, i-1)))
			c = math.Cos(phi.Get(i - 1))
			s = math.Sin(phi.Get(i - 1))
			x21.Set(i+1-1, i-1, one)
			Dlarf('L', toPtr((*m)-(*p)-i), toPtr((*q)-i), x21.Vector(i+1-1, i-1), func() *int { y := 1; return &y }(), taup2.GetPtr(i-1), x21.Off(i+1-1, i+1-1), ldx21, work.Off(ilarf-1))
		}
		x11.Set(i-1, i-1, one)
