...
func Dorbdb3(m, p, q *int, x11 *mat.Matrix, ldx11 *int, x21 *mat.Matrix, ldx21 *int, theta, phi, taup1, taup2, tauq1, work *mat.Vector, lwork, info *int) {
	...
	for i = 1; i <= (*m)-(*p); i++ {
		...
		if i > 1 {
			goblas.Drot((*q)-i+1, x11.Vector(i-1-1, i-1), *ldx11, x21.Vector(i-1, i-1), *ldx11, c, s)
		}
		...
		c = math.Sqrt(math.Pow(goblas.Dnrm2((*p)-i+1, x11.Vector(i-1, i-1), 1), 2) + math.Pow(goblas.Dnrm2((*m)-(*p)-i, x21.Vector(i+1-1, i-1), 1), 2))
