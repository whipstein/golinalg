...
func Dlantr(norm, uplo, diag byte, m, n *int, a *mat.Matrix, lda *int, work *mat.Vector) (dlantrReturn float64) {
	var udiag bool
		...
		if uplo == 'U' {
			...
			if diag == 'U' {
				...
				for j = 1; j <= (*n); j++ {
					colssq.Set(0, zero)
					colssq.Set(1, one)
					Dlassq(toPtr((*m)-j+1), a.Vector(j-1, j-1), func() *int { y := 1; return &y }(), colssq.GetPtr(0), colssq.GetPtr(1))
					Dcombssq(ssq, colssq)
				}
			}
		}
