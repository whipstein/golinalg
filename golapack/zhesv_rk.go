...
func ZhesvRk(uplo mat.MatUplo, n, nrhs int, a *mat.CMatrix, e *mat.CVector, ipiv *[]int, b *mat.CMatrix, work *mat.CVector, lwork int) (info int, err error) {
	var lquery bool
	var lwkopt int
	...
	if err == nil {
		if n == 0 {
			...
			if info, err = ZhetrfRk(uplo, n, a, e, ipiv, work, -1); err != nil {
				panic(err)
			}
	...
	//     A = P*U*D*(U**H)*(P**T).
	if info, err = ZhetrfRk(uplo, n, a, e, ipiv, work, lwork); err != nil {
		panic(err)
	}

	if info == 0 {
		//        Solve the system A*X = B with BLAS3 solver, overwriting B with X.
		if err = Zhetrs3(uplo, n, nrhs, a, e, ipiv, b); err != nil {
			panic(err)
		}
