...
func Dsyevx2stage(jobz, _range, uplo byte, n *int, a *mat.Matrix, lda *int, vl, vu *float64, il, iu *int, abstol *float64, m *int, w *mat.Vector, z *mat.Matrix, ldz *int, work *mat.Vector, lwork *int, iwork, ifail *[]int, info *int) {
	...
	var i, ib, iinfo, imax, indd, inde, indee, indhous, indibl, indisp, indiwo, indtau, indwkn, indwrk, iscale, itmp1, j, jj, kd, lhtrd, llwork, llwrkn, lwmin, lwtrd, nsplit int
	...
	if (*info) == 0 {
		if (*n) <= 1 {
			...
			lwmin = maxint(8*(*n), 3*(*n)+lhtrd+lwtrd)
			work.Set(0, float64(lwmin))
		}
	...
	if (*info) != 0 {
		gltest.Xerbla([]byte("DSYEVX_2STAGE"), -(*info))
		return
	} else if lquery {
		return
	}
	...
	Dsytrd2stage(jobz, uplo, n, a, lda, work.Off(indd-1), work.Off(inde-1), work.Off(indtau-1), work.Off(indhous-1), &lhtrd, work.Off(indwrk-1), &llwork, &iinfo)
