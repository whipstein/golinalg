...
func Dgeqr(m, n int, a *mat.Matrix, t *mat.Vector, tsize int, work *mat.Vector, lwork int) (err error) {
	...
	if min(m, n) > 0 {
		mb = Ilaenv(1, "Dgeqr", []byte{' '}, m, n, 1, -1)
		nb = Ilaenv(1, "Dgeqr", []byte{' '}, m, n, 2, -1)
	} else {
		mb = m
		nb = 1
	}
	...
	if m < 0 {
		...
		err = fmt.Errorf("tsize < max(1, nb*n*nblcks+5) && (!lquery) && (!lminws): tsize=%v, n=%v, nb=%v, nblcks=%v, lquery=%v, lminws=%v", tsize, n, nb, nblcks, lquery, lminws)
