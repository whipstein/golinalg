...
func Dgehrd(n, ilo, ihi *int, a *mat.Matrix, lda *int, tau, work *mat.Vector, lwork, info *int) {
	...
	if (*info) == 0 {
		...
		nb = minint(nbmax, Ilaenv(func() *int { y := 1; return &y }(), []byte("DGEHRD"), []byte{' '}, n, ilo, ihi, toPtr(-1)))
		lwkopt = (*n)*nb + tsize
	...
	if nb > 1 && nb < nh {
		...
		nx = maxint(nb, Ilaenv(func() *int { y := 3; return &y }(), []byte("DGEHRD"), []byte{' '}, n, ilo, ihi, toPtr(-1)))
		if nx < nh {
			...
			if (*lwork) < (*n)*nb+tsize {
				...
				//              unblocked code
				nbmin = maxint(2, Ilaenv(func() *int { y := 2; return &y }(), []byte("DGEHRD"), []byte{' '}, n, ilo, ihi, toPtr(-1)))
	...
	if nb < nbmin || nb >= nh {
		...
		for _, i = range genIter((*ilo), (*ihi)-1-nx, nb) {
			...
			//           left
			Dlarfb('L', 'T', 'F', 'C', toPtr((*ihi)-i), toPtr((*n)-i-ib+1), &ib, a.Off(i+1-1, i-1), lda, work.MatrixOff(iwt-1, ldt, opts), &ldt, a.Off(i+1-1, i+ib-1), lda, work.Matrix(ldwork, opts), &ldwork)
		}
	}
