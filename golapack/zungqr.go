...
func Zungqr(m, n, k *int, a *mat.CMatrix, lda *int, tau, work *mat.CVector, lwork, info *int) {
	...
	if nb > 1 && nb < (*k) {
		...
		nx = max(0, Ilaenv(func() *int { y := 3; return &y }(), []byte("ZUNGQR"), []byte{' '}, m, n, k, toPtr(-1)))
		if nx < (*k) {
			...
			if (*lwork) < iws {
				...
				nb = (*lwork) / ldwork
				nbmin = max(2, Ilaenv(func() *int { y := 2; return &y }(), []byte("ZUNGQR"), []byte{' '}, m, n, k, toPtr(-1)))
			}
		}
	}
	...
	if kk < (*n) {
		Zung2r(toPtr((*m)-kk), toPtr((*n)-kk), toPtr((*k)-kk), a.Off(kk, kk), lda, tau.Off(kk), work, &iinfo)
	}
	...
	if kk > 0 {
		...
		for i = ki + 1; i >= 1; i -= nb {
			...
			if i+ib <= (*n) {
				...
				Zlarfb('L', 'N', 'F', 'C', toPtr((*m)-i+1), toPtr((*n)-i-ib+1), &ib, a.Off(i-1, i-1), lda, work.CMatrix(ldwork, opts), &ldwork, a.Off(i-1, i+ib-1), lda, work.CMatrixOff(ib, ldwork, opts), &ldwork)
			}
