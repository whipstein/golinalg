...
func ZlahefRook(uplo mat.MatUplo, n, nb int, a *mat.CMatrix, ipiv *[]int, w *mat.CMatrix) (kb, info int) {
	...
	label60:
		...
		j = j + 1
		if jp2 != jj && j <= n {
			goblas.Zswap(n-j+1, a.CVector(jp2-1, j-1), a.CVector(jj-1, j-1))
		}
		jj = jj + 1
		if kstep == 2 && jp1 != jj && j <= n {
			goblas.Zswap(n-j+1, a.CVector(jp1-1, j-1), a.CVector(jj-1, j-1))
		}
		if j < n {
			goto label60
		}
	...
	label120:
		...
		j = j - 1
		if jp2 != jj && j >= 1 {
			goblas.Zswap(j, a.CVector(jp2-1, 0), a.CVector(jj-1, 0))
		}
		jj = jj - 1
		if kstep == 2 && jp1 != jj && j >= 1 {
			goblas.Zswap(j, a.CVector(jp1-1, 0), a.CVector(jj-1, 0))
		}
		if j > 1 {
			goto label120
		}
