...
func ZlahefRook(uplo mat.MatUplo, n, nb int, a *mat.CMatrix, ipiv *[]int, w *mat.CMatrix) (kb, info int) {
	var done bool
	...
	label10:
		...
		if math.Max(absakk, colmax) == zero {
			...
			if k > 1 {
				a.Off(0, k-1).CVector().Copy(k-1, w.Off(0, kw-1).CVector(), 1, 1)
			}
		} else {
			//           ============================================================
			//
			...
			if (kstep == 2) && (p != k) {
				...
				a.Off(p-1, p).CVector().Copy(k-1-p, a.Off(p, k-1).CVector(), 1, a.Rows)
				Zlacgv(k-1-p, a.Off(p-1, p).CVector(), a.Rows)
				if p > 1 {
					a.Off(0, p-1).CVector().Copy(p-1, a.Off(0, k-1).CVector(), 1, 1)
				}
	...
	label70:
		...
		if math.Max(absakk, colmax) == zero {
			...
			if k < n {
				a.Off(k, k-1).CVector().Copy(n-k, w.Off(k, k-1).CVector(), 1, 1)
			}
		} else {
			//           ============================================================
			//
			...
			if (kstep == 2) && (p != k) {
				...
				//              will be later overwritten.
				a.Set(p-1, p-1, a.GetReCmplx(k-1, k-1))
				a.Off(p-1, k).CVector().Copy(p-k-1, a.Off(k, k-1).CVector(), 1, a.Rows)
				Zlacgv(p-k-1, a.Off(p-1, k).CVector(), a.Rows)
				if p < n {
					a.Off(p, p-1).CVector().Copy(n-p, a.Off(p, k-1).CVector(), 1, 1)
				}
