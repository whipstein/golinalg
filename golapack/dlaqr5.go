...
func Dlaqr5(wantt, wantz bool, kacc22, n, ktop, kbot, nshfts *int, sr, si *mat.Vector, h *mat.Matrix, ldh, iloz, ihiz *int, z *mat.Matrix, ldz *int, v *mat.Matrix, ldv *int, u *mat.Matrix, ldu, nv *int, wv *mat.Matrix, ldwv, nh *int, wh *mat.Matrix, ldwh *int) {
	...
	for i = 1; i <= (*nshfts)-2; i += 2 {
		if si.Get(i-1) != -si.Get(i) {

			swap = sr.Get(i - 1)
			sr.Set(i-1, sr.Get(i))
			sr.Set(i, sr.Get(i+2-1))
			sr.Set(i+2-1, swap)

			swap = si.Get(i - 1)
			si.Set(i-1, si.Get(i))
			si.Set(i, si.Get(i+2-1))
			si.Set(i+2-1, swap)
		}
	}
	...
	for incol = 3*(1-nbmps) + (*ktop) - 1; incol <= (*kbot)-2; incol += 3*nbmps - 2 {
		...
		for krcol = incol; krcol <= min(incol+3*nbmps-3, (*kbot)-2); krcol++ {
			...
			for m = mtop; m <= mbot; m++ {
				...
				if k == (*ktop)-1 {
					...
					if h.Get(k+3-1, k-1) != zero || h.Get(k+3-1, k) != zero || h.Get(k+3-1, k+2-1) == zero {
						...
						Dlaqr1(func() *int { y := 3; return &y }(), h.Off(k, k), ldh, sr.GetPtr(2*m-1-1), si.GetPtr(2*m-1-1), sr.GetPtr(2*m-1), si.GetPtr(2*m-1), vt)
						alpha = vt.Get(0)
			...
			if bmp22 {
				if k == (*ktop)-1 {
					Dlaqr1(func() *int { y := 2; return &y }(), h.Off(k, k), ldh, sr.GetPtr(2*m22-1-1), si.GetPtr(2*m22-1-1), sr.GetPtr(2*m22-1), si.GetPtr(2*m22-1), v.Vector(0, m22-1))
					beta = v.Get(0, m22-1)
			...
			for j = max(*ktop, krcol); j <= jbot; j++ {
				...
				for m = mtop; m <= mend; m++ {
					k = krcol + 3*(m-1)
					refsum = v.Get(0, m-1) * (h.Get(k, j-1) + v.Get(1, m-1)*h.Get(k+2-1, j-1) + v.Get(2, m-1)*h.Get(k+3-1, j-1))
			...
			if bmp22 {
				...
				for j = max(k+1, *ktop); j <= jbot; j++ {
					refsum = v.Get(0, m22-1) * (h.Get(k, j-1) + v.Get(1, m22-1)*h.Get(k+2-1, j-1))
					h.Set(k, j-1, h.Get(k, j-1)-refsum)
			...
			for m = mstart; m <= mend; m++ {
				...
				//              .    unnecessary. ====
				if h.Get(k, k-1) != zero {
					tst1 = math.Abs(h.Get(k-1, k-1)) + math.Abs(h.Get(k, k))
					if tst1 == zero {
						if k >= (*ktop)+1 {
						...
						if k <= (*kbot)-2 {
							tst1 = tst1 + math.Abs(h.Get(k+2-1, k))
						}
						if k <= (*kbot)-3 {
							tst1 = tst1 + math.Abs(h.Get(k+3-1, k))
						}
						if k <= (*kbot)-4 {
							tst1 = tst1 + math.Abs(h.Get(k+4-1, k))
						}
					}
					if math.Abs(h.Get(k, k-1)) <= math.Max(smlnum, ulp*tst1) {
						h12 = math.Max(math.Abs(h.Get(k, k-1)), math.Abs(h.Get(k-1, k)))
						h21 = math.Min(math.Abs(h.Get(k, k-1)), math.Abs(h.Get(k-1, k)))
						h11 = math.Max(math.Abs(h.Get(k, k)), math.Abs(h.Get(k-1, k-1)-h.Get(k, k)))
						h22 = math.Min(math.Abs(h.Get(k, k)), math.Abs(h.Get(k-1, k-1)-h.Get(k, k)))
						scl = h11 + h12
						tst2 = h22 * (h11 / scl)

						if tst2 == zero || h21*(h12/scl) <= math.Max(smlnum, ulp*tst2) {
							h.Set(k, k-1, zero)
						}
					}
				}
		...
		if accum {
			...
			if (!blk22) || (incol < (*ktop)) || (ndcol > (*kbot)) || (ns <= 2) {
				...
				for jcol = min(ndcol, *kbot) + 1; jcol <= jbot; jcol += (*nh) {
					...
					err = goblas.Dgemm(ConjTrans, NoTrans, nu, jlen, nu, one, u.Off(k1-1, k1-1), h.Off(incol+k1-1, jcol-1), zero, wh)
				...
				for jrow = jtop; jrow <= max(*ktop, incol)-1; jrow += (*nv) {
					...
					err = goblas.Dgemm(NoTrans, NoTrans, jlen, nu, nu, one, h.Off(jrow-1, incol+k1-1), u.Off(k1-1, k1-1), zero, wv)
				...
				if wantz {
					for jrow = (*iloz); jrow <= (*ihiz); jrow += (*nv) {
						...
						err = goblas.Dgemm(NoTrans, NoTrans, jlen, nu, nu, one, z.Off(jrow-1, incol+k1-1), u.Off(k1-1, k1-1), zero, wv)
				...
				for jcol = min(ndcol, *kbot) + 1; jcol <= jbot; jcol += (*nh) {
					...
					Dlacpy('A', &knz, &jlen, h.Off(incol+1+j2-1, jcol-1), ldh, wh.Off(kzs, 0), ldwh)

					//                 ==== Multiply by U21**T ====
					Dlaset('A', &kzs, &jlen, &zero, &zero, wh, ldwh)
					err = goblas.Dtrmm(Left, Upper, ConjTrans, NonUnit, knz, jlen, one, u.Off(j2, 1+kzs-1), wh.Off(kzs, 0))

					//                 ==== Multiply top of H by U11**T ====
					err = goblas.Dgemm(ConjTrans, NoTrans, i2, jlen, j2, one, u, h.Off(incol, jcol-1), one, wh)

					//                 ==== Copy top of H to bottom of WH ====
					Dlacpy('A', &j2, &jlen, h.Off(incol, jcol-1), ldh, wh.Off(i2, 0), ldwh)

					//                 ==== Multiply by U21**T ====
					err = goblas.Dtrmm(Left, Lower, ConjTrans, NonUnit, j2, jlen, one, u.Off(0, i2), wh.Off(i2, 0))

					//                 ==== Multiply by U22 ====
					err = goblas.Dgemm(ConjTrans, NoTrans, i4-i2, jlen, j4-j2, one, u.Off(j2, i2), h.Off(incol+1+j2-1, jcol-1), one, wh.Off(i2, 0))

					//                 ==== Copy it back ====
					Dlacpy('A', &kdu, &jlen, wh, ldwh, h.Off(incol, jcol-1), ldh)
				}
				...
				for jrow = jtop; jrow <= max(incol, *ktop)-1; jrow += (*nv) {
					...
					err = goblas.Dtrmm(Right, Upper, NoTrans, NonUnit, jlen, knz, one, u.Off(j2, 1+kzs-1), wv.Off(0, 1+kzs-1))

					//                 ==== Multiply by U11 ====
					err = goblas.Dgemm(NoTrans, NoTrans, jlen, i2, j2, one, h.Off(jrow-1, incol), u, one, wv)

					//                 ==== Copy left of H to right of scratch ====
					Dlacpy('A', &jlen, &j2, h.Off(jrow-1, incol), ldh, wv.Off(0, 1+i2-1), ldwv)

					//                 ==== Multiply by U21 ====
					err = goblas.Dtrmm(Right, Lower, NoTrans, NonUnit, jlen, i4-i2, one, u.Off(0, i2), wv.Off(0, 1+i2-1))

					//                 ==== Multiply by U22 ====
					err = goblas.Dgemm(NoTrans, NoTrans, jlen, i4-i2, j4-j2, one, h.Off(jrow-1, incol+1+j2-1), u.Off(j2, i2), one, wv.Off(0, 1+i2-1))

					//                 ==== Copy it back ====
					Dlacpy('A', &jlen, &kdu, wv, ldwv, h.Off(jrow-1, incol), ldh)
				}
				...
				if wantz {
					for jrow = (*iloz); jrow <= (*ihiz); jrow += (*nv) {
						...
						//                    ==== Multiply by U11 ====
						err = goblas.Dgemm(NoTrans, NoTrans, jlen, i2, j2, one, z.Off(jrow-1, incol), u, one, wv)

						//                    ==== Copy left of Z to right of scratch ====
						Dlacpy('A', &jlen, &j2, z.Off(jrow-1, incol), ldz, wv.Off(0, 1+i2-1), ldwv)

						//                    ==== Multiply by U21 ====
						err = goblas.Dtrmm(Right, Lower, NoTrans, NonUnit, jlen, i4-i2, one, u.Off(0, i2), wv.Off(0, 1+i2-1))

						//                    ==== Multiply by U22 ====
						err = goblas.Dgemm(NoTrans, NoTrans, jlen, i4-i2, j4-j2, one, z.Off(jrow-1, incol+1+j2-1), u.Off(j2, i2), one, wv.Off(0, 1+i2-1))

						//                    ==== Copy the result back to Z ====
						Dlacpy('A', &jlen, &kdu, wv, ldwv, z.Off(jrow-1, incol), ldz)
					}
				}
			}
		}
