...
func Dlaqr5(wantt, wantz bool, kacc22, n, ktop, kbot, nshfts int, sr, si *mat.Vector, h *mat.Matrix, iloz, ihiz int, z, v, u *mat.Matrix, nv int, wv *mat.Matrix, nh int, wh *mat.Matrix) {
	...
	for incol = 3*(1-nbmps) + ktop - 1; incol <= kbot-2; incol += 3*nbmps - 2 {
		ndcol = incol + kdu
		if accum {
			Dlaset(Full, kdu, kdu, zero, one, u)
		}
		...
		if accum {
			...
			if (!blk22) || (incol < ktop) || (ndcol > kbot) || (ns <= 2) {
				...
				for jcol = min(ndcol, kbot) + 1; jcol <= jbot; jcol += nh {
					...
					Dlacpy(Full, knz, jlen, h.Off(incol+1+j2-1, jcol-1), wh.Off(kzs, 0))
					...
					//                 ==== Copy it back ====
					Dlacpy(Full, kdu, jlen, wh, h.Off(incol, jcol-1))
				}
				...
				for jrow = jtop; jrow <= max(incol, ktop)-1; jrow += nv {
					...
					Dlacpy(Full, jlen, knz, h.Off(jrow-1, incol+1+j2-1), wv.Off(0, 1+kzs-1))
					...
					//                 ==== Copy it back ====
					Dlacpy(Full, jlen, kdu, wv, h.Off(jrow-1, incol))
				}
				...
				if wantz {
					for jrow = iloz; jrow <= ihiz; jrow += nv {
						...
						Dlacpy(Full, jlen, knz, z.Off(jrow-1, incol+1+j2-1), wv.Off(0, 1+kzs-1))
						...
						Dlacpy(Full, jlen, kdu, wv, z.Off(jrow-1, incol))
					}
				}
			}
		}
