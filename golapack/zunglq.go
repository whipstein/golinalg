...
func Zunglq(m, n, k *int, a *mat.CMatrix, lda *int, tau, work *mat.CVector, lwork, info *int) {
	...
	if nb > 1 && nb < (*k) {
		...
		nx = maxint(0, Ilaenv(func() *int { y := 3; return &y }(), []byte("ZUNGLQ"), []byte{' '}, m, n, k, toPtr(-1)))
		if nx < (*k) {
			...
			if (*lwork) < iws {
				...
				nb = (*lwork) / ldwork
				nbmin = maxint(2, Ilaenv(func() *int { y := 2; return &y }(), []byte("ZUNGLQ"), []byte{' '}, m, n, k, toPtr(-1)))
			}
		}
	}
	...
	if kk < (*m) {
		Zungl2(toPtr(((*m) - kk)), toPtr((*n)-kk), toPtr((*k)-kk), a.Off(kk+1-1, kk+1-1), lda, tau.Off(kk+1-1), work, &iinfo)
	}
	...
	if kk > 0 {
		...
		for i = ki + 1; i >= 1; i -= nb {
			...
			if i+ib <= (*m) {
				...
				Zlarfb('R', 'C', 'F', 'R', toPtr((*m)-i-ib+1), toPtr((*n)-i+1), &ib, a.Off(i-1, i-1), lda, work.CMatrix(ldwork, opts), &ldwork, a.Off(i+ib-1, i-1), lda, work.CMatrixOff(ib+1-1, ldwork, opts), &ldwork)
			}
