...
func Dtbrfs(uplo mat.MatUplo, trans mat.MatTrans, diag mat.MatDiag, n, kd, nrhs int, ab, b, x *mat.Matrix, ferr, berr, work *mat.Vector, iwork *[]int) (err error) {
	...
	for j = 1; j <= nrhs; j++ {
		...
		if err = work.Off(n).Tbmv(uplo, trans, diag, n, kd, ab, 1); err != nil {
			panic(err)
		}
		...
		*_ferr, kase = Dlacn2(n, work.Off(2*n), work.Off(n), iwork, ferr.Get(j-1), kase, &isave)
		if kase != 0 {
			if kase == 1 {
				...
				if err = work.Off(n).Tbsv(uplo, transt, diag, n, kd, ab, 1); err != nil {
					panic(err)
				}
				for i = 1; i <= n; i++ {
				...
				if err = work.Off(n).Tbsv(uplo, trans, diag, n, kd, ab, 1); err != nil {
					panic(err)
				}
			}
			goto label210
