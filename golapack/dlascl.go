...
// or banded.
func Dlascl(_type byte, kl, ku int, cfrom, cto float64, m, n int, a *mat.Matrix) (err error) {
	var done bool
	...
	if itype == -1 {
		...
		err = fmt.Errorf("n < 0 || (itype == 4 && n != m) || (itype == 5 && n != m): _type='%c', m=%v, n=%v", _type, m, n)
		...
		if kl < 0 || kl > max(m-1, 0) {
			err = fmt.Errorf("itype >= 4, kl < 0 || kl > max(m-1, 0): _type='%c', m=%v, kl=%v", _type, m, kl)
		} else if ku < 0 || ku > max(n-1, 0) || ((itype == 4 || itype == 5) && kl != ku) {
			err = fmt.Errorf("itype >= 4, ku < 0 || ku > max(n-1, 0) || ((itype == 4 || itype == 5) && kl != ku): _type='%c', n=%v, kl=%v, ku=%v", _type, n, kl, ku)
		} else if (itype == 4 && a.Rows < kl+1) || (itype == 5 && a.Rows < ku+1) || (itype == 6 && a.Rows < 2*kl+ku+1) {
			err = fmt.Errorf("itype >= 4, (itype == 4 && a.Rows < kl+1) || (itype == 5 && a.Rows < ku+1) || (itype == 6 && a.Rows < 2*kl+ku+1): _type='%c', a.Rows=%v, kl=%v, ku=%v", _type, a.Rows, kl, ku)
		}
	}

	if err != nil {
		gltest.Xerbla2("Dlascl", err)
		return
	}
...
label10:
	...
	if itype == 0 {
		...
		for j = 1; j <= n; j++ {
			for i = 1; i <= m; i++ {
				a.Set(i-1, j-1, a.Get(i-1, j-1)*mul)
			}
		}
