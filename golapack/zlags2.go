...
// zero.
func Zlags2(upper bool, a1 *float64, a2 *complex128, a3, b1 *float64, b2 *complex128, b3, csu *float64, snu *complex128, csv *float64, snv *complex128, csq *float64, snq *complex128) {
	var b, c, d1, r, ua11, ua12, ua21, ua22, vb11, vb12, vb21, vb22 complex128
	var a, aua11, aua12, aua21, aua22, avb11, avb12, avb21, avb22, csl, csr, d, fb, fc, one, s1, s2, snl, snr, ua11r, ua22r, vb11r, vb22r, zero float64
	...
	if upper {
		...
		if math.Abs(csl) >= math.Abs(snl) || math.Abs(csr) >= math.Abs(snr) {
			...
			if (math.Abs(ua11r) + abs1(ua12)) == zero {
				Zlartg(toPtrc128(toCmplx(-vb11r)), toPtrc128(cmplx.Conj(vb12)), csq, snq, &r)
			} else if (math.Abs(vb11r) + abs1(vb12)) == zero {
				Zlartg(toPtrc128(toCmplx(-ua11r)), toPtrc128(cmplx.Conj(ua12)), csq, snq, &r)
			...
			if (abs1(ua21) + abs1(ua22)) == zero {
				Zlartg(toPtrc128(-cmplx.Conj(vb21)), toPtrc128(cmplx.Conj(vb22)), csq, snq, &r)
			} else if (abs1(vb21) + cmplx.Abs(vb22)) == zero {
				Zlartg(toPtrc128(-cmplx.Conj(ua21)), toPtrc128(cmplx.Conj(ua22)), csq, snq, &r)
		...
		if math.Abs(csr) >= math.Abs(snr) || math.Abs(csl) >= math.Abs(snl) {
			...
			if (abs1(ua21) + math.Abs(ua22r)) == zero {
				Zlartg(toPtrc128(toCmplx(vb22r)), &vb21, csq, snq, &r)
			} else if (abs1(vb21) + math.Abs(vb22r)) == zero {
				Zlartg(toPtrc128(toCmplx(ua22r)), &ua21, csq, snq, &r)
