...
func Dlarrd(_range, order byte, n int, vl, vu float64, il, iu int, gers *mat.Vector, reltol float64, d, e, e2 *mat.Vector, pivmin float64, nsplit int, isplit *[]int, w, werr *mat.Vector, iblock, indexw *[]int, work *mat.Vector, iwork *[]int) (m int, wl, wu float64, info int, err error) {
	...
	if irange <= 0 {
		...
		err = fmt.Errorf("!(order == 'B' || order == 'E'): order='%c'", order)
	} else if n < 0 {
		err = fmt.Errorf("n < 0: n=%v", n)
	} else if irange == valrng {
		if vl >= vu {
			err = fmt.Errorf("irange == valrng, vl >= vu: _range='%c', vl=%v, vu=%v", _range, vl, vu)
		}
	} else if irange == indrng && (il < 1 || il > max(1, n)) {
		err = fmt.Errorf("irange == indrng && (il < 1 || il > max(1, n)): _range='%c', il=%v, n=%v", _range, il, n)
	} else if irange == indrng && (iu < min(n, il) || iu > n) {
		err = fmt.Errorf("irange == indrng && (iu < min(n, il) || iu > n): _range='%c', il=%v, iu=%v, n=%v", _range, il, iu, n)
	}
