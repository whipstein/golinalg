...
func ZhetrfAa(uplo mat.MatUplo, n int, a *mat.CMatrix, ipiv *[]int, work *mat.CVector, lwork int) (err error) {
	...
	nb = Ilaenv(1, "ZhetrfAa", []byte{uplo.Byte()}, n, -1, -1, -1)
	...
	if upper {
		...
		goblas.Zcopy(n, a.CVector(0, 0), work.Off(0, 1))
	...
	label10:
		...
		//        Panel factorization
		ZlahefAa(uplo, 2-k1, n-j, jb, a.Off(max(1, j)-1, j), toSlice(ipiv, j), work.CMatrix(n, opts), work.Off(n*nb))
		...
		if j < n {
			...
			if j1 > 1 || jb > 1 {
				...
				a.Set(j-1, j, one)
				goblas.Zcopy(n-j, a.CVector(j-1-1, j), work.Off((j+1-j1+1)+jb*n-1, 1))
				...
				for j2 = j + 1; j2 <= n; j2 += nb {
					...
					j3 = j2
					for mj = nj - 1; mj >= 1; mj-- {
						if err = goblas.Zgemm(ConjTrans, Trans, 1, mj, jb+1, -one, a.Off(j1-k2-1, j3-1), work.CMatrixOff((j3-j1+1)+k1*n-1, n, opts), one, a.Off(j3-1, j3-1)); err != nil {
							panic(err)
						}
						j3 = j3 + 1
					}
					...
					if err = goblas.Zgemm(ConjTrans, Trans, nj, n-j3+1, jb+1, -one, a.Off(j1-k2-1, j2-1), work.CMatrixOff((j3-j1+1)+k1*n-1, n, opts), one, a.Off(j2-1, j3-1)); err != nil {
						panic(err)
					}
				}
			...
			goblas.Zcopy(n-j, a.CVector(j, j), work.Off(0, 1))
		}
		goto label10
	} else {
		//        .....................................................
	...
	label11:
		...
		//        Panel factorization
		ZlahefAa(uplo, 2-k1, n-j, jb, a.Off(j, max(1, j)-1), toSlice(ipiv, j), work.CMatrix(n, opts), work.Off(n*nb))

		//        Adjust IPIV and apply it back (J-th step picks (J+1)-th pivot)
		for j2 = j + 2; j2 <= min(n, j+jb+1); j2++ {
			(*ipiv)[j2-1] = (*ipiv)[j2-1] + j
			if (j2 != (*ipiv)[j2-1]) && ((j1 - k1) > 2) {
				goblas.Zswap(j1-k1-2, a.CVector(j2-1, 0), a.CVector((*ipiv)[j2-1]-1, 0))
			}
		}
		j = j + jb
		...
		if j < n {
			...
			if j1 > 1 || jb > 1 {
				...
				for j2 = j + 1; j2 <= n; j2 += nb {
					...
					j3 = j2
					for mj = nj - 1; mj >= 1; mj-- {
						if err = goblas.Zgemm(NoTrans, ConjTrans, mj, 1, jb+1, -one, work.CMatrixOff((j3-j1+1)+k1*n-1, n, opts), a.Off(j3-1, j1-k2-1), one, a.Off(j3-1, j3-1)); err != nil {
							panic(err)
						}
						j3 = j3 + 1
					}
					...
					if err = goblas.Zgemm(NoTrans, ConjTrans, n-j3+1, nj, jb+1, -one, work.CMatrixOff((j3-j1+1)+k1*n-1, n, opts), a.Off(j2-1, j1-k2-1), one, a.Off(j3-1, j2-1)); err != nil {
						panic(err)
					}
				}
