...
func Zhetrfaa(uplo byte, n *int, a *mat.CMatrix, lda *int, ipiv *[]int, work *mat.CVector, lwork, info *int) {
	...
	label10:
		...
		//        Panel factorization
		Zlahefaa(uplo, toPtr(2-k1), toPtr((*n)-j), &jb, a.Off(maxint(1, j)-1, j+1-1), lda, toSlice(ipiv, j+1-1), work.CMatrix(*n, opts), n, work.Off((*n)*nb+1-1))
		...
		if j < (*n) {
			...
			if j1 > 1 || jb > 1 {
				...
				//              Recover T( J, J+1 )
				a.Set(j-1, j+1-1, cmplx.Conj(alpha))
			}
	...
	label11:
		...
		//        Panel factorization
		Zlahefaa(uplo, toPtr(2-k1), toPtr((*n)-j), &jb, a.Off(j+1-1, maxint(1, j)-1), lda, toSlice(ipiv, j+1-1), work.CMatrix(*n, opts), n, work.Off((*n)*nb+1-1))
		...
		if j < (*n) {
			...
			if j1 > 1 || jb > 1 {
				...
				//              Recover T( J+1, J )
				a.Set(j+1-1, j-1, cmplx.Conj(alpha))
			}
