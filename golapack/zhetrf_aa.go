		...
		if j < n {
			...
			if j1 > 1 || jb > 1 {
				...
				for j2 = j + 1; j2 <= n; j2 += nb {
					...
					j3 = j2
					for mj = nj - 1; mj >= 1; mj-- {
						if err = a.Off(j3-1, j3-1).Gemm(ConjTrans, Trans, 1, mj, jb+1, -one, a.Off(j1-k2-1, j3-1), work.Off((j3-j1+1)+k1*n-1).CMatrix(n, opts), one); err != nil {
							panic(err)
						}
						j3 = j3 + 1
					}
					...
					if err = a.Off(j2-1, j3-1).Gemm(ConjTrans, Trans, nj, n-j3+1, jb+1, -one, a.Off(j1-k2-1, j2-1), work.Off((j3-j1+1)+k1*n-1).CMatrix(n, opts), one); err != nil {
						panic(err)
					}
				}
		...
		if j < n {
			...
			if j1 > 1 || jb > 1 {
				...
				for j2 = j + 1; j2 <= n; j2 += nb {
					...
					j3 = j2
					for mj = nj - 1; mj >= 1; mj-- {
						if err = a.Off(j3-1, j3-1).Gemm(NoTrans, ConjTrans, mj, 1, jb+1, -one, work.Off((j3-j1+1)+k1*n-1).CMatrix(n, opts), a.Off(j3-1, j1-k2-1), one); err != nil {
							panic(err)
						}
						j3 = j3 + 1
					}
					...
					if err = a.Off(j3-1, j2-1).Gemm(NoTrans, ConjTrans, n-j3+1, nj, jb+1, -one, work.Off((j3-j1+1)+k1*n-1).CMatrix(n, opts), a.Off(j2-1, j1-k2-1), one); err != nil {
						panic(err)
					}
				}
