...
func Dorbdb2(m, p, q *int, x11 *mat.Matrix, ldx11 *int, x21 *mat.Matrix, ldx21 *int, theta, phi, taup1, taup2, tauq1, work *mat.Vector, lwork, info *int) {
	...
	for i = 1; i <= (*p); i++ {
		...
		if i > 1 {
			goblas.Drot(toPtr((*q)-i+1), x11.Vector(i-1, i-1), ldx11, x21.Vector(i-1-1, i-1), ldx21, &c, &s)
		}
		...
		s = math.Sqrt(math.Pow(goblas.Dnrm2(toPtr((*p)-i), x11.Vector(i+1-1, i-1), func() *int { y := 1; return &y }()), 2) + math.Pow(goblas.Dnrm2(toPtr((*m)-(*p)-i+1), x21.Vector(i-1, i-1), func() *int { y := 1; return &y }()), 2))
		...
		goblas.Dscal(toPtr((*p)-i), &negone, x11.Vector(i+1-1, i-1), func() *int { y := 1; return &y }())
