...
label10:
	...
	if (k < (*nb)) && (lsticc == 0) {
		...
		if k > 1 {
			err = goblas.Dgemv(NoTrans, (*m)-rk+1, k-1, -one, a.Off(rk-1, 0), f.Vector(k-1, 0, *ldf), one, a.Vector(rk-1, k-1, 1))
		}

		//        Generate elementary reflector H(k).
		if rk < (*m) {
			Dlarfg(toPtr((*m)-rk+1), a.GetPtr(rk-1, k-1), a.Vector(rk, k-1), func() *int { y := 1; return &y }(), tau.GetPtr(k-1))
		} else {
		...
		if k < (*n) {
			err = goblas.Dgemv(Trans, (*m)-rk+1, (*n)-k, tau.Get(k-1), a.Off(rk-1, k), a.Vector(rk-1, k-1, 1), zero, f.Vector(k, k-1, 1))
		}
		...
		//                    *A(RK:M,K).
		if k > 1 {
			err = goblas.Dgemv(Trans, (*m)-rk+1, k-1, -tau.Get(k-1), a.Off(rk-1, 0), a.Vector(rk-1, k-1, 1), zero, auxv)

			err = goblas.Dgemv(NoTrans, *n, k-1, one, f.Off(0, 0), auxv, one, f.Vector(0, k-1, 1))
		}

		//        Update the current row of A:
		//        A(RK,K+1:N) := A(RK,K+1:N) - A(RK,1:K)*F(K+1:N,1:K)**T.
		if k < (*n) {
			err = goblas.Dgemv(NoTrans, (*n)-k, k, -one, f.Off(k, 0), a.Vector(rk-1, 0), one, a.Vector(rk-1, k))
		}
	...
	if (*kb) < min(*n, (*m)-(*offset)) {
		err = goblas.Dgemm(NoTrans, Trans, (*m)-rk, (*n)-(*kb), *kb, -one, a.Off(rk, 0), f.Off((*kb), 0), one, a.Off(rk, (*kb)))
	}
