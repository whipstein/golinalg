...
func Zunbdb2(m, p, q *int, x11 *mat.CMatrix, ldx11 *int, x21 *mat.CMatrix, ldx21 *int, theta, phi *mat.Vector, taup1, taup2, tauq1, work *mat.CVector, lwork, info *int) {
	...
	for i = 1; i <= (*p); i++ {
		...
		Zunbdb5(toPtr((*p)-i), toPtr((*m)-(*p)-i+1), toPtr((*q)-i), x11.CVector(i, i-1), func() *int { y := 1; return &y }(), x21.CVector(i-1, i-1), func() *int { y := 1; return &y }(), x11.Off(i, i), ldx11, x21.Off(i-1, i), ldx21, work.Off(iorbdb5-1), &lorbdb5, &childinfo)
		...
		if i < (*p) {
			Zlarfgp(toPtr((*p)-i), x11.GetPtr(i, i-1), x11.CVector(i+2-1, i-1), func() *int { y := 1; return &y }(), taup1.GetPtr(i-1))
			phi.Set(i-1, math.Atan2(x11.GetRe(i, i-1), x21.GetRe(i-1, i-1)))
			c = math.Cos(phi.Get(i - 1))
			s = math.Sin(phi.Get(i - 1))
			x11.Set(i, i-1, one)
			Zlarf('L', toPtr((*p)-i), toPtr((*q)-i), x11.CVector(i, i-1), func() *int { y := 1; return &y }(), toPtrc128(taup1.GetConj(i-1)), x11.Off(i, i), ldx11, work.Off(ilarf-1))
		}
		x21.Set(i-1, i-1, one)
