		...
		if uplo == 'U' {
			if diag == 'U' {
				...
				for j = 1; j <= (*n); j++ {
					colssq.Set(0, zero)
					colssq.Set(1, one)
					Zlassq(toPtr(min(j, (*k)+1)), ab.CVector(max((*k)+2-j, 1)-1, j-1), func() *int { y := 1; return &y }(), colssq.GetPtr(0), colssq.GetPtr(1))
					Dcombssq(ssq, colssq)
				}
			}
		} else {
			if diag == 'U' {
				...
				for j = 1; j <= (*n); j++ {
					colssq.Set(0, zero)
					colssq.Set(1, one)
					Zlassq(toPtr(min((*n)-j+1, (*k)+1)), ab.CVector(0, j-1), func() *int { y := 1; return &y }(), colssq.GetPtr(0), colssq.GetPtr(1))
					Dcombssq(ssq, colssq)
				}
			}
		}
