...
func Zlals0(icompq, nl, nr, sqre, nrhs *int, b *mat.CMatrix, ldb *int, bx *mat.CMatrix, ldbx *int, perm *[]int, givptr *int, givcol *[]int, ldgcol *int, givnum *mat.Matrix, ldgnum *int, poles *mat.Matrix, difl *mat.Vector, difr *mat.Matrix, z *mat.Vector, k *int, c, s *float64, rwork *mat.Vector, info *int) {
	...
	if (*icompq) == 0 {
		...
		if (*k) < maxint(m, n) {
			Zlacpy('A', toPtr(n-(*k)), nrhs, bx.Off((*k)+1-1, 0), ldbx, b.Off((*k)+1-1, 0), ldb)
		}
	} else {
			...
			for j = 1; j <= (*k); j++ {
				...
				for i = 1; i <= j-1; i++ {
					if z.Get(j-1) == zero {
						...
						rwork.Set(i-1, z.Get(j-1)/(Dlamc3(&dsigj, toPtrf64(-poles.Get(i+1-1, 1)))-difr.Get(i-1, 0))/(dsigj+poles.Get(i-1, 0))/difr.Get(i-1, 1))
					}
				}
				for i = j + 1; i <= (*k); i++ {
					if z.Get(j-1) == zero {
