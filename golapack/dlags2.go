...
func Dlags2(upper bool, a1, a2, a3, b1, b2, b3, csu, snu, csv, snv, csq, snq *float64) {
	var a, aua11, aua12, aua21, aua22, avb11, avb12, avb21, avb22, b, c, csl, csr, d, r, s1, s2, snl, snr, ua11, ua11r, ua12, ua21, ua22, ua22r, vb11, vb11r, vb12, vb21, vb22, vb22r, zero float64
		...
		if math.Abs(csl) >= math.Abs(snl) || math.Abs(csr) >= math.Abs(snr) {
			...
			if (math.Abs(ua11r) + math.Abs(ua12)) != zero {
				if aua12/(math.Abs(ua11r)+math.Abs(ua12)) <= avb12/(math.Abs(vb11r)+math.Abs(vb12)) {
					Dlartg(toPtrf64(-ua11r), &ua12, csq, snq, &r)
				} else {
					Dlartg(toPtrf64(-vb11r), &vb12, csq, snq, &r)
				}
			} else {
				Dlartg(toPtrf64(-vb11r), &vb12, csq, snq, &r)
			}
			...
			if (math.Abs(ua21) + math.Abs(ua22)) != zero {
				if aua22/(math.Abs(ua21)+math.Abs(ua22)) <= avb22/(math.Abs(vb21)+math.Abs(vb22)) {
					Dlartg(toPtrf64(-ua21), &ua22, csq, snq, &r)
				} else {
					Dlartg(toPtrf64(-vb21), &vb22, csq, snq, &r)
				}
			} else {
				Dlartg(toPtrf64(-vb21), &vb22, csq, snq, &r)
			}
