...
// complex matrix A.
func Zlange(norm byte, m, n int, a *mat.CMatrix, work *mat.Vector) (zlangeReturn float64) {
	...
	if min(m, n) == 0 {
		...
		for j = 1; j <= n; j++ {
			colssq.Set(0, zero)
			colssq.Set(1, one)
			*colssq.GetPtr(0), *colssq.GetPtr(1) = Zlassq(m, a.Off(0, j-1).CVector(), 1, colssq.Get(0), colssq.Get(1))
			Dcombssq(ssq, colssq)
		}
