...
func Zunmqr(side, trans byte, m, n, k *int, a *mat.CMatrix, lda *int, tau *mat.CVector, c *mat.CMatrix, ldc *int, work *mat.CVector, lwork, info *int) {
	...
	if (*info) == 0 {
		...
		nb = min(nbmax, Ilaenv(func() *int { y := 1; return &y }(), []byte("ZUNMQR"), []byte{side, trans}, m, n, k, toPtr(-1)))
	...
	if nb > 1 && nb < (*k) {
		if (*lwork) < nw*nb+tsize {
			...
			nbmin = max(2, Ilaenv(func() *int { y := 2; return &y }(), []byte("ZUNMQR"), []byte{side, trans}, m, n, k, toPtr(-1)))
		}
	}
