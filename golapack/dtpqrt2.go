...
func Dtpqrt2(m, n, l *int, a *mat.Matrix, lda *int, b *mat.Matrix, ldb *int, t *mat.Matrix, ldt, info *int) {
	...
	for i = 1; i <= (*n); i++ {
		...
		if i < (*n) {
			...
			err = goblas.Dgemv(Trans, p, (*n)-i, one, b.Off(0, i), b.Vector(0, i-1, 1), one, t.Vector(0, (*n)-1, 1))
			...
			err = goblas.Dger(p, (*n)-i, alpha, b.Vector(0, i-1, 1), t.Vector(0, (*n)-1, 1), b.Off(0, i))
		}
	}
	...
	for i = 2; i <= (*n); i++ {
		...
		err = goblas.Dtrmv(Upper, Trans, NonUnit, p, b.Off(mp-1, 0), t.Vector(0, i-1, 1))

		//        Rectangular part of B2
		err = goblas.Dgemv(Trans, *l, i-1-p, alpha, b.Off(mp-1, np-1), b.Vector(mp-1, i-1, 1), zero, t.Vector(np-1, i-1, 1))

		//        B1
		err = goblas.Dgemv(Trans, (*m)-(*l), i-1, alpha, b, b.Vector(0, i-1, 1), one, t.Vector(0, i-1, 1))
