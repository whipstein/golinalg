...
func zlattb(imat int, uplo mat.MatUplo, trans mat.MatTrans, iseed *[]int, n, kd int, ab *mat.CMatrix, b, work *mat.CVector, rwork *mat.Vector) (diag mat.MatDiag) {
	var upper bool
	...
	if imat <= 5 {
		if err = matgen.Zlatms(n, n, dist, iseed, _type, rwork, mode, cndnum, anorm, kl, ku, packit, ab.Off(ioff-1, 0), work); err != nil {
			panic(err)
		}
		...
		if kd == 1 {
			if upper {
				ab.Set(0, 1, complex(tnorm, 0)*matgen.Zlarnd(5, *iseed))
				lenj = (n - 3) / 2
				golapack.Zlarnv(2, iseed, lenj, work)
				for j = 1; j <= lenj; j++ {
					ab.Set(0, 2*(j+1)-1, complex(tnorm, 0)*work.Get(j-1))
				}
			} else {
				ab.Set(1, 0, complex(tnorm, 0)*matgen.Zlarnd(5, *iseed))
				lenj = (n - 3) / 2
				golapack.Zlarnv(2, iseed, lenj, work)
				for j = 1; j <= lenj; j++ {
			...
			star1 = complex(tnorm, 0) * matgen.Zlarnd(5, *iseed)
		...
		if upper {
			...
			for j = n; j >= 1; j -= 1 {
				...
				if jcount <= 2 {
					ab.Set(kd, j-1, complex(smlnum, 0)*matgen.Zlarnd(5, *iseed))
				} else {
					ab.Set(kd, j-1, matgen.Zlarnd(5, *iseed))
				}
				jcount = jcount + 1
			...
			for j = 1; j <= n; j++ {
				...
				if jcount <= 2 {
					ab.Set(0, j-1, complex(smlnum, 0)*matgen.Zlarnd(5, *iseed))
				} else {
					ab.Set(0, j-1, matgen.Zlarnd(5, *iseed))
				}
				jcount = jcount + 1
		...
		if upper {
			for j = 1; j <= n; j++ {
				...
				ab.Set(kd, j-1, complex(tscal, 0)*matgen.Zlarnd(5, *iseed))
			}
			...
			for j = 1; j <= n; j++ {
				...
				ab.Set(0, j-1, complex(tscal, 0)*matgen.Zlarnd(5, *iseed))
			}
