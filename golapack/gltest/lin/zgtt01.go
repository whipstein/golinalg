...
func Zgtt01(n *int, dl, d, du, dlf, df, duf, du2 *mat.CVector, ipiv *[]int, work *mat.CMatrix, ldwork *int, rwork *mat.Vector, resid *float64) {
	...
	for i = (*n) - 1; i >= 1; i-- {
		li = dlf.Get(i - 1)
		goblas.Zaxpy(lastj-i+1, li, work.CVector(i-1, i-1, *ldwork), work.CVector(i, i-1, *ldwork))
		ip = (*ipiv)[i-1]
		if ip == i {
			lastj = min(i+2, *n)
		} else {
			goblas.Zswap(lastj-i+1, work.CVector(i-1, i-1, *ldwork), work.CVector(i, i-1, *ldwork))
		}
	}
