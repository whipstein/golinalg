...
func Zchkqr(dotype *[]bool, nm *int, mval *[]int, nn *int, nval *[]int, nnb *int, nbval *[]int, nxval *[]int, nrhs *int, thresh *float64, tsterr *bool, nmax *int, a, af, aq, ar, ac, b, x, xact, tau, work *mat.CVector, rwork *mat.Vector, iwork *[]int, nout *int, t *testing.T) {
	var dist, _type byte
				...
				*srnamt = "ZLATMS"
				...
				for ik = 1; ik <= nk; ik++ {
					...
					for inb = 1; inb <= (*nnb); inb++ {
						...
						if ik == 1 {
							//                       Test ZGEQRF
							Zqrt01(&m, &n, a.CMatrix(lda, opts), af.CMatrix(lda, opts), aq.CMatrix(lda, opts), ar.CMatrix(lda, opts), &lda, tau, work, &lwork, rwork, result.Off(0))

							//                       Test ZGEQRFP
							Zqrt01p(&m, &n, a.CMatrix(lda, opts), af.CMatrix(lda, opts), aq.CMatrix(lda, opts), ar.CMatrix(lda, opts), &lda, tau, work, &lwork, rwork, result.Off(7))
							if !Zgennd(&m, &n, af.CMatrix(lda, opts), &lda) {
							...
							//                       returned by ZQRT01
							Zqrt02(&m, &n, &k, a.CMatrix(lda, opts), af.CMatrix(lda, opts), aq.CMatrix(lda, opts), ar.CMatrix(lda, opts), &lda, tau, work, &lwork, rwork, result.Off(0))
						}
						if m >= k {
							//                       Test ZUNMQR, using factorization returned
							//                       by ZQRT01
							Zqrt03(&m, &n, &k, af.CMatrix(lda, opts), ac.CMatrix(lda, opts), ar.CMatrix(lda, opts), aq.CMatrix(lda, opts), &lda, tau, work, &lwork, rwork, result.Off(2))
							nt = nt + 4
							...
							if k == n && inb == 1 {
								...
								*srnamt = "ZGEQRS"
								Zgeqrs(&m, &n, nrhs, af.CMatrix(lda, opts), &lda, tau, x.CMatrix(lda, opts), &lda, work, &lwork, &info)

								//                          Check error code from ZGEQRS.
								if info != 0 {
									t.Fail()
									Alaerh(path, []byte("ZGEQRS"), &info, func() *int { y := 0; return &y }(), []byte{' '}, &m, &n, nrhs, toPtr(-1), &nb, &imat, &nfail, &nerrs)
								}

								Zget02('N', &m, &n, nrhs, a.CMatrix(lda, opts), &lda, x.CMatrix(lda, opts), &lda, b.CMatrix(lda, opts), &lda, rwork, result.GetPtr(6))
								nt = nt + 1
							}
						}
