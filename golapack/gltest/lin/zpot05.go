...
func Zpot05(uplo byte, n, nrhs *int, a *mat.CMatrix, lda *int, b *mat.CMatrix, ldb *int, x *mat.CMatrix, ldx *int, xact *mat.CMatrix, ldxact *int, ferr, berr, reslts *mat.Vector) {
	var upper bool
	...
	for k = 1; k <= (*nrhs); k++ {
		for i = 1; i <= (*n); i++ {
			...
			if upper {
				...
				tmp = tmp + math.Abs(real(a.Get(i-1, i-1)))*Cabs1(x.Get(i-1, k-1))
				for j = i + 1; j <= (*n); j++ {
