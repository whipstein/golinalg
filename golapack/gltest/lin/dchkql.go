...
func dchkql(dotype []bool, nm int, mval []int, nn int, nval []int, nnb int, nbval []int, nxval []int, nrhs int, thresh float64, tsterr bool, nmax int, a, af, aq, al, ac, b, x, xact, tau, work, rwork *mat.Vector, t *testing.T) {
	var dist, _type byte
			...
			for imat = 1; imat <= ntypes; imat++ {
				...
				for ik = 1; ik <= nk; ik++ {
					...
					for inb = 1; inb <= nnb; inb++ {
						...
						if ik == 1 {
							//                       Test DGEQLF
							dqlt01(m, n, a.Matrix(lda, opts), af.Matrix(lda, opts), aq.Matrix(lda, opts), al.Matrix(lda, opts), tau, work, lwork, rwork, result)
						} else if m >= n {
							//                       Test DORGQL, using factorization
							//                       returned by DQLT01
							dqlt02(m, n, k, a.Matrix(lda, opts), af.Matrix(lda, opts), aq.Matrix(lda, opts), al.Matrix(lda, opts), tau, work, lwork, rwork, result)
						}
						if m >= k {
							//                       Test DORMQL, using factorization returned
							//                       by DQLT01
							dqlt03(m, n, k, af.Matrix(lda, opts), ac.Matrix(lda, opts), al.Matrix(lda, opts), aq.Matrix(lda, opts), tau, work, lwork, rwork, result.Off(2))
							nt = nt + 4
							...
							if k == n && inb == 1 {
								...
								*srnamt = "Dlarhs"
								_in := byte('N')
								if err = Dlarhs(path, _in, Full, NoTrans, m, n, 0, 0, nrhs, a.Matrix(lda, opts), xact.Matrix(lda, opts), b.Matrix(lda, opts), &iseed); err != nil {
									panic(err)
								}

								golapack.Dlacpy(Full, m, nrhs, b.Matrix(lda, opts), x.Matrix(lda, opts))
								*srnamt = "Dgeqls"
								if err = dgeqls(m, n, nrhs, af.Matrix(lda, opts), tau, x.Matrix(lda, opts), work, lwork); err != nil {
