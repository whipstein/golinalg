...
func Dchkql(dotype *[]bool, nm *int, mval *[]int, nn *int, nval *[]int, nnb *int, nbval *[]int, nxval *[]int, nrhs *int, thresh *float64, tsterr *bool, nmax *int, a, af, aq, al, ac, b, x, xact, tau, work, rwork *mat.Vector, nout *int, t *testing.T) {
	var dist, _type byte
			...
			for imat = 1; imat <= ntypes; imat++ {
				...
				for ik = 1; ik <= nk; ik++ {
					...
					for inb = 1; inb <= (*nnb); inb++ {
						...
						if ik == 1 {
							//                       Test DGEQLF
							Dqlt01(&m, &n, a.Matrix(lda, opts), af.Matrix(lda, opts), aq.Matrix(lda, opts), al.Matrix(lda, opts), &lda, tau, work, &lwork, rwork, result)
						} else if m >= n {
							//                       Test DORGQL, using factorization
							//                       returned by DQLT01
							Dqlt02(&m, &n, &k, a.Matrix(lda, opts), af.Matrix(lda, opts), aq.Matrix(lda, opts), al.Matrix(lda, opts), &lda, tau, work, &lwork, rwork, result)
						}
						if m >= k {
							//                       Test DORMQL, using factorization returned
							//                       by DQLT01
							Dqlt03(&m, &n, &k, af.Matrix(lda, opts), ac.Matrix(lda, opts), al.Matrix(lda, opts), aq.Matrix(lda, opts), &lda, tau, work, &lwork, rwork, result.Off(2))
							nt = nt + 4
							...
							if k == n && inb == 1 {
								...
								*srnamt = "DGEQLS"
								Dgeqls(&m, &n, nrhs, af.Matrix(lda, opts), &lda, tau, x.Matrix(lda, opts), &lda, work, &lwork, &info)
