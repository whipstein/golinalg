...
func Dlattp(imat *int, uplo, trans byte, diag *byte, iseed *[]int, n *int, a, b, work *mat.Vector, info *int) {
	var upper bool
	...
	if (*imat) <= 6 {
		matgen.Dlatms(n, n, dist, iseed, _type, b, &mode, &cndnum, &anorm, &kl, &ku, packit, a.Matrix(*n, opts), n, work, info)
		...
		if upper {
			jc = 1
			for j = 1; j <= (*n); j++ {
				golapack.Dlarnv(func() *int { y := 2; return &y }(), iseed, &j, a.Off(jc-1))
			...
			jc = 1
			for j = 1; j <= (*n); j++ {
				golapack.Dlarnv(func() *int { y := 2; return &y }(), iseed, toPtr((*n)-j+1), a.Off(jc-1))
