...
func Zqlt01(m, n *int, a, af, q, l *mat.CMatrix, lda *int, tau, work *mat.CVector, lwork *int, rwork, result *mat.Vector) {
	...
	*srnamt = "ZGEQLF"
	...
	if (*m) >= (*n) {
		if (*n) < (*m) && (*n) > 0 {
			golapack.Zlacpy('F', toPtr((*m)-(*n)), n, af, lda, q.Off(0, (*m)-(*n)), lda)
		}
		if (*n) > 1 {
			golapack.Zlacpy('U', toPtr((*n)-1), toPtr((*n)-1), af.Off((*m)-(*n), 1), lda, q.Off((*m)-(*n), (*m)-(*n)+2-1), lda)
		}
	} else {
		if (*m) > 1 {
	...
	*srnamt = "ZUNGQL"
	...
	if (*m) >= (*n) {
		if (*n) > 0 {
			golapack.Zlacpy('L', n, n, af.Off((*m)-(*n), 0), lda, l.Off((*m)-(*n), 0), lda)
		}
	} else {
		if (*n) > (*m) && (*m) > 0 {
		...
		if (*m) > 0 {
			golapack.Zlacpy('L', m, m, af.Off(0, (*n)-(*m)), lda, l.Off(0, (*n)-(*m)), lda)
		}
	}

	//     Compute L - Q'*A
	err = goblas.Zgemm(ConjTrans, NoTrans, *m, *n, *m, complex(-one, 0), q, a, complex(one, 0), l)
	...
	err = goblas.Zherk(Upper, ConjTrans, *m, *m, -one, q, one, l)
