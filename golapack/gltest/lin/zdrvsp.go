		...
		for imat = 1; imat <= nimat; imat++ {
			...
			for iuplo = 1; iuplo <= 2; iuplo++ {
				...
				for ifact = 1; ifact <= nfact; ifact++ {
					...
					*srnamt = "ZSPSVX"
					golapack.Zspsvx(fact, uplo, &n, nrhs, a, afac, iwork, b.CMatrix(lda, opts), &lda, x.CMatrix(lda, opts), &lda, &rcond, rwork, rwork.Off((*nrhs)), work, rwork.Off(2*(*nrhs)), &info)
					...
					if info == 0 {
						if ifact >= 2 {
							//                       Reconstruct matrix from factors and compute
							//                       residual.
							Zspt01(uplo, &n, a, afac, iwork, ainv.CMatrix(lda, opts), &lda, rwork.Off(2*(*nrhs)), result.GetPtr(0))
							k1 = 1
						} else {
							k1 = 2
						}
						...
						//                    Compute residual of the computed solution.
						golapack.Zlacpy('F', &n, nrhs, b.CMatrix(lda, opts), &lda, work.CMatrix(lda, opts), &lda)
						Zspt02(uplo, &n, nrhs, a, x.CMatrix(lda, opts), &lda, work.CMatrix(lda, opts), &lda, rwork.Off(2*(*nrhs)), result.GetPtr(1))
