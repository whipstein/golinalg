...
func Zdrvsp(dotype *[]bool, nn *int, nval *[]int, nrhs *int, thresh *float64, tsterr *bool, nmax *int, a, afac, ainv, b, x, xact, work *mat.CVector, rwork *mat.Vector, iwork *[]int, nout *int, t *testing.T) {
	var zerot bool
				...
				for ifact = 1; ifact <= nfact; ifact++ {
					...
					*srnamt = "ZLARHS"
					...
					if ifact == 2 {
						...
						*srnamt = "ZSPSV "
						golapack.Zspsv(uplo, &n, nrhs, afac, iwork, x.CMatrix(lda, opts), &lda, &info)
						...
						if info != k {
							t.Fail()
							Alaerh(path, []byte("ZSPSV "), &info, &k, []byte{uplo}, &n, &n, toPtr(-1), toPtr(-1), nrhs, &imat, &nfail, &nerrs)
							goto label120
						} else if info != 0 {
							goto label120
						}
						...
						Zspt02(uplo, &n, nrhs, a, x.CMatrix(lda, opts), &lda, work.CMatrix(lda, opts), &lda, rwork, result.GetPtr(1))
					...
					//                 --- Test ZSPSVX ---
					if ifact == 2 && npp > 0 {
						golapack.Zlaset('F', &npp, func() *int { y := 1; return &y }(), toPtrc128(complex(zero, 0)), toPtrc128(complex(zero, 0)), afac.CMatrix(lda, opts), &npp)
					}
					...
					*srnamt = "ZSPSVX"
					golapack.Zspsvx(fact, uplo, &n, nrhs, a, afac, iwork, b.CMatrix(lda, opts), &lda, x.CMatrix(lda, opts), &lda, &rcond, rwork, rwork.Off((*nrhs)+1-1), work, rwork.Off(2*(*nrhs)+1-1), &info)
					...
					if info != k {
						Alaerh(path, []byte("ZSPSVX"), &info, &k, []byte{fact, uplo}, &n, &n, toPtr(-1), toPtr(-1), nrhs, &imat, &nfail, &nerrs)
						goto label150
					}
					...
					if info == 0 {
						if ifact >= 2 {
							//                       Reconstruct matrix from factors and compute
							//                       residual.
							Zspt01(uplo, &n, a, afac, iwork, ainv.CMatrix(lda, opts), &lda, rwork.Off(2*(*nrhs)+1-1), result.GetPtr(0))
							k1 = 1
						} else {
							k1 = 2
						}
						...
						//                    Compute residual of the computed solution.
						golapack.Zlacpy('F', &n, nrhs, b.CMatrix(lda, opts), &lda, work.CMatrix(lda, opts), &lda)
						Zspt02(uplo, &n, nrhs, a, x.CMatrix(lda, opts), &lda, work.CMatrix(lda, opts), &lda, rwork.Off(2*(*nrhs)+1-1), result.GetPtr(1))
