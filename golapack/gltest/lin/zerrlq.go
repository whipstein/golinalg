...
func zerrlq(path string, t *testing.T) {
	...
	*srnamt = "zgelqs"
	*errt = fmt.Errorf("m < 0: m=-1")
	err = zgelqs(-1, 0, 0, a.Off(0, 0).UpdateRows(1), x, b.CMatrix(1, opts), w, 1)
	chkxer2("zgelqs", err)
	*errt = fmt.Errorf("n < 0 || m > n: m=0, n=-1")
	err = zgelqs(0, -1, 0, a.Off(0, 0).UpdateRows(1), x, b.CMatrix(1, opts), w, 1)
	chkxer2("zgelqs", err)
	*errt = fmt.Errorf("n < 0 || m > n: m=2, n=1")
	err = zgelqs(2, 1, 0, a.Off(0, 0).UpdateRows(2), x, b.CMatrix(1, opts), w, 1)
	chkxer2("zgelqs", err)
	*errt = fmt.Errorf("nrhs < 0: nrhs=-1")
	err = zgelqs(0, 0, -1, a.Off(0, 0).UpdateRows(1), x, b.CMatrix(1, opts), w, 1)
	chkxer2("zgelqs", err)
	*errt = fmt.Errorf("a.Rows < max(1, m): a.Rows=1, m=2")
	err = zgelqs(2, 2, 0, a.Off(0, 0).UpdateRows(1), x, b.CMatrix(2, opts), w, 1)
	chkxer2("zgelqs", err)
	*errt = fmt.Errorf("b.Rows < max(1, n): b.Rows=1, n=2")
	err = zgelqs(1, 2, 0, a.Off(0, 0).UpdateRows(1), x, b.CMatrix(1, opts), w, 1)
	chkxer2("zgelqs", err)
	*errt = fmt.Errorf("lwork < 1 || lwork < nrhs && m > 0 && n > 0: lwork=1, nrhs=2, m=1, n=1")
	err = zgelqs(1, 1, 2, a.Off(0, 0).UpdateRows(1), x, b.CMatrix(1, opts), w, 1)
	chkxer2("zgelqs", err)

	//     Zunglq
	*srnamt = "Zunglq"
	*errt = fmt.Errorf("m < 0: m=-1")
	err = golapack.Zunglq(-1, 0, 0, a.Off(0, 0).UpdateRows(1), x, w, 1)
	chkxer2("Zunglq", err)
	*errt = fmt.Errorf("n < m: m=0, n=-1")
	err = golapack.Zunglq(0, -1, 0, a.Off(0, 0).UpdateRows(1), x, w, 1)
	chkxer2("Zunglq", err)
	*errt = fmt.Errorf("n < m: m=2, n=1")
	err = golapack.Zunglq(2, 1, 0, a.Off(0, 0).UpdateRows(2), x, w, 2)
	chkxer2("Zunglq", err)
	*errt = fmt.Errorf("k < 0 || k > m: m=0, k=-1")
	err = golapack.Zunglq(0, 0, -1, a.Off(0, 0).UpdateRows(1), x, w, 1)
	chkxer2("Zunglq", err)
	*errt = fmt.Errorf("k < 0 || k > m: m=1, k=2")
	err = golapack.Zunglq(1, 1, 2, a.Off(0, 0).UpdateRows(1), x, w, 1)
	chkxer2("Zunglq", err)
	*errt = fmt.Errorf("a.Rows < max(1, m): a.Rows=1, m=2")
	err = golapack.Zunglq(2, 2, 0, a.Off(0, 0).UpdateRows(1), x, w, 2)
	chkxer2("Zunglq", err)
	*errt = fmt.Errorf("lwork < max(1, m) && !lquery: lwork=1, m=2, lquery=false")
	err = golapack.Zunglq(2, 2, 0, a.Off(0, 0).UpdateRows(2), x, w, 1)
	chkxer2("Zunglq", err)
	...
	*errt = fmt.Errorf("!left && side != Right: side=Unrecognized: /")
	err = golapack.Zunmlq('/', NoTrans, 0, 0, 0, a.Off(0, 0).UpdateRows(1), x, af.Off(0, 0).UpdateRows(1), w, 1)
	chkxer2("Zunmlq", err)
	*errt = fmt.Errorf("!notran && trans != ConjTrans: trans=Unrecognized: /")
	err = golapack.Zunmlq(Left, '/', 0, 0, 0, a.Off(0, 0).UpdateRows(1), x, af.Off(0, 0).UpdateRows(1), w, 1)
	chkxer2("Zunmlq", err)
	*errt = fmt.Errorf("m < 0: m=-1")
	err = golapack.Zunmlq(Left, NoTrans, -1, 0, 0, a.Off(0, 0).UpdateRows(1), x, af.Off(0, 0).UpdateRows(1), w, 1)
	chkxer2("Zunmlq", err)
	*errt = fmt.Errorf("n < 0: n=-1")
	err = golapack.Zunmlq(Left, NoTrans, 0, -1, 0, a.Off(0, 0).UpdateRows(1), x, af.Off(0, 0).UpdateRows(1), w, 1)
	chkxer2("Zunmlq", err)
	*errt = fmt.Errorf("k < 0 || k > nq: k=-1, nq=0")
	err = golapack.Zunmlq(Left, NoTrans, 0, 0, -1, a.Off(0, 0).UpdateRows(1), x, af.Off(0, 0).UpdateRows(1), w, 1)
	chkxer2("Zunmlq", err)
	*errt = fmt.Errorf("k < 0 || k > nq: k=1, nq=0")
	err = golapack.Zunmlq(Left, NoTrans, 0, 1, 1, a.Off(0, 0).UpdateRows(1), x, af.Off(0, 0).UpdateRows(1), w, 1)
	chkxer2("Zunmlq", err)
	*errt = fmt.Errorf("k < 0 || k > nq: k=1, nq=0")
	err = golapack.Zunmlq(Right, NoTrans, 1, 0, 1, a.Off(0, 0).UpdateRows(1), x, af.Off(0, 0).UpdateRows(1), w, 1)
	chkxer2("Zunmlq", err)
	*errt = fmt.Errorf("a.Rows < max(1, k): a.Rows=1, k=2")
	err = golapack.Zunmlq(Left, NoTrans, 2, 0, 2, a.Off(0, 0).UpdateRows(1), x, af.Off(0, 0).UpdateRows(2), w, 1)
	chkxer2("Zunmlq", err)
	*errt = fmt.Errorf("a.Rows < max(1, k): a.Rows=1, k=2")
	err = golapack.Zunmlq(Right, NoTrans, 0, 2, 2, a.Off(0, 0).UpdateRows(1), x, af.Off(0, 0).UpdateRows(1), w, 1)
	chkxer2("Zunmlq", err)
	*errt = fmt.Errorf("c.Rows < max(1, m): c.Rows=1, m=2")
	err = golapack.Zunmlq(Left, NoTrans, 2, 1, 0, a.Off(0, 0).UpdateRows(2), x, af.Off(0, 0).UpdateRows(1), w, 1)
	chkxer2("Zunmlq", err)
	*errt = fmt.Errorf("lwork < max(1, nw) && !lquery: lwork=1, nw=2, lquery=false")
	err = golapack.Zunmlq(Left, NoTrans, 1, 2, 0, a.Off(0, 0).UpdateRows(1), x, af.Off(0, 0).UpdateRows(1), w, 1)
	chkxer2("Zunmlq", err)
	*errt = fmt.Errorf("lwork < max(1, nw) && !lquery: lwork=1, nw=2, lquery=false")
	err = golapack.Zunmlq(Right, NoTrans, 2, 1, 0, a.Off(0, 0).UpdateRows(1), x, af.Off(0, 0).UpdateRows(2), w, 1)
	chkxer2("Zunmlq", err)
