...
func Zchkrq(dotype *[]bool, nm *int, mval *[]int, nn *int, nval *[]int, nnb *int, nbval *[]int, nxval *[]int, nrhs *int, thresh *float64, tsterr *bool, nmax *int, a, af, aq, ar, ac, b, x, xact, tau, work *mat.CVector, rwork *mat.Vector, iwork *[]int, nout *int, t *testing.T) {
	var dist, _type byte
				...
				*srnamt = "ZLATMS"
				...
				for ik = 1; ik <= nk; ik++ {
					...
					for inb = 1; inb <= (*nnb); inb++ {
						...
						if ik == 1 {
							//                       Test ZGERQF
							Zrqt01(&m, &n, a.CMatrix(lda, opts), af.CMatrix(lda, opts), aq.CMatrix(lda, opts), ar.CMatrix(lda, opts), &lda, tau, work, &lwork, rwork, result.Off(0))
						} else if m <= n {
							//                       Test ZUNGRQ, using factorization
							//                       returned by ZRQT01
							Zrqt02(&m, &n, &k, a.CMatrix(lda, opts), af.CMatrix(lda, opts), aq.CMatrix(lda, opts), ar.CMatrix(lda, opts), &lda, tau, work, &lwork, rwork, result.Off(0))
						}
						if m >= k {
							//                       Test ZUNMRQ, using factorization returned
							//                       by ZRQT01
							Zrqt03(&m, &n, &k, af.CMatrix(lda, opts), ac.CMatrix(lda, opts), ar.CMatrix(lda, opts), aq.CMatrix(lda, opts), &lda, tau, work, &lwork, rwork, result.Off(2))
							nt = nt + 4
							...
							if k == m && inb == 1 {
								...
								*srnamt = "ZGERQS"
								...
								if info != 0 {
									t.Fail()
									Alaerh(path, []byte("ZGERQS"), &info, func() *int { y := 0; return &y }(), []byte{' '}, &m, &n, nrhs, toPtr(-1), &nb, &imat, &nfail, &nerrs)
								}
