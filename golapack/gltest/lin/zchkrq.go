...
func zchkrq(dotype []bool, nm int, mval []int, nn int, nval []int, nnb int, nbval []int, nxval []int, nrhs int, thresh float64, tsterr bool, nmax int, a, af, aq, ar, ac, b, x, xact, tau, work *mat.CVector, rwork *mat.Vector, iwork []int, t *testing.T) {
	var dist, _type byte
				...
				for ik = 1; ik <= nk; ik++ {
					...
					for inb = 1; inb <= nnb; inb++ {
						...
						if m >= k {
							...
							//                       by ZRQT01
							zrqt03(m, n, k, af.CMatrix(lda, opts), ac.CMatrix(lda, opts), ar.CMatrix(lda, opts), aq.CMatrix(lda, opts), tau, work, lwork, rwork, result.Off(2))
							nt = nt + 4
							...
							if k == m && inb == 1 {
								...
								*srnamt = "zgerqs"
								if err = zgerqs(m, n, nrhs, af.CMatrix(lda, opts), tau, x.CMatrix(lda, opts), work, lwork); err != nil {
									t.Fail()
									nerrs = alaerh(path, "zgerqs", info, 0, []byte{' '}, m, n, nrhs, -1, nb, imat, nfail, nerrs)
								}
