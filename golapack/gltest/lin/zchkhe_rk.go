...
// Zchkherk tests ZHETRF_RK, -TRI_3, -TRS_3,
// and -CON_3.
func Zchkherk(dotype *[]bool, nn *int, nval *[]int, nnb *int, nbval *[]int, nns *int, nsval *[]int, thresh *float64, tsterr *bool, nmax *int, a, afac, e, ainv, b, x, xact, work *mat.CVector, rwork *mat.Vector, iwork *[]int, nout *int, t *testing.T) {
			...
			for iuplo = 1; iuplo <= 2; iuplo++ {
				...
				*srnamt = "ZLATMS"
				...
				for inb = 1; inb <= (*nnb); inb++ {
					...
					lwork = max(2, nb) * lda
					*srnamt = "ZHETRF_RK"
					golapack.Zhetrfrk(uplo, &n, afac.CMatrix(lda, opts), &lda, e, iwork, ainv, &lwork, &info)
					...
					if info != k {
						t.Fail()
						Alaerh(path, []byte("ZHETRF_RK"), &info, &k, []byte{uplo}, &n, &n, toPtr(-1), toPtr(-1), &nb, &imat, &nfail, &nerrs)
					}
					...
					if inb == 1 && !trfcon {
						...
						*srnamt = "ZHETRI_3"
						...
						if info != 0 {
							t.Fail()
							Alaerh(path, []byte("ZHETRI_3"), &info, toPtr(-1), []byte{uplo}, &n, &n, toPtr(-1), toPtr(-1), toPtr(-1), &imat, &nfail, &nerrs)
						}
					...
					for irhs = 1; irhs <= (*nns); irhs++ {
						...
						*srnamt = "ZHETRS_3"
						golapack.Zhetrs3(uplo, &n, &nrhs, afac.CMatrix(lda, opts), &lda, e, iwork, x.CMatrix(lda, opts), &lda, &info)

						//                    Check error code from ZHETRS_3 and handle error.
						if info != 0 {
							t.Fail()
							Alaerh(path, []byte("ZHETRS_3"), &info, func() *int { y := 0; return &y }(), []byte{uplo}, &n, &n, toPtr(-1), toPtr(-1), &nrhs, &imat, &nfail, &nerrs)
						}

						golapack.Zlacpy('F', &n, &nrhs, b.CMatrix(lda, opts), &lda, work.CMatrix(lda, opts), &lda)

						//                    Compute the residual for the solution
						Zpot02(uplo, &n, &nrhs, a.CMatrix(lda, opts), &lda, x.CMatrix(lda, opts), &lda, work.CMatrix(lda, opts), &lda, rwork, result.GetPtr(4))
				...
				label230:
					...
					*srnamt = "ZHECON_3"
					golapack.Zhecon3(uplo, &n, afac.CMatrix(lda, opts), &lda, e, iwork, &anorm, &rcond, work, &info)

					//                 Check error code from ZHECON_3 and handle error.
					if info != 0 {
						t.Fail()
						Alaerh(path, []byte("ZHECON_3"), &info, func() *int { y := 0; return &y }(), []byte{uplo}, &n, &n, toPtr(-1), toPtr(-1), toPtr(-1), &imat, &nfail, &nerrs)
					}
