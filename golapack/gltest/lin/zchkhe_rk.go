...
// zchkheRk tests ZhetrfRk, -TRI_3, -TRS_3,
// and -CON_3.
func zchkheRk(dotype []bool, nn int, nval []int, nnb int, nbval []int, nns int, nsval []int, thresh float64, tsterr bool, nmax int, a, afac, e, ainv, b, x, xact, work *mat.CVector, rwork *mat.Vector, iwork []int, t *testing.T) {
			...
			for _, uplo = range mat.IterMatUplo(false) {
				...
				*srnamt = "Zlatms"
				...
				for inb = 1; inb <= nnb; inb++ {
					...
					lwork = max(2, nb) * lda
					*srnamt = "ZhetrfRk"
					info, err = golapack.ZhetrfRk(uplo, n, afac.CMatrix(lda, opts), e, &iwork, ainv, lwork)
					...
					//                 Check error code from ZhetrfRk and handle error.
					if err != nil || info != k {
						t.Fail()
						nerrs = alaerh(path, "ZhetrfRk", info, k, []byte{uplo.Byte()}, n, n, -1, -1, nb, imat, nfail, nerrs)
					}
					...
					*result.GetPtr(0) = zhet013(uplo, n, a.CMatrix(lda, opts), afac.CMatrix(lda, opts), e, &iwork, ainv.CMatrix(lda, opts), rwork)
					...
					if inb == 1 && !trfcon {
						...
						*srnamt = "Zhetri3"
						...
						//                    in TEST6 and TEST7.
						lwork = (n + nb + 1) * (nb + 3)
						info, err = golapack.Zhetri3(uplo, n, ainv.CMatrix(lda, opts), e, &iwork, work, lwork)

						//                    Check error code from Zhetri3 and handle error.
						if err != nil || info != 0 {
							t.Fail()
							nerrs = alaerh(path, "Zhetri3", info, -1, []byte{uplo.Byte()}, n, n, -1, -1, -1, imat, nfail, nerrs)
						}
					...
					for irhs = 1; irhs <= nns; irhs++ {
						...
						*srnamt = "Zhetrs3"
						if err = golapack.Zhetrs3(uplo, n, nrhs, afac.CMatrix(lda, opts), e, &iwork, x.CMatrix(lda, opts)); err != nil {
							t.Fail()
							nerrs = alaerh(path, "Zhetrs3", info, 0, []byte{uplo.Byte()}, n, n, -1, -1, nrhs, imat, nfail, nerrs)
						}
				...
				label230:
					...
					*srnamt = "Zhecon3"
					if rcond, err = golapack.Zhecon3(uplo, n, afac.CMatrix(lda, opts), e, &iwork, anorm, work); err != nil {
						t.Fail()
						nerrs = alaerh(path, "Zhecon3", info, 0, []byte{uplo.Byte()}, n, n, -1, -1, -1, imat, nfail, nerrs)
					}
