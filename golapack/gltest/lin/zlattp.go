...
func Zlattp(imat *int, uplo, trans byte, diag *byte, iseed *[]int, n *int, ap, b, work *mat.CVector, rwork *mat.Vector, info *int) {
	var upper bool
	...
	if (*imat) <= 6 {
		matgen.Zlatms(n, n, dist, iseed, _type, rwork, &mode, &cndnum, &anorm, &kl, &ku, packit, ap.CMatrix(*n, opts), n, work, info)
		...
		for j = 1; j <= (*n); j += 2 {
			...
			if j+1 <= (*n) {
				...
				plus1 = star1 / plus2
				rexp = real(matgen.Zlarnd(func() *int { y := 2; return &y }(), iseed))
				if rexp < zero {
		...
		if upper {
			...
			for j = 1; j <= (*n)-1; j++ {
				...
				if (*n) > j+1 {
					jx = jcnext + j
					for i = j + 2; i <= (*n); i++ {
						ctemp = complex(c, 0)*ap.Get(jx+j-1) + s*ap.Get(jx+j+1-1)
						ap.Set(jx+j+1-1, -cmplx.Conj(s)*ap.Get(jx+j-1)+complex(c, 0)*ap.Get(jx+j+1-1))
						ap.Set(jx+j-1, ctemp)
						jx = jx + i
					}
				}
				...
				if j > 1 {
					golapack.Zrot(toPtr(j-1), ap.Off(jcnext-1), func() *int { y := 1; return &y }(), ap.Off(jc-1), func() *int { y := 1; return &y }(), toPtrf64(-c), toPtrc128(-s))
				}
			...
			for j = 1; j <= (*n)-1; j++ {
				...
				//              Multiply by [ c -s;  conjg(s) c] on the right.
				if (*n) > j+1 {
					golapack.Zrot(toPtr((*n)-j-1), ap.Off(jcnext+1-1), func() *int { y := 1; return &y }(), ap.Off(jc+2-1), func() *int { y := 1; return &y }(), &c, toPtrc128(-s))
				}
				...
				if j > 1 {
					jx = 1
					for i = 1; i <= j-1; i++ {
						ctemp = complex(-c, 0)*ap.Get(jx+j-i-1) + s*ap.Get(jx+j-i+1-1)
						ap.Set(jx+j-i+1-1, -cmplx.Conj(s)*ap.Get(jx+j-i-1)-complex(c, 0)*ap.Get(jx+j-i+1-1))
		...
		if upper {
			...
			for j = 1; j <= (*n); j++ {
				...
				ap.Set(jc+j-1-1, matgen.Zlarnd(func() *int { y := 5; return &y }(), iseed)*complex(two, 0))
				jc = jc + j
			}
		} else {
			jc = 1
			for j = 1; j <= (*n); j++ {
				if j < (*n) {
					golapack.Zlarnv(func() *int { y := 4; return &y }(), iseed, toPtr((*n)-j), ap.Off(jc+1-1))
				}
				ap.Set(jc-1, matgen.Zlarnd(func() *int { y := 5; return &y }(), iseed)*complex(two, 0))
				jc = jc + (*n) - j + 1
			}
		}
		...
		if upper {
			...
			for j = (*n); j >= 1; j -= 1 {
				...
				if jcount <= 2 {
					ap.Set(jc+j-1-1, complex(smlnum, 0)*matgen.Zlarnd(func() *int { y := 5; return &y }(), iseed))
				} else {
					ap.Set(jc+j-1-1, matgen.Zlarnd(func() *int { y := 5; return &y }(), iseed))
				}
				jcount = jcount + 1
			...
			for j = 1; j <= (*n); j++ {
				...
				if jcount <= 2 {
					ap.Set(jc-1, complex(smlnum, 0)*matgen.Zlarnd(func() *int { y := 5; return &y }(), iseed))
				} else {
					ap.Set(jc-1, matgen.Zlarnd(func() *int { y := 5; return &y }(), iseed))
				}
				jcount = jcount + 1
		...
		if upper {
			...
			for j = 1; j <= (*n); j++ {
				...
				ap.Set(jc+j-1-1, complex(tscal, 0)*matgen.Zlarnd(func() *int { y := 5; return &y }(), iseed))
				jc = jc + j
			}
			...
			for j = 1; j <= (*n); j++ {
				...
				if j < (*n) {
					ap.Set(jc+1-1, complex(-one, -one))
				}
				ap.Set(jc-1, complex(tscal, 0)*matgen.Zlarnd(func() *int { y := 5; return &y }(), iseed))
				jc = jc + (*n) - j + 1
			}
		...
		if upper {
			jc = 1
			for j = 1; j <= (*n); j++ {
				golapack.Zlarnv(func() *int { y := 4; return &y }(), iseed, &j, ap.Off(jc-1))
				if j != iy {
					ap.Set(jc+j-1-1, matgen.Zlarnd(func() *int { y := 5; return &y }(), iseed)*complex(two, 0))
				} else {
			...
			jc = 1
			for j = 1; j <= (*n); j++ {
				golapack.Zlarnv(func() *int { y := 4; return &y }(), iseed, toPtr((*n)-j+1), ap.Off(jc-1))
				if j != iy {
					ap.Set(jc-1, matgen.Zlarnd(func() *int { y := 5; return &y }(), iseed)*complex(two, 0))
				} else {
					ap.SetRe(jc-1, zero)
		...
		if upper {
			jc = 1
			for j = 1; j <= (*n); j++ {
				golapack.Zlarnv(func() *int { y := 5; return &y }(), iseed, &j, ap.Off(jc-1))
			...
			jc = 1
			for j = 1; j <= (*n); j++ {
				golapack.Zlarnv(func() *int { y := 5; return &y }(), iseed, toPtr((*n)-j+1), ap.Off(jc-1))
				golapack.Dlarnv(func() *int { y := 1; return &y }(), iseed, toPtr((*n)-j+1), rwork)
				for i = j; i <= (*n); i++ {
					ap.Set(jc+i-j-1, ap.Get(jc+i-j-1)*complex(tleft+rwork.Get(i-j+1-1)*tscal, 0))
				}
				jc = jc + (*n) - j + 1
			}
		}
