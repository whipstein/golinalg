...
// and -CON_ROOK.
func dchksyRook(dotype []bool, nn int, nval []int, nnb int, nbval []int, nns int, nsval []int, thresh float64, tsterr bool, nmax int, a, afac, ainv, b, x, xact, work, rwork *mat.Vector, iwork []int, t *testing.T) {
			...
			for _, uplo = range mat.IterMatUplo(false) {
				...
				*srnamt = "Dlatms"
				...
				for inb = 1; inb <= nnb; inb++ {
					...
					lwork = max(2, nb) * lda
					*srnamt = "DsytrfRook"
					if info, err = golapack.DsytrfRook(uplo, n, afac.Matrix(lda, opts), &iwork, ainv.Matrix(lda, opts), lwork); err != nil {
						panic(err)
					}
					...
					if info != k {
						nerrs = alaerh(path, "DsytrfRook", info, k, []byte{uplo.Byte()}, n, n, -1, -1, nb, imat, nfail, nerrs)
					}
					...
					result.Set(0, Dsyt01Rook(uplo, n, a.Matrix(lda, opts), afac.Matrix(lda, opts), iwork, ainv.Matrix(lda, opts), rwork))
					nt = 1
					...
					if inb == 1 && !trfcon {
						...
						*srnamt = "DsytriRook"
						if info, err = golapack.DsytriRook(uplo, n, ainv.Matrix(lda, opts), &iwork, work); err != nil || info != 0 {
							nerrs = alaerh(path, "DsytriRook", info, -1, []byte{uplo.Byte()}, n, n, -1, -1, -1, imat, nfail, nerrs)
						}
					...
					label160:
						...
						if iwork[k-1] < int(zero) {
							...
							if info, err = golapack.Dgesvd('N', 'N', 2, 2, block, rwork, ddummy.Matrix(1, opts), ddummy.Matrix(1, opts), work, 10); err != nil {
								panic(err)
							}
					...
					label180:
						...
						if iwork[k-1] < int(zero) {
							...
							block.Set(1, 0, afac.Get(k+(k-1)*lda))
					...
					for irhs = 1; irhs <= nns; irhs++ {
						...
						*srnamt = "DsytrsRook"
						if err = golapack.DsytrsRook(uplo, n, nrhs, afac.Matrix(lda, opts), &iwork, x.Matrix(lda, opts)); err != nil {
							nerrs = alaerh(path, "DsytrsRook", info, 0, []byte{uplo.Byte()}, n, n, -1, -1, nrhs, imat, nfail, nerrs)
						}
				...
				label230:
					...
					*srnamt = "DsyconRook"
					if rcond, err = golapack.DsyconRook(uplo, n, afac.Matrix(lda, opts), &iwork, anorm, work, toSlice(&iwork, n)); err != nil {
						nerrs = alaerh(path, "DsyconRook", info, 0, []byte{uplo.Byte()}, n, n, -1, -1, -1, imat, nfail, nerrs)
					}
