		...
		label70:
			...
			if (*ipiv)[k-1] > 0 {
				if k > 1 {
					...
					err = goblas.Zgemv(ConjTrans, k-1, *nrhs, one, b, *ldb, a.Off(kc-1), 1, one, b.CVector(k-1, 0), *ldb)
					golapack.Zlacgv(nrhs, b.CVector(k-1, 0), ldb)
				}
				if nounit {
					goblas.Zscal(*nrhs, a.Get(kc+k-1-1), b.CVector(k-1, 0), *ldb)
				...
				if k > 2 {
					...
					//                 Apply the transformations.
					golapack.Zlacgv(nrhs, b.CVector(k-1, 0), ldb)
					err = goblas.Zgemv(ConjTrans, k-2, *nrhs, one, b, *ldb, a.Off(kc-1), 1, one, b.CVector(k-1, 0), *ldb)
					golapack.Zlacgv(nrhs, b.CVector(k-1, 0), ldb)

					golapack.Zlacgv(nrhs, b.CVector(k-1-1, 0), ldb)
					err = goblas.Zgemv(ConjTrans, k-2, *nrhs, one, b, *ldb, a.Off(kcnext-1), 1, one, b.CVector(k-1-1, 0), *ldb)
					golapack.Zlacgv(nrhs, b.CVector(k-1-1, 0), ldb)
				}
		...
		label100:
			...
			if (*ipiv)[k-1] > 0 {
				if k < (*n) {
					...
					//                 Apply the transformation
					golapack.Zlacgv(nrhs, b.CVector(k-1, 0), ldb)
					err = goblas.Zgemv(ConjTrans, (*n)-k, *nrhs, one, b.Off(k+1-1, 0), *ldb, a.Off(kc+1-1), 1, one, b.CVector(k-1, 0), *ldb)
					golapack.Zlacgv(nrhs, b.CVector(k-1, 0), ldb)
				}
				if nounit {
					goblas.Zscal(*nrhs, a.Get(kc-1), b.CVector(k-1, 0), *ldb)
				...
				if k < (*n)-1 {
					...
					//                 Apply the transformation
					golapack.Zlacgv(nrhs, b.CVector(k+1-1, 0), ldb)
					err = goblas.Zgemv(ConjTrans, (*n)-k-1, *nrhs, one, b.Off(k+2-1, 0), *ldb, a.Off(kcnext+1-1), 1, one, b.CVector(k+1-1, 0), *ldb)
					golapack.Zlacgv(nrhs, b.CVector(k+1-1, 0), ldb)

					golapack.Zlacgv(nrhs, b.CVector(k-1, 0), ldb)
					err = goblas.Zgemv(ConjTrans, (*n)-k-1, *nrhs, one, b.Off(k+2-1, 0), *ldb, a.Off(kc+2-1), 1, one, b.CVector(k-1, 0), *ldb)
					golapack.Zlacgv(nrhs, b.CVector(k-1, 0), ldb)
				}
