...
func dgtt01(n int, dl, d, du, dlf, df, duf, du2 *mat.Vector, ipiv []int, work *mat.Matrix, rwork *mat.Vector) (resid float64) {
	...
	for i = n - 1; i >= 1; i-- {
		li = dlf.Get(i - 1)
		work.Off(i, i-1).Vector().Axpy(lastj-i+1, li, work.Off(i-1, i-1).Vector(), work.Rows, work.Rows)
		ip = ipiv[i-1]
		if ip == i {
			lastj = min(i+2, n)
		} else {
			work.Off(i, i-1).Vector().Swap(lastj-i+1, work.Off(i-1, i-1).Vector(), work.Rows, work.Rows)
		}
	}
	...
	//     upper Hessenberg.
	resid = golapack.Dlanhs('1', n, work, rwork)
