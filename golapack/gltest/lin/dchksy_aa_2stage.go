...
func DchksyAa2stage(dotype *[]bool, nn *int, nval *[]int, nnb *int, nbval *[]int, nns *int, nsval *[]int, thresh *float64, tsterr *bool, nmax *int, a, afac, ainv, b, x, xact, work, rwork *mat.Vector, iwork *[]int, nout *int, t *testing.T) {
	var zerot bool
				...
				*srnamt = "DLATMS"
				...
				for inb = 1; inb <= (*nnb); inb++ {
					...
					*srnamt = "DSYTRF_AA_2STAGE"
					...
					golapack.DsytrfAa2stage(uplo, &n, afac.Matrix(lda, opts), &lda, ainv, toPtr((3*nb+1)*n), iwork, toSlice(iwork, 1+n-1), work, &lwork, &info)
					...
					if info != k {
						Alaerh(path, []byte("DSYTRF_AA_2STAGE"), &info, &k, []byte{uplo}, &n, &n, toPtr(-1), toPtr(-1), &nb, &imat, &nfail, &nerrs)
					}
					...
					for irhs = 1; irhs <= (*nns); irhs++ {
						...
						*srnamt = "DSYTRS_AA_2STAGE"
						...
						golapack.DsytrsAa2stage(uplo, &n, &nrhs, afac.Matrix(lda, opts), &lda, ainv, toPtr((3*nb+1)*n), iwork, toSlice(iwork, 1+n-1), x.Matrix(lda, opts), &lda, &info)

						//                    Check error code from DSYTRS and handle error.
						if info != 0 {
							if izero == 0 {
								Alaerh(path, []byte("DSYTRS_AA_2STAGE"), &info, func() *int { y := 0; return &y }(), []byte{uplo}, &n, &n, toPtr(-1), toPtr(-1), &nrhs, &imat, &nfail, &nerrs)
							}
						} else {
