...
func dlattb(imat int, uplo mat.MatUplo, trans mat.MatTrans, iseed *[]int, n, kd int, ab *mat.Matrix, b, work *mat.Vector) (diag mat.MatDiag, err error) {
	var upper bool
	...
	if imat <= 5 {
		_, _ = matgen.Dlatms(n, n, dist, iseed, _type, b, mode, cndnum, anorm, kl, ku, packit, ab.Off(ioff-1, 0), work)
		...
		if kd == 1 {
			if upper {
				ab.Set(0, 1, math.Copysign(tnorm, matgen.Dlarnd(2, iseed)))
				lenj = (n - 3) / 2
				golapack.Dlarnv(2, iseed, lenj, work)
				for j = 1; j <= lenj; j++ {
					ab.Set(0, 2*(j+1)-1, tnorm*work.Get(j-1))
				}
			} else {
				ab.Set(1, 0, math.Copysign(tnorm, matgen.Dlarnd(2, iseed)))
				lenj = (n - 3) / 2
				golapack.Dlarnv(2, iseed, lenj, work)
				for j = 1; j <= lenj; j++ {
			...
			star1 = math.Copysign(tnorm, matgen.Dlarnd(2, iseed))
			sfac = math.Sqrt(tnorm)
			plus1 = math.Copysign(sfac, matgen.Dlarnd(2, iseed))
			for j = 1; j <= n; j += 2 {
				plus2 = star1 / plus1
				work.Set(j-1, plus1)
			...
			if upper {
				ab.Off(kd-1, 1).Vector().Copy(n-1, work, 1, ab.Rows)
				ab.Off(kd-1-1, 2).Vector().Copy(n-2, work.Off(n), 1, ab.Rows)
			} else {
				ab.Off(1, 0).Vector().Copy(n-1, work, 1, ab.Rows)
				ab.Off(2, 0).Vector().Copy(n-2, work.Off(n), 1, ab.Rows)
			}
		}
		...
		if upper {
			for j = 1; j <= n; j++ {
				lenj = min(j, kd+1)
				golapack.Dlarnv(2, iseed, lenj, ab.Off(kd+2-lenj-1, j-1).Vector())
			...
			for j = 1; j <= n; j++ {
				lenj = min(n-j+1, kd+1)
				if lenj > 0 {
					golapack.Dlarnv(2, iseed, lenj, ab.Off(0, j-1).Vector())
				}
		...
		if upper {
			for j = 1; j <= n; j++ {
				lenj = min(j, kd+1)
				golapack.Dlarnv(2, iseed, lenj, ab.Off(kd+2-lenj-1, j-1).Vector())
				ab.Off(kd+2-lenj-1, j-1).Vector().Scal(lenj-1, tscal, 1)
			...
			for j = 1; j <= n; j++ {
				...
				golapack.Dlarnv(2, iseed, lenj, ab.Off(0, j-1).Vector())
				if lenj > 1 {
					ab.Off(1, j-1).Vector().Scal(lenj-1, tscal, 1)
				}
		...
		if upper {
			for j = 1; j <= n; j++ {
				lenj = min(j-1, kd)
				golapack.Dlarnv(2, iseed, lenj, ab.Off(kd+1-lenj-1, j-1).Vector())
				ab.Set(kd, j-1, float64(j))
			}
		} else {
			for j = 1; j <= n; j++ {
				lenj = min(n-j, kd)
				if lenj > 0 {
					golapack.Dlarnv(2, iseed, lenj, ab.Off(1, j-1).Vector())
				}
				ab.Set(0, j-1, float64(j))
			}
		}
	...
	if trans != NoTrans {
		if upper {
			for j = 1; j <= n/2; j++ {
				...
				ab.Off(kd+2-lenj-1, n-j).Vector().Swap(lenj, ab.Off(kd, j-1).Vector(), ab.Rows-1, -1)
			}
		} else {
			for j = 1; j <= n/2; j++ {
				lenj = min(n-2*j+1, kd+1)
				ab.Off(lenj-1, n-j+2-lenj-1).Vector().Swap(lenj, ab.Off(0, j-1).Vector(), 1, -ab.Rows+1)
			}
		}
	}
