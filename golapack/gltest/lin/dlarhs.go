...
func Dlarhs(path []byte, xtype *byte, uplo, trans byte, m, n, kl, ku, nrhs *int, a *mat.Matrix, lda *int, x *mat.Matrix, ldx *int, b *mat.Matrix, ldb *int, iseed *[]int, info *int) {
	...
	if string(c2) == "GE" || string(c2) == "QR" || string(c2) == "LQ" || string(c2) == "QL" || string(c2) == "RQ" {
		...
		for j = 1; j <= (*nrhs); j++ {
			goblas.Dgbmv(mat.TransByte(trans), &mb, &nx, kl, ku, &one, a, lda, x.Vector(0, j-1), toPtr(1), &zero, b.Vector(0, j-1), toPtr(1))
		}

	} else if string(c2) == "PB" {
		//        Symmetric matrix, band storage
		for j = 1; j <= (*nrhs); j++ {
			goblas.Dsbmv(mat.UploByte(uplo), n, kl, &one, a, lda, x.Vector(0, j-1), toPtr(1), &zero, b.Vector(0, j-1), toPtr(1))
		}

	} else if string(c2) == "PP" || string(c2) == "SP" {
		//        Symmetric matrix, packed storage
		for j = 1; j <= (*nrhs); j++ {
			goblas.Dspmv(mat.UploByte(uplo), n, &one, a.VectorIdx(0), x.Vector(0, j-1), toPtr(1), &zero, b.Vector(0, j-1), toPtr(1))
		}
		...
		for j = 1; j <= (*nrhs); j++ {
			goblas.Dtpmv(mat.UploByte(uplo), mat.TransByte(trans), mat.DiagByte(diag), n, a.VectorIdx(0), b.Vector(0, j-1), toPtr(1))
		}
		//
		...
		for j = 1; j <= (*nrhs); j++ {
			goblas.Dtbmv(mat.UploByte(uplo), mat.TransByte(trans), mat.DiagByte(diag), n, kl, a, lda, b.Vector(0, j-1), toPtr(1))
		}
