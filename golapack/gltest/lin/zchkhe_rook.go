...
// zchkheRook tests ZhetrfRook, -TRI_ROOK, -TRS_ROOK,
// and -CON_ROOK.
func zchkheRook(dotype []bool, nn int, nval []int, nnb int, nbval []int, nns int, nsval []int, thresh float64, tsterr bool, nmax int, a, afac, ainv, b, x, xact, work *mat.CVector, rwork *mat.Vector, iwork []int, t *testing.T) {
			...
			for _, uplo = range mat.IterMatUplo(false) {
				...
				*srnamt = "Zlatms"
				...
				for inb = 1; inb <= nnb; inb++ {
					...
					lwork = max(2, nb) * lda
					*srnamt = "ZhetrfRook"
					info, err = golapack.ZhetrfRook(uplo, n, afac.CMatrix(lda, opts), &iwork, ainv, lwork)
					...
					//                 Check error code from ZhetrfRook and handle error.
					if err != nil || info != k {
						t.Fail()
						nerrs = alaerh(path, "ZhetrfRook", info, k, []byte{uplo.Byte()}, n, n, -1, -1, nb, imat, nfail, nerrs)
					}
					...
					*result.GetPtr(0) = zhet01Rook(uplo, n, a.CMatrix(lda, opts), afac.CMatrix(lda, opts), &iwork, ainv.CMatrix(lda, opts), rwork)
					...
					if inb == 1 && !trfcon {
						...
						*srnamt = "ZhetriRook"
						if info, err = golapack.ZhetriRook(uplo, n, ainv.CMatrix(lda, opts), &iwork, work); err != nil || info != 0 {
							t.Fail()
							nerrs = alaerh(path, "ZhetriRook", info, -1, []byte{uplo.Byte()}, n, n, -1, -1, -1, imat, nfail, nerrs)
						}
					...
					label160:
						...
						if iwork[k-1] < int(zero) {
							...
							block.Set(1, 0, block.GetConj(0, 1))
					...
					for irhs = 1; irhs <= nns; irhs++ {
						...
						*srnamt = "ZhetrsRook"
						if err = golapack.ZhetrsRook(uplo, n, nrhs, afac.CMatrix(lda, opts), &iwork, x.CMatrix(lda, opts)); err != nil {
							panic(err)
						}

						//                    Check error code from ZhetrsRook and handle error.
						if info != 0 {
							t.Fail()
							nerrs = alaerh(path, "ZhetrsRook", info, 0, []byte{uplo.Byte()}, n, n, -1, -1, nrhs, imat, nfail, nerrs)
						}

						golapack.Zlacpy(Full, n, nrhs, b.CMatrix(lda, opts), work.CMatrix(lda, opts))

						//                    Compute the residual for the solution
						*result.GetPtr(4) = zpot02(uplo, n, nrhs, a.CMatrix(lda, opts), x.CMatrix(lda, opts), work.CMatrix(lda, opts), rwork)
				...
				label230:
					...
					*srnamt = "ZheconRook"
					if rcond, err = golapack.ZheconRook(uplo, n, afac.CMatrix(lda, opts), &iwork, anorm, work); err != nil {
						t.Fail()
						nerrs = alaerh(path, "ZheconRook", info, 0, []byte{uplo.Byte()}, n, n, -1, -1, -1, imat, nfail, nerrs)
					}
