...
// Zchkherook tests ZHETRF_ROOK, -TRI_ROOK, -TRS_ROOK,
// and -CON_ROOK.
func Zchkherook(dotype *[]bool, nn *int, nval *[]int, nnb *int, nbval *[]int, nns *int, nsval *[]int, thresh *float64, tsterr *bool, nmax *int, a, afac, ainv, b, x, xact, work *mat.CVector, rwork *mat.Vector, iwork *[]int, nout *int, t *testing.T) {
				...
				*srnamt = "ZLATMS"
				...
				for inb = 1; inb <= (*nnb); inb++ {
					...
					*srnamt = "ZHETRF_ROOK"
					...
					if info != k {
						t.Fail()
						Alaerh(path, []byte("ZHETRF_ROOK"), &info, &k, []byte{uplo}, &n, &n, toPtr(-1), toPtr(-1), &nb, &imat, &nfail, &nerrs)
					}
					...
					if inb == 1 && !trfcon {
						...
						*srnamt = "ZHETRI_ROOK"
						...
						if info != 0 {
							t.Fail()
							Alaerh(path, []byte("ZHETRI_ROOK"), &info, toPtr(-1), []byte{uplo}, &n, &n, toPtr(-1), toPtr(-1), toPtr(-1), &imat, &nfail, &nerrs)
						}
					...
					for irhs = 1; irhs <= (*nns); irhs++ {
						...
						*srnamt = "ZHETRS_ROOK"
						golapack.Zhetrsrook(uplo, &n, &nrhs, afac.CMatrix(lda, opts), &lda, iwork, x.CMatrix(lda, opts), &lda, &info)

						//                    Check error code from ZHETRS_ROOK and handle error.
						if info != 0 {
							t.Fail()
							Alaerh(path, []byte("ZHETRS_ROOK"), &info, func() *int { y := 0; return &y }(), []byte{uplo}, &n, &n, toPtr(-1), toPtr(-1), &nrhs, &imat, &nfail, &nerrs)
						}
				...
				label230:
					...
					*srnamt = "ZHECON_ROOK"
					golapack.Zheconrook(uplo, &n, afac.CMatrix(lda, opts), &lda, iwork, &anorm, &rcond, work, &info)

					//                 Check error code from ZHECON_ROOK and handle error.
					if info != 0 {
						t.Fail()
						Alaerh(path, []byte("ZHECON_ROOK"), &info, func() *int { y := 0; return &y }(), []byte{uplo}, &n, &n, toPtr(-1), toPtr(-1), toPtr(-1), &imat, &nfail, &nerrs)
					}
