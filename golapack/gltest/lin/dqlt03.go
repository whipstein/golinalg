...
func Dqlt03(m, n, k *int, af, c, cc, q *mat.Matrix, lda *int, tau, work *mat.Vector, lwork *int, rwork, result *mat.Vector) {
	...
	if (*k) > 0 && (*m) > (*k) {
		golapack.Dlacpy('F', toPtr((*m)-(*k)), k, af.Off(0, (*n)-(*k)+1-1), lda, q.Off(0, (*m)-(*k)+1-1), lda)
	}
	if (*k) > 1 {
		golapack.Dlacpy('U', toPtr((*k)-1), toPtr((*k)-1), af.Off((*m)-(*k)+1-1, (*n)-(*k)+2-1), lda, q.Off((*m)-(*k)+1-1, (*m)-(*k)+2-1), lda)
	}

	//     Generate the m-by-m matrix Q
	*srnamt = "DORGQL"
	golapack.Dorgql(m, m, k, q, lda, tau.Off(minmn-(*k)+1-1), work, lwork, &info)
	...
	for iside = 1; iside <= 2; iside++ {
		...
		for itrans = 1; itrans <= 2; itrans++ {
			...
			*srnamt = "DORMQL"
			if (*k) > 0 {
				golapack.Dormql(side, trans, &mc, &nc, k, af.Off(0, (*n)-(*k)+1-1), lda, tau.Off(minmn-(*k)+1-1), cc, lda, work, lwork, &info)
			}
