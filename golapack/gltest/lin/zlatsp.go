...
func zlatsp(uplo mat.MatUplo, n int, x *mat.CVector, iseed *[]int) {
	...
	if uplo == Upper {
		...
		for j = n; j >= n5; j -= 5 {
			...
			x.Set(jj-2-1, b)
			jj = jj - j
			x.Set(jj-1, matgen.Zlarnd(2, *iseed))
			x.Set(jj-1-1, r)
			jj = jj - (j - 1)
			x.Set(jj-1, c)
			jj = jj - (j - 2)
		...
		if j > 1 {
			x.Set(jj-1, matgen.Zlarnd(2, *iseed))
			x.Set(jj-j-1, matgen.Zlarnd(2, *iseed))
		...
		if j < n {
			x.Set(jj-1, matgen.Zlarnd(2, *iseed))
			x.Set(jj+(n-j+1)-1, matgen.Zlarnd(2, *iseed))
