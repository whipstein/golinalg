...
func zerrqr(path string, t *testing.T) {
	...
	*srnamt = "zgeqrs"
	*errt = fmt.Errorf("m < 0: m=-1")
	err = zgeqrs(-1, 0, 0, a.Off(0, 0).UpdateRows(1), x, b.CMatrix(1, opts), w, 1)
	chkxer2("zgeqrs", err)
	*errt = fmt.Errorf("n < 0 || n > m: n=-1, m=0")
	err = zgeqrs(0, -1, 0, a.Off(0, 0).UpdateRows(1), x, b.CMatrix(1, opts), w, 1)
	chkxer2("zgeqrs", err)
	*errt = fmt.Errorf("n < 0 || n > m: n=2, m=1")
	err = zgeqrs(1, 2, 0, a.Off(0, 0).UpdateRows(2), x, b.CMatrix(2, opts), w, 1)
	chkxer2("zgeqrs", err)
	*errt = fmt.Errorf("nrhs < 0: nrhs=-1")
	err = zgeqrs(0, 0, -1, a.Off(0, 0).UpdateRows(1), x, b.CMatrix(1, opts), w, 1)
	chkxer2("zgeqrs", err)
	*errt = fmt.Errorf("a.Rows < max(1, m): a.Rows=1, m=2")
	err = zgeqrs(2, 1, 0, a.Off(0, 0).UpdateRows(1), x, b.CMatrix(2, opts), w, 1)
	chkxer2("zgeqrs", err)
	*errt = fmt.Errorf("b.Rows < max(1, m): b.Rows=1, m=2")
	err = zgeqrs(2, 1, 0, a.Off(0, 0).UpdateRows(2), x, b.CMatrix(1, opts), w, 1)
	chkxer2("zgeqrs", err)
	*errt = fmt.Errorf("lwork < 1 || lwork < nrhs && m > 0 && n > 0: lwork=1, nrhs=2, m=1, n=1")
	err = zgeqrs(1, 1, 2, a.Off(0, 0).UpdateRows(1), x, b.CMatrix(1, opts), w, 1)
	chkxer2("zgeqrs", err)

	//     Zungqr
	*srnamt = "Zungqr"
	*errt = fmt.Errorf("m < 0: m=-1")
	err = golapack.Zungqr(-1, 0, 0, a.Off(0, 0).UpdateRows(1), x, w, 1)
	chkxer2("Zungqr", err)
	*errt = fmt.Errorf("n < 0 || n > m: m=0, n=-1")
	err = golapack.Zungqr(0, -1, 0, a.Off(0, 0).UpdateRows(1), x, w, 1)
	chkxer2("Zungqr", err)
	*errt = fmt.Errorf("n < 0 || n > m: m=1, n=2")
	err = golapack.Zungqr(1, 2, 0, a.Off(0, 0).UpdateRows(1), x, w, 2)
	chkxer2("Zungqr", err)
	*errt = fmt.Errorf("k < 0 || k > n: n=0, k=-1")
	err = golapack.Zungqr(0, 0, -1, a.Off(0, 0).UpdateRows(1), x, w, 1)
	chkxer2("Zungqr", err)
	*errt = fmt.Errorf("k < 0 || k > n: n=1, k=2")
	err = golapack.Zungqr(1, 1, 2, a.Off(0, 0).UpdateRows(1), x, w, 1)
	chkxer2("Zungqr", err)
	*errt = fmt.Errorf("a.Rows < max(1, m): a.Rows=1, m=2")
	err = golapack.Zungqr(2, 2, 0, a.Off(0, 0).UpdateRows(1), x, w, 2)
	chkxer2("Zungqr", err)
	*errt = fmt.Errorf("lwork < max(1, n) && !lquery: lwork=1, n=2, lquery=false")
	err = golapack.Zungqr(2, 2, 0, a.Off(0, 0).UpdateRows(2), x, w, 1)
	chkxer2("Zungqr", err)
	...
	*errt = fmt.Errorf("!left && side != Right: side=Unrecognized: /")
	err = golapack.Zunmqr('/', NoTrans, 0, 0, 0, a.Off(0, 0).UpdateRows(1), x, af.Off(0, 0).UpdateRows(1), w, 1)
	chkxer2("Zunmqr", err)
	*errt = fmt.Errorf("!notran && trans != ConjTrans: trans=Unrecognized: /")
	err = golapack.Zunmqr(Left, '/', 0, 0, 0, a.Off(0, 0).UpdateRows(1), x, af.Off(0, 0).UpdateRows(1), w, 1)
	chkxer2("Zunmqr", err)
	*errt = fmt.Errorf("m < 0: m=-1")
	err = golapack.Zunmqr(Left, NoTrans, -1, 0, 0, a.Off(0, 0).UpdateRows(1), x, af.Off(0, 0).UpdateRows(1), w, 1)
	chkxer2("Zunmqr", err)
	*errt = fmt.Errorf("n < 0: n=-1")
	err = golapack.Zunmqr(Left, NoTrans, 0, -1, 0, a.Off(0, 0).UpdateRows(1), x, af.Off(0, 0).UpdateRows(1), w, 1)
	chkxer2("Zunmqr", err)
	*errt = fmt.Errorf("k < 0 || k > nq: k=-1, nq=0")
	err = golapack.Zunmqr(Left, NoTrans, 0, 0, -1, a.Off(0, 0).UpdateRows(1), x, af.Off(0, 0).UpdateRows(1), w, 1)
	chkxer2("Zunmqr", err)
	*errt = fmt.Errorf("k < 0 || k > nq: k=1, nq=0")
	err = golapack.Zunmqr(Left, NoTrans, 0, 1, 1, a.Off(0, 0).UpdateRows(1), x, af.Off(0, 0).UpdateRows(1), w, 1)
	chkxer2("Zunmqr", err)
	*errt = fmt.Errorf("k < 0 || k > nq: k=1, nq=0")
	err = golapack.Zunmqr(Right, NoTrans, 1, 0, 1, a.Off(0, 0).UpdateRows(1), x, af.Off(0, 0).UpdateRows(1), w, 1)
	chkxer2("Zunmqr", err)
	*errt = fmt.Errorf("a.Rows < max(1, nq): a.Rows=1, nq=2")
	err = golapack.Zunmqr(Left, NoTrans, 2, 1, 0, a.Off(0, 0).UpdateRows(1), x, af.Off(0, 0).UpdateRows(2), w, 1)
	chkxer2("Zunmqr", err)
	*errt = fmt.Errorf("a.Rows < max(1, nq): a.Rows=1, nq=2")
	err = golapack.Zunmqr(Right, NoTrans, 1, 2, 0, a.Off(0, 0).UpdateRows(1), x, af.Off(0, 0).UpdateRows(1), w, 1)
	chkxer2("Zunmqr", err)
	*errt = fmt.Errorf("c.Rows < max(1, m): c.Rows=1, m=2")
	err = golapack.Zunmqr(Left, NoTrans, 2, 1, 0, a.Off(0, 0).UpdateRows(2), x, af.Off(0, 0).UpdateRows(1), w, 1)
	chkxer2("Zunmqr", err)
	*errt = fmt.Errorf("lwork < max(1, nw) && !lquery: lwork=1, nw=2, lquery=false")
	err = golapack.Zunmqr(Left, NoTrans, 1, 2, 0, a.Off(0, 0).UpdateRows(1), x, af.Off(0, 0).UpdateRows(1), w, 1)
	chkxer2("Zunmqr", err)
	*errt = fmt.Errorf("lwork < max(1, nw) && !lquery: lwork=1, nw=2, lquery=false")
	err = golapack.Zunmqr(Right, NoTrans, 2, 1, 0, a.Off(0, 0).UpdateRows(1), x, af.Off(0, 0).UpdateRows(2), w, 1)
	chkxer2("Zunmqr", err)
