...
func zchklq(dotype []bool, nm int, mval []int, nn int, nval []int, nnb int, nbval []int, nxval []int, nrhs int, thresh float64, tsterr bool, nmax int, a, af, aq, al, ac, b, x, xact, tau, work *mat.CVector, rwork *mat.Vector, t *testing.T) {
	var dist, _type byte
				...
				*srnamt = "Zlatms"
				...
				for ik = 1; ik <= nk; ik++ {
					...
					for inb = 1; inb <= nnb; inb++ {
						...
						if ik == 1 {
							//                       Test ZGELQF
							zlqt01(m, n, a.CMatrix(lda, opts), af.CMatrix(lda, opts), aq.CMatrix(lda, opts), al.CMatrix(lda, opts), tau, work, lwork, rwork, result)
						} else if m <= n {
							//                       Test ZUNGLQ, using factorization
							//                       returned by ZLQT01
							zlqt02(m, n, k, a.CMatrix(lda, opts), af.CMatrix(lda, opts), aq.CMatrix(lda, opts), al.CMatrix(lda, opts), tau, work, lwork, rwork, result)
						}
						if m >= k {
							//                       Test ZUNMLQ, using factorization returned
							//                       by ZLQT01
							zlqt03(m, n, k, af.CMatrix(lda, opts), ac.CMatrix(lda, opts), al.CMatrix(lda, opts), aq.CMatrix(lda, opts), tau, work, lwork, rwork, result.Off(2))
							nt = nt + 4
							...
							if k == m && inb == 1 {
								...
								*srnamt = "zgelqs"
								if err = zgelqs(m, n, nrhs, af.CMatrix(lda, opts), tau, x.CMatrix(lda, opts), work, lwork); err != nil {
									t.Fail()
									nerrs = alaerh(path, "zgelqs", info, 0, []byte{' '}, m, n, nrhs, -1, nb, imat, nfail, nerrs)
								}
