...
// zdrvsyRook tests the driver routines ZsysvRook.
func zdrvsyRook(dotype []bool, nn int, nval []int, nrhs int, thresh float64, tsterr bool, nmax int, a, afac, ainv, b, x, xact, work *mat.CVector, rwork *mat.Vector, iwork []int, t *testing.T) {
	var zerot bool
			...
			for _, uplo = range mat.IterMatUplo(false) {
				...
				for ifact = 1; ifact <= nfact; ifact++ {
					...
					if zerot {
						...
						if info, err = golapack.ZsytrfRook(uplo, n, afac.CMatrix(lda, opts), &iwork, work, lwork); err != nil {
							panic(err)
						}
						...
						lwork = (n + nb + 1) * (nb + 3)
						if info, err = golapack.ZsytriRook(uplo, n, ainv.CMatrix(lda, opts), &iwork, work); err != nil {
							panic(err)
						}
					...
					*srnamt = "zlarhs"
					...
					if ifact == 2 {
						...
						//                    ZsysvRook.
						*srnamt = "ZsysvRook"
						info, err = golapack.ZsysvRook(uplo, n, nrhs, afac.CMatrix(lda, opts), &iwork, x.CMatrix(lda, opts), work, lwork)
						...
						//                    Check error code from ZsysvRook and handle error.
						if err != nil || info != k {
							t.Fail()
							nerrs = alaerh(path, "ZsysvRook", info, k, []byte{uplo.Byte()}, n, n, -1, -1, nrhs, imat, nfail, nerrs)
							goto label120
						} else if info != 0 {
							goto label120
						}
