...
// Zdrvsyrook tests the driver routines ZSYSV_ROOK.
func Zdrvsyrook(dotype *[]bool, nn *int, nval *[]int, nrhs *int, thresh *float64, tsterr *bool, nmax *int, a, afac, ainv, b, x, xact, work *mat.CVector, rwork *mat.Vector, iwork *[]int, nout *int, t *testing.T) {
	var zerot bool
			...
			for iuplo = 1; iuplo <= 2; iuplo++ {
				...
				for ifact = 1; ifact <= nfact; ifact++ {
					...
					if zerot {
						...
						golapack.Zsytrfrook(uplo, &n, afac.CMatrix(lda, opts), &lda, iwork, work, &lwork, &info)
					...
					*srnamt = "ZLARHS"
					...
					if ifact == 2 {
						...
						//                    ZSYSV_ROOK.
						*srnamt = "ZSYSV_ROOK"
						golapack.Zsysvrook(uplo, &n, nrhs, afac.CMatrix(lda, opts), &lda, iwork, x.CMatrix(lda, opts), &lda, work, &lwork, &info)
						...
						if info != k {
							t.Fail()
							Alaerh(path, []byte("ZSYSV_ROOK"), &info, &k, []byte{uplo}, &n, &n, toPtr(-1), toPtr(-1), nrhs, &imat, &nfail, &nerrs)
							goto label120
						} else if info != 0 {
							goto label120
						}
