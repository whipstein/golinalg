...
func Dptt05(n, nrhs *int, d, e *mat.Vector, b *mat.Matrix, ldb *int, x *mat.Matrix, ldx *int, xact *mat.Matrix, ldxact *int, ferr, berr, reslts *mat.Vector) {
	...
	for k = 1; k <= (*nrhs); k++ {
		if (*n) == 1 {
			...
			for i = 2; i <= (*n)-1; i++ {
				tmp = math.Abs(b.Get(i-1, k-1)) + math.Abs(e.Get(i-1-1)*x.Get(i-1-1, k-1)) + math.Abs(d.Get(i-1)*x.Get(i-1, k-1)) + math.Abs(e.Get(i-1)*x.Get(i+1-1, k-1))
