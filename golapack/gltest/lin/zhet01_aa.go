...
func Zhet01aa(uplo byte, n *int, a *mat.CMatrix, lda *int, afac *mat.CMatrix, ldafac *int, ipiv *[]int, c *mat.CMatrix, ldc *int, rwork *mat.Vector, resid *float64) {
	...
	if (*n) > 1 {
		...
		if uplo == 'U' {
			err = goblas.Ztrmm(Left, mat.UploByte(uplo), ConjTrans, Unit, (*n)-1, *n, cone, afac.Off(0, 1), c.Off(1, 0))
		} else {
		...
		if uplo == 'U' {
			...
			err = goblas.Ztrmm(Right, mat.UploByte(uplo), ConjTrans, Unit, *n, (*n)-1, cone, afac.Off(1, 0), c.Off(0, 1))
		}
