...
func Zlaghe(n, k int, d *mat.Vector, a *mat.CMatrix, iseed *[]int, work *mat.CVector) (err error) {
	...
	for i = n - 1; i >= 1; i-- {
		...
		//
		//        compute  y := tau * A * u
		if err = goblas.Zhemv(Lower, n-i+1, tau, a.Off(i-1, i-1), work.Off(0, 1), zero, work.Off(n, 1)); err != nil {
			panic(err)
		}
		...
		if err = goblas.Zher2(Lower, n-i+1, -one, work.Off(0, 1), work.Off(n, 1), a.Off(i-1, i-1)); err != nil {
			panic(err)
		}
	}
	...
	for i = 1; i <= n-1-k; i++ {
		...
		//
		//        compute  y := tau * A * u
		if err = goblas.Zhemv(Lower, n-k-i+1, tau, a.Off(k+i-1, k+i-1), a.CVector(k+i-1, i-1, 1), zero, work.Off(0, 1)); err != nil {
			panic(err)
		}
		...
		if err = goblas.Zher2(Lower, n-k-i+1, -one, a.CVector(k+i-1, i-1, 1), work.Off(0, 1), a.Off(k+i-1, k+i-1)); err != nil {
			panic(err)
		}
