...
func Dlatmt(m, n *int, dist byte, iseed *[]int, sym byte, d *mat.Vector, mode *int, cond, dmax *float64, rank, kl, ku *int, pack byte, a *mat.Matrix, lda *int, work *mat.Vector, info *int) {
	...
	if (*mode) != 0 && absint(*mode) != 6 {
		...
		goblas.Dscal(rank, &alpha, d, toPtr(1))
	}
	...
	if llb == 0 && uub == 0 {
		goblas.Dcopy(&mnmin, d, toPtr(1), a.Vector(1-iskew+ioffst-1, 0), toPtr(ilda+1))
		if ipack <= 2 || ipack >= 5 {
			ipackg = ipack
		}
		...
		if isym == 1 {
			...
			if topdwn {
				...
				goblas.Dcopy(&mnmin, d, toPtr(1), a.Vector(1-iskew+ioffg-1, 0), toPtr(ilda+1))
