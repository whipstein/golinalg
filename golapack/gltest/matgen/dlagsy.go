...
func Dlagsy(n *int, k *int, d *mat.Vector, a *mat.Matrix, lda *int, iseed *[]int, work *mat.Vector, info *int) {
	...
	for i = (*n) - 1; i >= 1; i-- {
		...
		//
		//        compute  y := tau * A * u
		err = goblas.Dsymv(mat.Lower, (*n)-i+1, tau, a.Off(i-1, i-1), work.Off(0, 1), zero, work.Off((*n), 1))
		...
		err = goblas.Dsyr2(mat.Lower, (*n)-i+1, -one, work.Off(0, 1), work.Off((*n), 1), a.Off(i-1, i-1))
	}
	...
	for i = 1; i <= (*n)-1-(*k); i++ {
		...
		err = goblas.Dgemv(mat.Trans, (*n)-(*k)-i+1, (*k)-1, one, a.Off((*k)+i-1, i), a.Vector((*k)+i-1, i-1, 1), zero, work.Off(0, 1))
		err = goblas.Dger((*n)-(*k)-i+1, (*k)-1, -tau, a.Vector((*k)+i-1, i-1, 1), work.Off(0, 1), a.Off((*k)+i-1, i))

		//        apply reflection to A(k+i:n,k+i:n) from the left and the right
		//
		//        compute  y := tau * A * u
		err = goblas.Dsymv(mat.Lower, (*n)-(*k)-i+1, tau, a.Off((*k)+i-1, (*k)+i-1), a.Vector((*k)+i-1, i-1, 1), zero, work.Off(0, 1))
		...
		err = goblas.Dsyr2(mat.Lower, (*n)-(*k)-i+1, -one, a.Vector((*k)+i-1, i-1, 1), work.Off(0, 1), a.Off((*k)+i-1, (*k)+i-1))
