...
func Zlagge(m, n, kl, ku *int, d *mat.Vector, a *mat.CMatrix, lda *int, iseed *[]int, work *mat.CVector, info *int) {
	...
	for i = min(*m, *n); i >= 1; i-- {
		if i < (*m) {
			...
			err = goblas.Zgemv(ConjTrans, (*m)-i+1, (*n)-i+1, one, a.Off(i-1, i-1), work.Off(0, 1), zero, work.Off((*m), 1))
			err = goblas.Zgerc((*m)-i+1, (*n)-i+1, -tau, work.Off(0, 1), work.Off((*m), 1), a.Off(i-1, i-1))
		}
		if i < (*n) {
			//           generate random reflection
			...
			err = goblas.Zgemv(NoTrans, (*m)-i+1, (*n)-i+1, one, a.Off(i-1, i-1), work.Off(0, 1), zero, work.Off((*n), 1))
			err = goblas.Zgerc((*m)-i+1, (*n)-i+1, -tau, work.Off((*n), 1), work.Off(0, 1), a.Off(i-1, i-1))
		}
	}
	...
	for i = 1; i <= max((*m)-1-(*kl), (*n)-1-(*ku)); i++ {
		if (*kl) <= (*ku) {
			...
			if i <= min((*m)-1-(*kl), *n) {
				...
				err = goblas.Zgemv(ConjTrans, (*m)-(*kl)-i+1, (*n)-i, one, a.Off((*kl)+i-1, i), a.CVector((*kl)+i-1, i-1, 1), zero, work.Off(0, 1))
				err = goblas.Zgerc((*m)-(*kl)-i+1, (*n)-i, -tau, a.CVector((*kl)+i-1, i-1, 1), work.Off(0, 1), a.Off((*kl)+i-1, i))
				a.Set((*kl)+i-1, i-1, -wa)
			}
			...
			if i <= min((*n)-1-(*ku), *m) {
				...
				if complex(wn, 0) == zero {
					...
					goblas.Zscal((*n)-(*ku)-i, one/wb, a.CVector(i-1, (*ku)+i, *lda))
				...
				err = goblas.Zgemv(NoTrans, (*m)-i, (*n)-(*ku)-i+1, one, a.Off(i, (*ku)+i-1), a.CVector(i-1, (*ku)+i-1, *lda), zero, work.Off(0, 1))
				err = goblas.Zgerc((*m)-i, (*n)-(*ku)-i+1, -tau, work.Off(0, 1), a.CVector(i-1, (*ku)+i-1, *lda), a.Off(i, (*ku)+i-1))
				a.Set(i-1, (*ku)+i-1, -wa)
			}
		} else {
