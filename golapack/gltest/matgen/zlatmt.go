...
//         will be necessary.
func Zlatmt(m, n int, dist byte, iseed *[]int, sym byte, d *mat.Vector, mode int, cond, dmax float64, rank, kl, ku int, pack byte, a *mat.CMatrix, work *mat.CVector) (err error) {
	var csym, givens, ilextr, iltemp, topdwn bool
	var c, ct, czero, dummy, extra, s, st, ztemp complex128
	...
	if m < 0 {
		...
		err = fmt.Errorf("ipack == -1 || (isympk == 1 && isym == 1) || (isympk == 2 && isym == 1 && kl > 0) || (isympk == 3 && isym == 1 && ku > 0) || (isympk != 0 && m != n): pack='%c', sym='%c', kl=%v, ku='%c', m=%v, n=%v", pack, sym, kl, ku, m, n)
	} else if a.Rows < max(1, minlda) {
		err = fmt.Errorf("a.Rows < max(1, minlda): a.Rows=%v, minlda=%v", a.Rows, minlda)
	}

	if err != nil {
		gltest.Xerbla2("Zlatmt", err)
		return
	}
					...
					for jr = 1; jr <= min(m+jku, n)+jkl-1; jr++ {
						...
						for jch = jr - jkl; jch >= 1; jch -= jkl - jku {
							if ir < m {
								...
								dummy = complex(Dlarnd(5, iseed), 0)
							...
							ztemp = czero
							iltemp = jch > jku
							ztemp, extra = Zlarot(false, iltemp, true, il, c, s, a.Off(irow-iskew*ic+ioffst-1, ic-1).CVector(), ilda, ztemp, extra)
							if iltemp {
								realc, s, dummy = golapack.Zlartg(a.Get(irow+1-iskew*(ic+1)+ioffst-1, ic), ztemp)
								...
								il = ic + 2 - icol
								extra = czero
								extra, ztemp = Zlarot(true, jch > jku+jkl, true, il, c, s, a.Off(irow-iskew*icol+ioffst-1, icol-1).CVector(), ilda, extra, ztemp)
								ic = icol
								ir = irow
							}
						}
				...
				for jkl = 1; jkl <= llb; jkl++ {
					...
					for jc = 1; jc <= min(n+jkl, m)+jku-1; jc++ {
						...
						for jch = jc - jku; jch >= 1; jch -= jkl - jku {
							...
							ztemp = czero
							iltemp = jch > jkl
							ztemp, extra = Zlarot(true, iltemp, true, il, c, s, a.Off(ir-iskew*icol+ioffst-1, icol-1).CVector(), ilda, ztemp, extra)
							if iltemp {
								realc, s, dummy = golapack.Zlartg(a.Get(ir+1-iskew*(icol+1)+ioffst-1, icol), ztemp)
								dummy = Zlarnd(5, *iseed)
								c = cmplx.Conj(complex(realc, 0) * dummy)
								s = cmplx.Conj(-s * dummy)
								...
								il = ir + 2 - irow
								extra = czero
								extra, ztemp = Zlarot(false, jch > jkl+jku, true, il, c, s, a.Off(irow-iskew*icol+ioffst-1, icol-1).CVector(), ilda, extra, ztemp)
								ic = icol
								ir = irow
							}
						}
				...
				for jku = 1; jku <= uub; jku++ {
					...
					for jc = min(m+jku, n) - 1; jc >= 1-jkl; jc-- {
						...
						for jch = jc + jkl; jch <= iendch; jch += jkl + jku {
							...
							iltemp = jch+jku < n
							ztemp = czero
							Zlarot(true, ilextr, iltemp, icol+2-ic, c, s, a.Off(jch-iskew*ic+ioffst-1, ic-1).CVector(), ilda, extra, ztemp)
							if iltemp {
								realc, s, dummy = golapack.Zlartg(a.Get(jch-iskew*icol+ioffst-1, icol-1), ztemp)
								dummy = Zlarnd(5, *iseed)
								c = complex(realc, 0) * dummy
								s = s * dummy
								il = min(iendch, jch+jkl+jku) + 2 - jch
								extra = czero
								ztemp, extra = Zlarot(false, true, jch+jkl+jku <= iendch, il, c, s, a.Off(jch-iskew*icol+ioffst-1, icol-1).CVector(), ilda, ztemp, extra)
								ic = icol
							}
						}
					}
				...
				for jkl = 1; jkl <= llb; jkl++ {
					...
					for jr = min(n+jkl, m) - 1; jr >= 1-jku; jr-- {
						...
						for jch = jr + jku; jch <= iendch; jch += jkl + jku {
							...
							iltemp = jch+jkl < m
							ztemp = czero
							extra, ztemp = Zlarot(false, ilextr, iltemp, irow+2-ir, c, s, a.Off(ir-iskew*jch+ioffst-1, jch-1).CVector(), ilda, extra, ztemp)
							if iltemp {
								realc, s, dummy = golapack.Zlartg(a.Get(irow-iskew*jch+ioffst-1, jch-1), ztemp)
								dummy = Zlarnd(5, *iseed)
								c = complex(realc, 0) * dummy
								s = s * dummy
								il = min(iendch, jch+jkl+jku) + 2 - jch
								extra = czero
								ztemp, extra = Zlarot(true, true, jch+jkl+jku <= iendch, il, c, s, a.Off(irow-iskew*jch+ioffst-1, jch-1).CVector(), ilda, ztemp, extra)
								ir = irow
							}
						}
					}
			...
			if topdwn {
				...
				for k = 1; k <= uub; k++ {
					for jc = 1; jc <= n-1; jc++ {
						...
						il = min(jc+1, k+2)
						extra = czero
						ztemp = a.Get(jc-iskew*(jc+1)+ioffg-1, jc)
						angle = twopi * Dlarnd(1, iseed)
						c = complex(math.Cos(angle), 0) * Zlarnd(5, *iseed)
						s = complex(math.Sin(angle), 0) * Zlarnd(5, *iseed)
						if csym {
							...
							ztemp = cmplx.Conj(ztemp)
							ct = cmplx.Conj(c)
							st = cmplx.Conj(s)
						}
						extra, ztemp = Zlarot(false, jc > k, true, il, c, s, a.Off(irow-iskew*jc+ioffg-1, jc-1).CVector(), ilda, extra, ztemp)
						ztemp, dummy = Zlarot(true, true, false, min(k, n-jc)+1, ct, st, a.Off((1-iskew)*jc+ioffg-1, jc-1).CVector(), ilda, ztemp, dummy)
						...
						for jch = jc - k; jch >= 1; jch -= k {
							...
							c = cmplx.Conj(complex(realc, 0) * dummy)
							s = cmplx.Conj(-s * dummy)
							ztemp = a.Get(jch-iskew*(jch+1)+ioffg-1, jch)
							if csym {
								ct = c
								st = s
							} else {
							...
							ztemp, extra = Zlarot(true, true, true, k+2, c, s, a.Off((1-iskew)*jch+ioffg-1, jch-1).CVector(), ilda, ztemp, extra)
							irow = max(1, jch-k)
							il = min(jch+1, k+2)
							extra = czero
							extra, ztemp = Zlarot(false, jch > k, true, il, ct, st, a.Off(irow-iskew*jch+ioffg-1, jch-1).CVector(), ilda, extra, ztemp)
							icol = jch
						}
					}
				}
				...
				for k = 1; k <= uub; k++ {
					for jc = n - 1; jc >= 1; jc-- {
						il = min(n+1-jc, k+2)
						extra = czero
						ztemp = a.Get(1+(1-iskew)*jc+ioffg-1, jc-1)
						angle = twopi * Dlarnd(1, iseed)
						c = complex(math.Cos(angle), 0) * Zlarnd(5, *iseed)
						s = complex(math.Sin(angle), 0) * Zlarnd(5, *iseed)
						if csym {
						...
						ztemp, extra = Zlarot(false, true, n-jc > k, il, c, s, a.Off((1-iskew)*jc+ioffg-1, jc-1).CVector(), ilda, ztemp, extra)
						icol = max(1, jc-k+1)
						dummy, ztemp = Zlarot(true, false, true, jc+2-icol, ct, st, a.Off(jc-iskew*icol+ioffg-1, icol-1).CVector(), ilda, dummy, ztemp)
						...
						for jch = jc + k; jch <= n-1; jch += k {
							...
							c = complex(realc, 0) * dummy
							s = s * dummy
							ztemp = a.Get(1+(1-iskew)*jch+ioffg-1, jch-1)
							if csym {
								ct = c
								st = s
							} else {
							...
							extra, ztemp = Zlarot(true, true, true, k+2, c, s, a.Off(jch-iskew*icol+ioffg-1, icol-1).CVector(), ilda, extra, ztemp)
							il = min(n+1-jch, k+2)
							extra = czero
							ztemp, extra = Zlarot(false, true, n-jch > k, il, ct, st, a.Off((1-iskew)*jch+ioffg-1, jch-1).CVector(), ilda, ztemp, extra)
							icol = jch
						}
					}
				}
