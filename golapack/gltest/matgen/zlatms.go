				...
				for jku = 1; jku <= uub; jku++ {
					...
					for jr = 1; jr <= min((*m)+jku, *n)+jkl-1; jr++ {
						...
						for jch = jr - jkl; jch >= 1; jch -= (jkl + jku) {
							...
							if iltemp {
								golapack.Zlartg(a.GetPtr(irow+1-iskew*(ic+1)+ioffst-1, ic), &ctemp, &realc, &s, &dummy)
								dummy = Zlarnd(func() *int { y := 5; return &y }(), iseed)
								c = cmplx.Conj(complex(realc, 0) * dummy)
								s = cmplx.Conj(-s * dummy)
				...
				for jkl = 1; jkl <= llb; jkl++ {
					...
					for jc = 1; jc <= min((*n)+jkl, *m)+jku-1; jc++ {
						...
						for jch = jc - jku; jch >= 1; jch -= (jkl + jku) {
							...
							if iltemp {
								golapack.Zlartg(a.GetPtr(ir+1-iskew*(icol+1)+ioffst-1, icol), &ctemp, &realc, &s, &dummy)
								dummy = Zlarnd(func() *int { y := 5; return &y }(), iseed)
								c = cmplx.Conj(complex(realc, 0) * dummy)
								s = cmplx.Conj(-s * dummy)
			...
			if topdwn {
				...
				for k = 1; k <= uub; k++ {
					for jc = 1; jc <= (*n)-1; jc++ {
						...
						Zlarot(true, true, false, toPtr(min(k, (*n)-jc)+1), &ct, &st, a.CVector((1-iskew)*jc+ioffg-1, jc-1), &ilda, &ctemp, &dummy)
