...
func dchkst(nsizes int, nn []int, ntypes int, dotype []bool, iseed []int, thresh float64, nounit int, a *mat.Matrix, ap, sd, se, d1, d2, d3, d4, d5, wa1, wa2, wa3, wr *mat.Vector, u, v *mat.Matrix, vp, tau *mat.Vector, z *mat.Matrix, work *mat.Vector, lwork int, iwork []int, liwork int, result *mat.Vector, t *testing.T) (nerrs, ntestt int, err error) {
	...
	var i, iinfo, il, imode, itemp, itype, iu, j, jc, jr, jsize, jtype, lgn, liwedc, log2ui, lwedc, m, m2, m3, maxtyp, mtypes, n, nap, nblock, nmats, nmax, ntest int
	idumma := make([]int, 1)
	ioldsd := make([]int, 4)
	iseed2 := make([]int, 4)
	kmagn := make([]int, 21)
	...
	for jsize = 1; jsize <= nsizes; jsize++ {
		...
		label100:
			...
			//           Do tests 1 and 2
			dsyt21(2, Upper, n, 1, a, sd, se, u, v, tau, work, result)
			...
			ntest = 3
			if err = golapack.Dsytrd(Lower, n, v, sd, se, tau, work, lwork); err != nil {
				t.Fail()
			...
			golapack.Dlacpy(Lower, n, n, v, u)

			ntest = 4
			if err = golapack.Dorgtr(Lower, n, u, tau, work, lwork); err != nil {
				t.Fail()
			...
			dsyt21(2, Lower, n, 1, a, sd, se, u, v, tau, work, result.Off(2))
			dsyt21(3, Lower, n, 1, a, sd, se, u, v, tau, work, result.Off(3))
		...
		label170:
			...
			if jtype > 15 {
				...
				ntest = 14
				if iinfo, err = golapack.Dpteqr('V', n, d4, work, z, work.Off(n)); err != nil {
					panic(err)
				}
				if iinfo != 0 {
					t.Fail()
				...
				ntest = 16
				if iinfo, err = golapack.Dpteqr('N', n, d5, work, z, work.Off(n)); err != nil {
					panic(err)
				}
				if iinfo != 0 {
					t.Fail()
