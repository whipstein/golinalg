...
func Dchksb2stg(nsizes *int, nn *[]int, nwdths *int, kk *[]int, ntypes *int, dotype *[]bool, iseed *[]int, thresh *float64, nounit *int, a *mat.Matrix, lda *int, sd, se, d1, d2, d3 *mat.Vector, u *mat.Matrix, ldu *int, work *mat.Vector, lwork *int, result *mat.Vector, info *int, t *testing.T) {
	...
	if (*info) != 0 {
		gltest.Xerbla([]byte("DCHKSBSTG"), -(*info))
		return
	}
	...
	for jsize = 1; jsize <= (*nsizes); jsize++ {
		...
		for jwidth = 1; jwidth <= (*nwdths); jwidth++ {
			...
			label100:
				...
				//              Do tests 1 and 2
				Dsbt21('U', &n, &k, func() *int { y := 1; return &y }(), a, lda, sd, se, u, ldu, work, result)
				...
				//              Do tests 3 and 4
				Dsbt21('L', &n, &k, func() *int { y := 1; return &y }(), a, lda, sd, se, u, ldu, work, result.Off(2))
