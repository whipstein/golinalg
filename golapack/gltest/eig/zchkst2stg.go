...
func zchkst2stg(nsizes int, nn []int, ntypes int, dotype []bool, iseed []int, thresh float64, a *mat.CMatrix, ap *mat.CVector, sd, se, d1, d2, d3, d4, d5, wa1, wa2, wa3, wr *mat.Vector, u, v *mat.CMatrix, vp, tau *mat.CVector, z *mat.CMatrix, work *mat.CVector, lwork int, rwork *mat.Vector, lrwork int, iwork []int, liwork int, result *mat.Vector) (nerrs, ntestt int, err error) {
	...
	var i, iinfo, il, imode, inde, indrwk, itemp, itype, iu, j, jc, jr, jsize, jtype, lgn, lh, liwedc, log2ui, lrwedc, lw, lwedc, m, m2, m3, maxtyp, mtypes, n, nap, nblock, nmats, nmax, ntest int
	dumma := vf(1)
	idumma := make([]int, 1)
	ioldsd := make([]int, 4)
	iseed2 := make([]int, 4)
	...
	for jsize = 1; jsize <= nsizes; jsize++ {
		...
		label100:
			...
			lh = max(1, 4*n)
			lw = lwork - lh
			if err = golapack.Zhetrd2stage('N', Upper, n, v, sd, se, tau, work, lh, work.Off(lh), lw); err != nil {
				panic(err)
			}
			...
			if err = golapack.Zhetrd2stage('N', Lower, n, v, sd, se, tau, work, lh, work.Off(lh), lw); err != nil {
				panic(err)
			}
		...
		label170:
			...
			d1.Copy(n, sd, 1, 1)
			if n > 0 {
				rwork.Off(inde-1).Copy(n-1, se, 1, 1)
			}
			golapack.Zlaset(Full, n, n, czero, cone, z)
			...
			if ilaenv(10, "Zstemr", []byte("VA"), 1, 0, 0, 0) == 1 && ilaenv(1, "Zstemr", []byte("VA"), 1, 0, 0, 0) == 1 {
