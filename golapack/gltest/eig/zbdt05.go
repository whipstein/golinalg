...
func Zbdt05(m, n *int, a *mat.CMatrix, lda *int, s *mat.Vector, ns *int, u *mat.CVector, ldu *int, vt *mat.CMatrix, ldvt *int, work *mat.CVector, resid *float64) {
	...
	//     Compute U' * A * V.
	goblas.Zgemm(NoTrans, ConjTrans, m, ns, n, &cone, a, lda, vt, ldvt, &czero, work.CMatrixOff(1+(*ns)*(*ns)-1, *m, opts), m)
	goblas.Zgemm(ConjTrans, NoTrans, ns, ns, m, toPtrc128(-cone), u.CMatrix(*ldu, opts), ldu, work.CMatrixOff(1+(*ns)*(*ns)-1, *m, opts), m, &czero, work.CMatrix(*ns, opts), ns)

	//     norm(S - U' * B * V)
	j = 0
	for i = 1; i <= (*ns); i++ {
		work.Set(j+i-1, work.Get(j+i-1)+complex(s.Get(i-1), zero))
		(*resid) = maxf64(*resid, goblas.Dzasum(ns, work.Off(j+1-1), func() *int { y := 1; return &y }()))
		j = j + (*ns)
	}
