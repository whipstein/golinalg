...
func Ddrvst(nsizes *int, nn *[]int, ntypes *int, dotype *[]bool, iseed *[]int, thresh *float64, nounit *int, a *mat.Matrix, lda *int, d1, d2, d3, d4, eveigs, wa1, wa2, wa3 *mat.Vector, u *mat.Matrix, ldu *int, v *mat.Matrix, tau *mat.Vector, z *mat.Matrix, work *mat.Vector, lwork *int, iwork *[]int, liwork *int, result *mat.Vector, info *int, t *testing.T) {
	var badnn bool
	var uplo byte
	var abstol, aninv, anorm, cond, half, one, ovfl, rtovfl, rtunfl, temp1, temp2, temp3, ten, two, ulp, ulpinv, unfl, vl, vu, zero float64
	var i, idiag, ihbw, iinfo, il, imode, indx, irow, itemp, itype, iu, iuplo, j, j1, j2, jcol, jsize, jtype, kd, lgn, liwedc, lwedc, m, m2, m3, maxtyp, mtypes, n, nerrs, nfails, nmats, nmax, ntest, ntestt int
	idumma := make([]int, 1)
	ioldsd := make([]int, 4)
	iseed2 := make([]int, 4)
	iseed3 := make([]int, 4)
	...
	*info = 0
	...
	if (*info) != 0 {
		gltest.Xerbla([]byte("DDRVST"), -(*info))
		return
	}
	...
	for jsize = 1; jsize <= (*nsizes); jsize++ {
		...
		label110:
			...
			if n <= 1 {
				...
				il = 1 + (n-1)*int(matgen.Dlarnd(toPtr(1), &iseed2))
				iu = 1 + (n-1)*int(matgen.Dlarnd(toPtr(1), &iseed2))
				if il > iu {
					itemp = il
					il = iu
					iu = itemp
			...
			for iuplo = 0; iuplo <= 1; iuplo++ {
				...
				*srnamt = "DSYEV"
				...
				ntest = ntest + 2
				*srnamt = "DSYEV"
				golapack.Dsyev('N', uplo, &n, a, ldu, d3, work, lwork, &iinfo)
				if iinfo != 0 {
					t.Fail()
					nerrs++
			...
			label1080:
				...
				*srnamt = "DSBEV"
				...
				ntest = ntest + 2
				*srnamt = "DSBEV"
				golapack.Dsbev('N', uplo, &n, &kd, v, ldu, d3, z, ldu, work, &iinfo)
				if iinfo != 0 {
					t.Fail()
					nerrs++
			...
			label1460:
				...
				*srnamt = "DSYEVD"
				...
				ntest = ntest + 2
				*srnamt = "DSYEVD"
				golapack.Dsyevd('N', uplo, &n, a, ldu, d3, work, &lwedc, iwork, &liwedc, &iinfo)
				if iinfo != 0 {
					t.Fail()
					nerrs++
			...
			label1580:
				...
				*srnamt = "DSBEVD"
				...
				ntest = ntest + 2
				*srnamt = "DSBEVD"
				golapack.Dsbevd('N', uplo, &n, &kd, v, ldu, d3, z, ldu, work, &lwedc, iwork, &liwedc, &iinfo)
				if iinfo != 0 {
					t.Fail()
					nerrs++
			...
			Dlafts([]byte("DST"), &n, &n, &jtype, &ntest, result, &ioldsd, thresh, &nfails, t)
	...
	//     Summary
	Alasvm([]byte("DST"), &nfails, &ntestt, &nerrs)
}
