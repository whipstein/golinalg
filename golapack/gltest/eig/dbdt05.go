...
func Dbdt05(m, n *int, a *mat.Matrix, lda *int, s *mat.Vector, ns *int, u *mat.Matrix, ldu *int, vt *mat.Matrix, ldvt *int, work *mat.Vector, resid *float64) {
	...
	//     Compute U' * A * V.
	goblas.Dgemm(NoTrans, Trans, m, ns, n, &one, a, lda, vt, ldvt, &zero, work.MatrixOff(1+(*ns)*(*ns)-1, *m, opts), m)
	goblas.Dgemm(Trans, NoTrans, ns, ns, m, toPtrf64(-one), u, ldu, work.MatrixOff(1+(*ns)*(*ns)-1, *m, opts), m, &zero, work.Matrix(*ns, opts), ns)

	//     norm(S - U' * B * V)
	j = 0
	for i = 1; i <= (*ns); i++ {
		work.Set(j+i-1, work.Get(j+i-1)+s.Get(i-1))
		(*resid) = maxf64(*resid, goblas.Dasum(ns, work.Off(j+1-1), toPtr(1)))
		j = j + (*ns)
	}
