...
func Zdrvbd(nsizes *int, mm, nn *[]int, ntypes *int, dotype *[]bool, iseed *[]int, thresh *float64, a *mat.CMatrix, lda *int, u *mat.CMatrix, ldu *int, vt *mat.CMatrix, ldvt *int, asav, usav, vtsav *mat.CMatrix, s, ssav, e *mat.Vector, work *mat.CVector, lwork *int, rwork *mat.Vector, iwork *[]int, nounit, info *int, t *testing.T) {
	...
	for j = 1; j <= (*nsizes); j++ {
		...
		minwrk = maxint(minwrk, maxint(3*minint((*mm)[j-1], (*nn)[j-1])+powint(maxint((*mm)[j-1], (*nn)[j-1]), 2), 5*minint((*mm)[j-1], (*nn)[j-1]), 3*maxint((*mm)[j-1], (*nn)[j-1])))
	}
		...
		label50:
			...
			for iwspc = 1; iwspc <= 4; iwspc++ {
				...
				*srnamt = "ZGESVD"
				...
				for iju = 0; iju <= 3; iju++ {
					for ijvt = 0; ijvt <= 3; ijvt++ {
						...
						*srnamt = "ZGESVD"
						...
						//                    Compare S
						dif = zero
						div = maxf64(float64(mnmin)*ulp*s.Get(0), golapack.Dlamch(SafeMinimum))
						for i = 1; i <= mnmin-1; i++ {
							if ssav.Get(i-1) < ssav.Get(i+1-1) {
								dif = ulpinv
							}
				...
				//              Test for ZGESDD
				iwtmp = 2*mnmin*mnmin + 2*mnmin + maxint(m, n)
				lswork = iwtmp + (iwspc-1)*((*lwork)-iwtmp)/3
				lswork = minint(lswork, *lwork)
				lswork = maxint(lswork, 1)
				if iwspc == 4 {
