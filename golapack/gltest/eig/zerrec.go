...
func zerrec(path string, t *testing.T) (nt int) {
	...
	*errt = fmt.Errorf("job != 'N' && !wants && !wantsp: job='X'")
	_, _, _, err = golapack.Ztrsen('X', 'N', sel, 0, a.Off(0, 0).UpdateRows(1), b.Off(0, 0).UpdateRows(1), x, work.Off(0, 1), 1)
	chkxer2("Ztrsen", err)
	*errt = fmt.Errorf("compq != 'N' && !wantq: compq='X'")
	_, _, _, err = golapack.Ztrsen('N', 'X', sel, 0, a.Off(0, 0).UpdateRows(1), b.Off(0, 0).UpdateRows(1), x, work.Off(0, 1), 1)
	chkxer2("Ztrsen", err)
	*errt = fmt.Errorf("n < 0: n=-1")
	_, _, _, err = golapack.Ztrsen('N', 'N', sel, -1, a.Off(0, 0).UpdateRows(1), b.Off(0, 0).UpdateRows(1), x, work.Off(0, 1), 1)
	chkxer2("Ztrsen", err)
	*errt = fmt.Errorf("t.Rows < max(1, n): t.Rows=1, n=2")
	_, _, _, err = golapack.Ztrsen('N', 'N', sel, 2, a.Off(0, 0).UpdateRows(1), b.Off(0, 0).UpdateRows(1), x, work.Off(0, 2), 2)
	chkxer2("Ztrsen", err)
	*errt = fmt.Errorf("q.Rows < 1 || (wantq && q.Rows < n): compq='V', q.Rows=1, n=2")
	_, _, _, err = golapack.Ztrsen('N', 'V', sel, 2, a.Off(0, 0).UpdateRows(2), b.Off(0, 0).UpdateRows(1), x, work.Off(0, 1), 1)
	chkxer2("Ztrsen", err)
	*errt = fmt.Errorf("lwork < lwmin && !lquery: lwork=0, lwmin=1, lquery=false")
	_, _, _, err = golapack.Ztrsen('N', 'V', sel, 2, a.Off(0, 0).UpdateRows(2), b.Off(0, 0).UpdateRows(2), x, work.Off(0, 0), 0)
	chkxer2("Ztrsen", err)
	*errt = fmt.Errorf("lwork < lwmin && !lquery: lwork=1, lwmin=2, lquery=false")
	_, _, _, err = golapack.Ztrsen('E', 'V', sel, 3, a.Off(0, 0).UpdateRows(3), b.Off(0, 0).UpdateRows(3), x, work.Off(0, 1), 1)
	chkxer2("Ztrsen", err)
	*errt = fmt.Errorf("lwork < lwmin && !lquery: lwork=3, lwmin=4, lquery=false")
	_, _, _, err = golapack.Ztrsen('V', 'V', sel, 3, a.Off(0, 0).UpdateRows(3), b.Off(0, 0).UpdateRows(3), x, work.Off(0, 3), 3)
	chkxer2("Ztrsen", err)
	nt = nt + 8
