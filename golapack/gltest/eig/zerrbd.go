...
func zerrbd(path string, t *testing.T) (nt int) {
	...
	if c2 == "bd" {
		...
		*errt = fmt.Errorf("!wantq && vect != 'P': vect='/'")
		err = golapack.Zungbr('/', 0, 0, 0, a.Off(0, 0).UpdateRows(1), tq, w, 1)
		chkxer2("Zungbr", err)
		*errt = fmt.Errorf("m < 0: m=-1")
		err = golapack.Zungbr('Q', -1, 0, 0, a.Off(0, 0).UpdateRows(1), tq, w, 1)
		chkxer2("Zungbr", err)
		*errt = fmt.Errorf("n < 0 || (wantq && (n > m || n < min(m, k))) || (!wantq && (m > n || m < min(n, k))): vect='Q', m=0, n=-1, k=0")
		err = golapack.Zungbr('Q', 0, -1, 0, a.Off(0, 0).UpdateRows(1), tq, w, 1)
		chkxer2("Zungbr", err)
		*errt = fmt.Errorf("n < 0 || (wantq && (n > m || n < min(m, k))) || (!wantq && (m > n || m < min(n, k))): vect='Q', m=0, n=1, k=0")
		err = golapack.Zungbr('Q', 0, 1, 0, a.Off(0, 0).UpdateRows(1), tq, w, 1)
		chkxer2("Zungbr", err)
		*errt = fmt.Errorf("n < 0 || (wantq && (n > m || n < min(m, k))) || (!wantq && (m > n || m < min(n, k))): vect='Q', m=1, n=0, k=1")
		err = golapack.Zungbr('Q', 1, 0, 1, a.Off(0, 0).UpdateRows(1), tq, w, 1)
		chkxer2("Zungbr", err)
		*errt = fmt.Errorf("n < 0 || (wantq && (n > m || n < min(m, k))) || (!wantq && (m > n || m < min(n, k))): vect='P', m=1, n=0, k=0")
		err = golapack.Zungbr('P', 1, 0, 0, a.Off(0, 0).UpdateRows(1), tq, w, 1)
		chkxer2("Zungbr", err)
		*errt = fmt.Errorf("n < 0 || (wantq && (n > m || n < min(m, k))) || (!wantq && (m > n || m < min(n, k))): vect='P', m=0, n=1, k=1")
		err = golapack.Zungbr('P', 0, 1, 1, a.Off(0, 0).UpdateRows(1), tq, w, 1)
		chkxer2("Zungbr", err)
		*errt = fmt.Errorf("k < 0: k=-1")
		err = golapack.Zungbr('Q', 0, 0, -1, a.Off(0, 0).UpdateRows(1), tq, w, 1)
		chkxer2("Zungbr", err)
		*errt = fmt.Errorf("a.Rows < max(1, m): a.Rows=1, m=2")
		err = golapack.Zungbr('Q', 2, 1, 1, a.Off(0, 0).UpdateRows(1), tq, w, 1)
		chkxer2("Zungbr", err)
		*errt = fmt.Errorf("lwork < max(1, mn) && !lquery: lwork=1, mn=2, lquery=false")
		err = golapack.Zungbr('Q', 2, 2, 1, a.Off(0, 0).UpdateRows(2), tq, w, 1)
		chkxer2("Zungbr", err)
		nt = nt + 10
		...
		*errt = fmt.Errorf("!applyq && vect != 'P': vect='/'")
		err = golapack.Zunmbr('/', Left, Trans, 0, 0, 0, a.Off(0, 0).UpdateRows(1), tq, u.Off(0, 0).UpdateRows(1), w, 1)
		chkxer2("Zunmbr", err)
		*errt = fmt.Errorf("!left && side != Right: side=Unrecognized: /")
		err = golapack.Zunmbr('Q', '/', Trans, 0, 0, 0, a.Off(0, 0).UpdateRows(1), tq, u.Off(0, 0).UpdateRows(1), w, 1)
		chkxer2("Zunmbr", err)
		*errt = fmt.Errorf("!notran && trans != ConjTrans: trans=Unrecognized: /")
		err = golapack.Zunmbr('Q', Left, '/', 0, 0, 0, a.Off(0, 0).UpdateRows(1), tq, u.Off(0, 0).UpdateRows(1), w, 1)
		chkxer2("Zunmbr", err)
		*errt = fmt.Errorf("m < 0: m=-1")
		err = golapack.Zunmbr('Q', Left, ConjTrans, -1, 0, 0, a.Off(0, 0).UpdateRows(1), tq, u.Off(0, 0).UpdateRows(1), w, 1)
		chkxer2("Zunmbr", err)
		*errt = fmt.Errorf("n < 0: n=-1")
		err = golapack.Zunmbr('Q', Left, ConjTrans, 0, -1, 0, a.Off(0, 0).UpdateRows(1), tq, u.Off(0, 0).UpdateRows(1), w, 1)
		chkxer2("Zunmbr", err)
		*errt = fmt.Errorf("k < 0: k=-1")
		err = golapack.Zunmbr('Q', Left, ConjTrans, 0, 0, -1, a.Off(0, 0).UpdateRows(1), tq, u.Off(0, 0).UpdateRows(1), w, 1)
		chkxer2("Zunmbr", err)
		*errt = fmt.Errorf("(applyq && a.Rows < max(1, nq)) || (!applyq && a.Rows < max(1, min(nq, k))): vect='Q', a.Rows=1, nq=2, k=0")
		err = golapack.Zunmbr('Q', Left, ConjTrans, 2, 0, 0, a.Off(0, 0).UpdateRows(1), tq, u.Off(0, 0).UpdateRows(2), w, 1)
		chkxer2("Zunmbr", err)
		*errt = fmt.Errorf("(applyq && a.Rows < max(1, nq)) || (!applyq && a.Rows < max(1, min(nq, k))): vect='Q', a.Rows=1, nq=2, k=0")
		err = golapack.Zunmbr('Q', Right, ConjTrans, 0, 2, 0, a.Off(0, 0).UpdateRows(1), tq, u.Off(0, 0).UpdateRows(1), w, 1)
		chkxer2("Zunmbr", err)
		*errt = fmt.Errorf("(applyq && a.Rows < max(1, nq)) || (!applyq && a.Rows < max(1, min(nq, k))): vect='P', a.Rows=1, nq=2, k=2")
		err = golapack.Zunmbr('P', Left, ConjTrans, 2, 0, 2, a.Off(0, 0).UpdateRows(1), tq, u.Off(0, 0).UpdateRows(2), w, 1)
		chkxer2("Zunmbr", err)
		*errt = fmt.Errorf("(applyq && a.Rows < max(1, nq)) || (!applyq && a.Rows < max(1, min(nq, k))): vect='P', a.Rows=1, nq=2, k=2")
		err = golapack.Zunmbr('P', Right, ConjTrans, 0, 2, 2, a.Off(0, 0).UpdateRows(1), tq, u.Off(0, 0).UpdateRows(1), w, 1)
		chkxer2("Zunmbr", err)
		*errt = fmt.Errorf("c.Rows < max(1, m): c.Rows=1, m=2")
		err = golapack.Zunmbr('Q', Right, ConjTrans, 2, 0, 0, a.Off(0, 0).UpdateRows(1), tq, u.Off(0, 0).UpdateRows(1), w, 1)
		chkxer2("Zunmbr", err)
		*errt = fmt.Errorf("lwork < max(1, nw) && !lquery: lwork=0, nw=0, lquery=false")
		err = golapack.Zunmbr('Q', Left, ConjTrans, 0, 2, 0, a.Off(0, 0).UpdateRows(1), tq, u.Off(0, 0).UpdateRows(1), w, 0)
		chkxer2("Zunmbr", err)
		*errt = fmt.Errorf("lwork < max(1, nw) && !lquery: lwork=0, nw=0, lquery=false")
		err = golapack.Zunmbr('Q', Right, ConjTrans, 2, 0, 0, a.Off(0, 0).UpdateRows(1), tq, u.Off(0, 0).UpdateRows(2), w, 0)
		chkxer2("Zunmbr", err)
		nt = nt + 13
