...
func zerrhs(path string, t *testing.T) (nt int) {
	...
	if c2 == "hs" {
		...
		*srnamt = "Zunghr"
		*errt = fmt.Errorf("n < 0: n=-1")
		err = golapack.Zunghr(-1, 1, 1, a.Off(0, 0).UpdateRows(1), tau, w, 1)
		chkxer2("Zunghr", err)
		*errt = fmt.Errorf("ilo < 1 || ilo > max(1, n): ilo=0, n=0")
		err = golapack.Zunghr(0, 0, 0, a.Off(0, 0).UpdateRows(1), tau, w, 1)
		chkxer2("Zunghr", err)
		*errt = fmt.Errorf("ilo < 1 || ilo > max(1, n): ilo=2, n=0")
		err = golapack.Zunghr(0, 2, 0, a.Off(0, 0).UpdateRows(1), tau, w, 1)
		chkxer2("Zunghr", err)
		*errt = fmt.Errorf("ihi < min(ilo, n) || ihi > n: ilo=1, ihi=0, n=1")
		err = golapack.Zunghr(1, 1, 0, a.Off(0, 0).UpdateRows(1), tau, w, 1)
		chkxer2("Zunghr", err)
		*errt = fmt.Errorf("ihi < min(ilo, n) || ihi > n: ilo=1, ihi=1, n=0")
		err = golapack.Zunghr(0, 1, 1, a.Off(0, 0).UpdateRows(1), tau, w, 1)
		chkxer2("Zunghr", err)
		*errt = fmt.Errorf("a.Rows < max(1, n): a.Rows=1, n=2")
		err = golapack.Zunghr(2, 1, 1, a.Off(0, 0).UpdateRows(1), tau, w, 1)
		chkxer2("Zunghr", err)
		*errt = fmt.Errorf("lwork < max(1, nh) && !lquery: lwork=1, nh=2, lquery=false")
		err = golapack.Zunghr(3, 1, 3, a.Off(0, 0).UpdateRows(3), tau, w, 1)
		chkxer2("Zunghr", err)
		nt = nt + 7
		...
		*errt = fmt.Errorf("!left && side != Right: side=Unrecognized: /")
		err = golapack.Zunmhr('/', NoTrans, 0, 0, 1, 0, a.Off(0, 0).UpdateRows(1), tau, c.Off(0, 0).UpdateRows(1), w, 1)
		chkxer2("Zunmhr", err)
		*errt = fmt.Errorf("trans != NoTrans && trans != ConjTrans: trans=Unrecognized: /")
		err = golapack.Zunmhr(Left, '/', 0, 0, 1, 0, a.Off(0, 0).UpdateRows(1), tau, c.Off(0, 0).UpdateRows(1), w, 1)
		chkxer2("Zunmhr", err)
		*errt = fmt.Errorf("m < 0: m=-1")
		err = golapack.Zunmhr(Left, NoTrans, -1, 0, 1, 0, a.Off(0, 0).UpdateRows(1), tau, c.Off(0, 0).UpdateRows(1), w, 1)
		chkxer2("Zunmhr", err)
		*errt = fmt.Errorf("n < 0: n=-1")
		err = golapack.Zunmhr(Left, NoTrans, 0, -1, 1, 0, a.Off(0, 0).UpdateRows(1), tau, c.Off(0, 0).UpdateRows(1), w, 1)
		chkxer2("Zunmhr", err)
		*errt = fmt.Errorf("ilo < 1 || ilo > max(1, nq): ilo=0, ihi=0, nq=0")
		err = golapack.Zunmhr(Left, NoTrans, 0, 0, 0, 0, a.Off(0, 0).UpdateRows(1), tau, c.Off(0, 0).UpdateRows(1), w, 1)
		chkxer2("Zunmhr", err)
		*errt = fmt.Errorf("ilo < 1 || ilo > max(1, nq): ilo=2, ihi=0, nq=0")
		err = golapack.Zunmhr(Left, NoTrans, 0, 0, 2, 0, a.Off(0, 0).UpdateRows(1), tau, c.Off(0, 0).UpdateRows(1), w, 1)
		chkxer2("Zunmhr", err)
		*errt = fmt.Errorf("ilo < 1 || ilo > max(1, nq): ilo=2, ihi=1, nq=1")
		err = golapack.Zunmhr(Left, NoTrans, 1, 2, 2, 1, a.Off(0, 0).UpdateRows(1), tau, c.Off(0, 0).UpdateRows(1), w, 2)
		chkxer2("Zunmhr", err)
		*errt = fmt.Errorf("ilo < 1 || ilo > max(1, nq): ilo=2, ihi=1, nq=1")
		err = golapack.Zunmhr(Right, NoTrans, 2, 1, 2, 1, a.Off(0, 0).UpdateRows(1), tau, c.Off(0, 0).UpdateRows(2), w, 2)
		chkxer2("Zunmhr", err)
		*errt = fmt.Errorf("ihi < min(ilo, nq) || ihi > nq: ilo=1, ihi=0, nq=1")
		err = golapack.Zunmhr(Left, NoTrans, 1, 1, 1, 0, a.Off(0, 0).UpdateRows(1), tau, c.Off(0, 0).UpdateRows(1), w, 1)
		chkxer2("Zunmhr", err)
		*errt = fmt.Errorf("ihi < min(ilo, nq) || ihi > nq: ilo=1, ihi=1, nq=0")
		err = golapack.Zunmhr(Left, NoTrans, 0, 1, 1, 1, a.Off(0, 0).UpdateRows(1), tau, c.Off(0, 0).UpdateRows(1), w, 1)
		chkxer2("Zunmhr", err)
		*errt = fmt.Errorf("ihi < min(ilo, nq) || ihi > nq: ilo=1, ihi=1, nq=0")
		err = golapack.Zunmhr(Right, NoTrans, 1, 0, 1, 1, a.Off(0, 0).UpdateRows(1), tau, c.Off(0, 0).UpdateRows(1), w, 1)
		chkxer2("Zunmhr", err)
		*errt = fmt.Errorf("a.Rows < max(1, nq): a.Rows=1, nq=2")
		err = golapack.Zunmhr(Left, NoTrans, 2, 1, 1, 1, a.Off(0, 0).UpdateRows(1), tau, c.Off(0, 0).UpdateRows(2), w, 1)
		chkxer2("Zunmhr", err)
		*errt = fmt.Errorf("a.Rows < max(1, nq): a.Rows=1, nq=2")
		err = golapack.Zunmhr(Right, NoTrans, 1, 2, 1, 1, a.Off(0, 0).UpdateRows(1), tau, c.Off(0, 0).UpdateRows(1), w, 1)
		chkxer2("Zunmhr", err)
		*errt = fmt.Errorf("c.Rows < max(1, m): c.Rows=1, m=2")
		err = golapack.Zunmhr(Left, NoTrans, 2, 1, 1, 1, a.Off(0, 0).UpdateRows(2), tau, c.Off(0, 0).UpdateRows(1), w, 1)
		chkxer2("Zunmhr", err)
		*errt = fmt.Errorf("lwork < max(1, nw) && !lquery: lwork=1, nw=2, lquery=false")
		err = golapack.Zunmhr(Left, NoTrans, 1, 2, 1, 1, a.Off(0, 0).UpdateRows(1), tau, c.Off(0, 0).UpdateRows(1), w, 1)
		chkxer2("Zunmhr", err)
		*errt = fmt.Errorf("lwork < max(1, nw) && !lquery: lwork=1, nw=2, lquery=false")
		err = golapack.Zunmhr(Right, NoTrans, 2, 1, 1, 1, a.Off(0, 0).UpdateRows(1), tau, c.Off(0, 0).UpdateRows(2), w, 1)
		chkxer2("Zunmhr", err)
		nt = nt + 16
