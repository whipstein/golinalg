...
func ddrvvx(nsizes int, nn []int, ntypes int, dotype []bool, iseed []int, thresh float64, nounit int, a, h *mat.Matrix, wr, wi, wr1, wi1 *mat.Vector, vl, vr, lre *mat.Matrix, rcondv, rcndv1, rcdvin, rconde, rcnde1, rcdein, scale, scale1, result, work *mat.Vector, nwork int, iwork []int, t *testing.T) (nerrs, ntestt int, err error) {
	var badnn bool
	var balanc byte
	var anorm, cond, conds, one, ovfl, rtulp, rtulpi, ulp, ulpinv, unfl, zero float64
	var _i, i, ibal, iinfo, imode, itype, iwk, j, jcol, jsize, jtype, maxtyp, mtypes, n, nfail, nmax, nnwork, ntest, ntestf int
	adumma := make([]byte, 1)
	bal := []byte{'N', 'P', 'S', 'B'}
	idumma := make([]int, 1)
	ioldsd := make([]int, 4)
