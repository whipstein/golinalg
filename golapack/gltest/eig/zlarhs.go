...
func Zlarhs(path []byte, xtype, uplo, trans byte, m, n, kl, ku, nrhs *int, a *mat.CMatrix, lda *int, x *mat.CMatrix, ldx *int, b *mat.CMatrix, ldb *int, iseed *[]int, info *int) {
	...
	*info = 0
	...
	if string(c2) == "GE" || string(c2) == "QR" || string(c2) == "LQ" || string(c2) == "QL" || string(c2) == "RQ" {
		//        General matrix
		if b.Rows < mb {
			err = goblas.Zgemm(mat.TransByte(trans), NoTrans, mb, *nrhs, nx, one, a, x, zero, b.Off(0, 0).UpdateRows(mb))
		} else {
		...
		for j = 1; j <= (*nrhs); j++ {
			golapack.Zspmv(uplo, n, &one, a.CVector(0, 0), x.CVector(0, j-1), func() *int { y := 1; return &y }(), &zero, b.CVector(0, j-1), func() *int { y := 1; return &y }())
		}
