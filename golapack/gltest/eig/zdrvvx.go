...
func Zdrvvx(nsizes *int, nn *[]int, ntypes *int, dotype *[]bool, iseed *[]int, thresh *float64, nounit *int, a *mat.CMatrix, lda *int, h *mat.CMatrix, w, w1 *mat.CVector, vl *mat.CMatrix, ldvl *int, vr *mat.CMatrix, ldvr *int, lre *mat.CMatrix, ldlre *int, rcondv, rcndv1, rcdvin, rconde, rcnde1, rcdein, scale, scale1, result *mat.Vector, work *mat.CVector, nwork *int, rwork *mat.Vector, info *int, t *testing.T) {
	...
	if (*nsizes) < 0 {
		...
		(*info) = -19
	} else if 6*nmax+2*powint(nmax, 2) > (*nwork) {
		(*info) = -30
	}

	if (*info) != 0 {
		gltest.Xerbla([]byte("ZDRVVX"), -(*info))
		return
	}
	...
	for jsize = 1; jsize <= (*nsizes); jsize++ {
		...
		label60:
			...
			if itype == 1 {
				...
				matgen.Zlatmr(&n, &n, 'D', iseed, 'S', work, func() *int { y := 6; return &y }(), &one, &cone, 'T', 'N', work.Off(n+1-1), func() *int { y := 1; return &y }(), &one, work.Off(2*n+1-1), func() *int { y := 1; return &y }(), &one, 'N', &idumma, func() *int { y := 0; return &y }(), func() *int { y := 0; return &y }(), &zero, &anorm, 'N', a, lda, &idumma, &iinfo)
		...
		label90:
			...
			for iwk = 1; iwk <= 3; iwk++ {
				if iwk == 1 {
					...
					nnwork = 2*n + powint(n, 2)
				} else {
					nnwork = 6*n + 2*powint(n, 2)
				}
				...
				for ibal = 1; ibal <= 4; ibal++ {
					...
					//                 Perform tests
					Zget23(false, func() *int { y := 0; return &y }(), balanc, &jtype, thresh, &ioldsd, nounit, &n, a, lda, h, w, w1, vl, ldvl, vr, ldvr, lre, ldlre, rcondv, rcndv1, rcdvin, rconde, rcnde1, rcdein, scale, scale1, result, work, &nnwork, rwork, info)
...
label160:
	...
	for _i, n = range nlist {
		...
		Zget23(true, &isrt, 'N', func() *int { y := 22; return &y }(), thresh, iseed, nounit, &n, a, lda, h, w, w1, vl, ldvl, vr, ldvr, lre, ldlre, rcondv, rcndv1, rcdvin, rconde, rcnde1, rcdein, scale, scale1, result, work, toPtr(6*n+2*powint(n, 2)), rwork, info)
