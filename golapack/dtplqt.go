...
func Dtplqt(m, n, l, mb *int, a *mat.Matrix, lda *int, b *mat.Matrix, ldb *int, t *mat.Matrix, ldt *int, work *mat.Vector, info *int) {
	...
	for i = 1; i <= (*m); i += (*mb) {
		...
		Dtplqt2(&ib, &nb, &lb, a.Off(i-1, i-1), lda, b.Off(i-1, 0), ldb, t.Off(0, i-1), ldt, &iinfo)

		//     Update by applying H**T to B(I+IB:M,:) from the right
		if i+ib <= (*m) {
			Dtprfb('R', 'N', 'F', 'R', toPtr((*m)-i-ib+1), &nb, &ib, &lb, b.Off(i-1, 0), ldb, t.Off(0, i-1), ldt, a.Off(i+ib-1, i-1), lda, b.Off(i+ib-1, 0), ldb, work.Matrix((*m)-i-ib+1, opts), toPtr((*m)-i-ib+1))
		}
	}
}
