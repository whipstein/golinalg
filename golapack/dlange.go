...
// real matrix A.
func Dlange(norm byte, m *int, n *int, a *mat.Matrix, lda *int, work *mat.Vector) (dlangeReturn float64) {
	...
	if minint(*m, *n) == 0 {
		...
		for j = 1; j <= (*n); j++ {
			colssq.Set(0, zero)
			colssq.Set(1, one)
			Dlassq(m, a.Vector(0, j-1), func() *int { y := 1; return &y }(), colssq.GetPtr(0), colssq.GetPtr(1))
			Dcombssq(ssq, colssq)
		}
