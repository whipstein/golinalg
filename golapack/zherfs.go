...
func Zherfs(uplo mat.MatUplo, n, nrhs int, a, af *mat.CMatrix, ipiv *[]int, b, x *mat.CMatrix, ferr, berr *mat.Vector, work *mat.CVector, rwork *mat.Vector) (err error) {
	var upper bool
	var one complex128
	...
	label20:
		...
		if err = goblas.Zhemv(uplo, n, -one, a, x.CVector(0, j-1, 1), one, work.Off(0, 1)); err != nil {
			panic(err)
		}
		...
		if berr.Get(j-1) > eps && two*berr.Get(j-1) <= lstres && count <= itmax {
			...
			if err = Zhetrs(uplo, n, 1, af, ipiv, work.CMatrix(n, opts)); err != nil {
				panic(err)
			}
