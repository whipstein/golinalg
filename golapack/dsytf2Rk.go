...
func Dsytf2Rk(uplo mat.MatUplo, n int, a *mat.Matrix, e *mat.Vector, ipiv *[]int) (info int, err error) {
	var done, upper bool
	...
	label10:
		...
		if math.Max(absakk, colmax) == zero {
			...
			if (kstep == 2) && (p != k) {
				...
				if p < (k - 1) {
					goblas.Dswap(k-p-1, a.Vector(p, k-1, 1), a.Vector(p-1, p))
				}
				t = a.Get(k-1, k-1)
				...
				if k < n {
					goblas.Dswap(n-k, a.Vector(k-1, k), a.Vector(p-1, k))
				}
			...
			if kstep == 1 {
				...
				if k > 1 {
					...
					if math.Abs(a.Get(k-1, k-1)) >= sfmin {
						...
						if err = goblas.Dsyr(uplo, k-1, -d11, a.Vector(0, k-1, 1), a); err != nil {
							panic(err)
						}
	...
	label40:
		...
		if math.Max(absakk, colmax) == zero {
			...
			if (kstep == 2) && (p != k) {
				...
				if p > (k + 1) {
					goblas.Dswap(p-k-1, a.Vector(k, k-1, 1), a.Vector(p-1, k))
				}
				t = a.Get(k-1, k-1)
				a.Set(k-1, k-1, a.Get(p-1, p-1))
				a.Set(p-1, p-1, t)
				...
				if k > 1 {
					goblas.Dswap(k-1, a.Vector(k-1, 0), a.Vector(p-1, 0))
				}
