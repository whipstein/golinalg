...
func Zunmlq(side mat.MatSide, trans mat.MatTrans, m, n, k int, a *mat.CMatrix, tau *mat.CVector, c *mat.CMatrix, work *mat.CVector, lwork int) (err error) {
	...
	if nb < nbmin || nb >= k {
		...
		for _, i = range genIter(i1, i2, i3) {
			...
			Zlarft('F', 'R', nq-i+1, ib, a.Off(i-1, i-1), tau.Off(i-1), work.CMatrixOff(iwt-1, ldt, opts))
			if left {
			...
			//           Apply H or H**H
			Zlarfb(side, transt, 'F', 'R', mi, ni, ib, a.Off(i-1, i-1), work.CMatrixOff(iwt-1, ldt, opts), c.Off(ic-1, jc-1), work.CMatrix(ldwork, opts))
		}
	}
	work.SetRe(0, float64(lwkopt))
