...
func Dtrrfs(uplo, trans, diag byte, n, nrhs *int, a *mat.Matrix, lda *int, b *mat.Matrix, ldb *int, x *mat.Matrix, ldx *int, ferr, berr, work *mat.Vector, iwork *[]int, info *int) {
	...
	if (*info) != 0 {
		gltest.Xerbla([]byte("DTRRFS"), -(*info))
		return
	}
	...
	for j = 1; j <= (*nrhs); j++ {
		...
		goblas.Dtrmv(mat.UploByte(uplo), mat.TransByte(trans), mat.DiagByte(diag), n, a, lda, work.Off((*n)+1-1), toPtr(1))
			...
			if upper {
				if nounit {
					for k = 1; k <= (*n); k++ {
						s = zero
						for i = 1; i <= k; i++ {
							s = s + math.Abs(a.Get(i-1, k-1))*math.Abs(x.Get(i-1, j-1))
						}
	...
	label210:
		...
		if kase != 0 {
			if kase == 1 {
				...
				goblas.Dtrsv(mat.UploByte(uplo), mat.TransByte(transt), mat.DiagByte(diag), n, a, lda, work.Off((*n)+1-1), toPtr(1))
				for i = 1; i <= (*n); i++ {
				...
				goblas.Dtrsv(mat.UploByte(uplo), mat.TransByte(trans), mat.DiagByte(diag), n, a, lda, work.Off((*n)+1-1), toPtr(1))
			}
			goto label210
		}
