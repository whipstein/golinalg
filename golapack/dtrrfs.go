...
func Dtrrfs(uplo mat.MatUplo, trans mat.MatTrans, diag mat.MatDiag, n, nrhs int, a, b, x *mat.Matrix, ferr, berr, work *mat.Vector, iwork *[]int) (err error) {
	...
	for j = 1; j <= nrhs; j++ {
		...
		if err = goblas.Dtrmv(uplo, trans, diag, n, a, work.Off(n, 1)); err != nil {
			panic(err)
		}
		...
		*_ferr, kase = Dlacn2(n, work.Off(2*n), work.Off(n), iwork, ferr.Get(j-1), kase, &isave)
		if kase != 0 {
			if kase == 1 {
				...
				err = goblas.Dtrsv(uplo, transt, diag, n, a, work.Off(n, 1))
				for i = 1; i <= n; i++ {
				...
				err = goblas.Dtrsv(uplo, trans, diag, n, a, work.Off(n, 1))
			}
			goto label210
		}
