...
func Dgeqlf(m, n *int, a *mat.Matrix, lda *int, tau, work *mat.Vector, lwork, info *int) {
	...
	if nb > 1 && nb < k {
		...
		nx = max(0, Ilaenv(func() *int { y := 3; return &y }(), []byte("DGEQLF"), []byte{' '}, m, n, toPtr(-1), toPtr(-1)))
		if nx < k {
			...
			if (*lwork) < iws {
				...
				nb = (*lwork) / ldwork
				nbmin = max(2, Ilaenv(func() *int { y := 2; return &y }(), []byte("DGEQLF"), []byte{' '}, m, n, toPtr(-1), toPtr(-1)))
			}
		}
	}
	...
	if nb >= nbmin && nb < k && nx < k {
		...
		for i = k - kk + ki + 1; i >= k-kk+1; i -= nb {
			...
			if (*n)-k+i > 1 {
				...
				Dlarfb('L', 'T', 'B', 'C', toPtr((*m)-k+i+ib-1), toPtr((*n)-k+i-1), &ib, a.Off(0, (*n)-k+i-1), lda, work.Matrix(ldwork, opts), &ldwork, a, lda, work.MatrixOff(ib, ldwork, opts), &ldwork)
			}
		}
		mu = (*m) - k + i + nb - 1
		nu = (*n) - k + i + nb - 1
