...
func Zlatps(uplo, trans, diag, normin byte, n *int, ap, x *mat.CVector, scale *float64, cnorm *mat.Vector, info *int) {
	...
	if normin == 'N' {
		...
		if upper {
			...
			ip = 1
			for j = 1; j <= (*n)-1; j++ {
				cnorm.Set(j-1, goblas.Dzasum((*n)-j, ap.Off(ip+1-1), 1))
				ip = ip + (*n) - j + 1
			}
			cnorm.Set((*n)-1, zero)
		}
	...
	if (grow * tscal) > smlnum {
		...
		err = goblas.Ztpsv(mat.UploByte(uplo), mat.TransByte(trans), mat.DiagByte(diag), *n, ap, x, 1)
	} else {
		...
		if notran {
			...
			label110:
				...
				if upper {
					if j > 1 {
						...
						goblas.Zaxpy(j-1, -x.Get(j-1)*complex(tscal, 0), ap.Off(ip-j+1-1), 1, x, 1)
					...
					if j < (*n) {
						...
						goblas.Zaxpy((*n)-j, -x.Get(j-1)*complex(tscal, 0), ap.Off(ip+1-1), 1, x.Off(j+1-1), 1)
			...
			for _, j = range genIter(jfirst, jlast, jinc) {
				...
				if uscal == complex(one, 0) {
					...
					if upper {
						csumj = goblas.Zdotu(j-1, ap.Off(ip-j+1-1), 1, x, 1)
					} else if j < (*n) {
						csumj = goblas.Zdotu((*n)-j, ap.Off(ip+1-1), 1, x.Off(j+1-1), 1)
					}
				} else {
			...
			for _, j = range genIter(jfirst, jlast, jinc) {
				...
				if uscal == complex(one, 0) {
					...
					if upper {
						csumj = goblas.Zdotc(j-1, ap.Off(ip-j+1-1), 1, x, 1)
					} else if j < (*n) {
						csumj = goblas.Zdotc((*n)-j, ap.Off(ip+1-1), 1, x.Off(j+1-1), 1)
					}
				} else {
					//                 Otherwise, use in-line code for the dot product.
					if upper {
