	...
	label30:
		...
		if (*ipiv)[k-1] > 0 {
			...
			if k > 1 {
				...
				ap.Set(kc+k-1-1, ap.Get(kc+k-1-1)-goblas.Zdotu(toPtr(k-1), work, func() *int { y := 1; return &y }(), ap.Off(kc-1), func() *int { y := 1; return &y }()))
			}
			kstep = 1
		} else {
			...
			if k > 1 {
				...
				Zspmv(uplo, toPtr(k-1), toPtrc128(-one), ap, work, func() *int { y := 1; return &y }(), &zero, ap.Off(kc-1), func() *int { y := 1; return &y }())
				ap.Set(kc+k-1-1, ap.Get(kc+k-1-1)-goblas.Zdotu(toPtr(k-1), work, func() *int { y := 1; return &y }(), ap.Off(kc-1), func() *int { y := 1; return &y }()))
				ap.Set(kcnext+k-1-1, ap.Get(kcnext+k-1-1)-goblas.Zdotu(toPtr(k-1), ap.Off(kc-1), func() *int { y := 1; return &y }(), ap.Off(kcnext-1), func() *int { y := 1; return &y }()))
				...
				ap.Set(kcnext+k-1, ap.Get(kcnext+k-1)-goblas.Zdotu(toPtr(k-1), work, func() *int { y := 1; return &y }(), ap.Off(kcnext-1), func() *int { y := 1; return &y }()))
			}
			kstep = 2
			kcnext = kcnext + k + 1
		}
	...
	label60:
		...
		if (*ipiv)[k-1] > 0 {
			...
			if k < (*n) {
				...
				Zspmv(uplo, toPtr((*n)-k), toPtrc128(-one), ap.Off(kc+(*n)-k+1-1), work, func() *int { y := 1; return &y }(), &zero, ap.Off(kc+1-1), func() *int { y := 1; return &y }())
				ap.Set(kc-1, ap.Get(kc-1)-goblas.Zdotu(toPtr((*n)-k), work, func() *int { y := 1; return &y }(), ap.Off(kc+1-1), func() *int { y := 1; return &y }()))
			}
			kstep = 1
		} else {
			//           2 x 2 diagonal block
			...
			akp1 = ap.Get(kc-1) / t
			akkp1 = ap.Get(kcnext+1-1) / t
			d = t * (ak*akp1 - one)
			ap.Set(kcnext-1, akp1/d)
			ap.Set(kc-1, ak/d)
			ap.Set(kcnext+1-1, -akkp1/d)
			//
			...
			if k < (*n) {
				...
				Zspmv(uplo, toPtr((*n)-k), toPtrc128(-one), ap.Off(kc+((*n)-k+1)-1), work, func() *int { y := 1; return &y }(), &zero, ap.Off(kc+1-1), func() *int { y := 1; return &y }())
				ap.Set(kc-1, ap.Get(kc-1)-goblas.Zdotu(toPtr((*n)-k), work, func() *int { y := 1; return &y }(), ap.Off(kc+1-1), func() *int { y := 1; return &y }()))
				ap.Set(kcnext+1-1, ap.Get(kcnext+1-1)-goblas.Zdotu(toPtr((*n)-k), ap.Off(kc+1-1), func() *int { y := 1; return &y }(), ap.Off(kcnext+2-1), func() *int { y := 1; return &y }()))
				...
				ap.Set(kcnext-1, ap.Get(kcnext-1)-goblas.Zdotu(toPtr((*n)-k), work, func() *int { y := 1; return &y }(), ap.Off(kcnext+2-1), func() *int { y := 1; return &y }()))
			}
			kstep = 2
			kcnext = kcnext - ((*n) - k + 3)
		}
