...
func Zlatdf(ijob, n int, z *mat.CMatrix, rhs *mat.CVector, rdsum, rdscal float64, ipiv, jpiv *[]int) (rdsumOut, rdscalOut float64) {
	...
	if ijob != 2 {
		...
		for j = 1; j <= n-1; j++ {
			...
			splus = splus + real(z.Off(j, j-1).CVector().Dotc(n-j, z.Off(j, j-1).CVector(), 1, 1))
			sminu = real(rhs.Off(j).Dotc(n-j, z.Off(j, j-1).CVector(), 1, 1))
			...
			temp = -rhs.Get(j - 1)
			rhs.Off(j).Axpy(n-j, temp, z.Off(j, j-1).CVector(), 1, 1)
		}
		...
		rdscalOut, rdsumOut = Zlassq(n, rhs, 1, rdscalOut, rdsumOut)
		return
	}
	...
	temp = cone / cmplx.Sqrt(xm.Dotc(n, xm, 1, 1))
	xm.Scal(n, temp, 1)
	xp.Copy(n, xm, 1, 1)
	xp.Axpy(n, cone, rhs, 1, 1)
	rhs.Axpy(n, -cone, xm, 1, 1)
