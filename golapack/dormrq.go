...
// if SIDE = 'R'.
func Dormrq(side, trans byte, m, n, k *int, a *mat.Matrix, lda *int, tau *mat.Vector, c *mat.Matrix, ldc *int, work *mat.Vector, lwork, info *int) {
	...
	if (*info) != 0 {
		gltest.Xerbla([]byte("DORMRQ"), -(*info))
		return
	} else if lquery {
		return
	}
	...
	if nb < nbmin || nb >= (*k) {
		...
		for _, i = range genIter(i1, i2, i3) {
			...
			Dlarft('B', 'R', toPtr(nq-(*k)+i+ib-1), &ib, a.Off(i-1, 0), lda, tau.Off(i-1), work.MatrixOff(iwt-1, ldt, opts), &ldt)
			if left {
			...
			//           Apply H or H**T
			Dlarfb(side, transt, 'B', 'R', &mi, &ni, &ib, a.Off(i-1, 0), lda, work.MatrixOff(iwt-1, ldt, opts), &ldt, c, ldc, work.Matrix(ldwork, opts), &ldwork)
		}
	}
	work.Set(0, float64(lwkopt))
}
