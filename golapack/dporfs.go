...
// solution.
func Dporfs(uplo mat.MatUplo, n, nrhs int, a, af, b, x *mat.Matrix, ferr, berr, work *mat.Vector, iwork *[]int) (err error) {
	var upper bool
	...
	label20:
		...
		if berr.Get(j-1) > eps && two*berr.Get(j-1) <= lstres && count <= itmax {
			...
			if err = Dpotrs(uplo, n, 1, af, work.Off(n).Matrix(n, opts)); err != nil {
				panic(err)
			}
