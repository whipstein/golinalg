...
func Dormql(side mat.MatSide, trans mat.MatTrans, m, n, k int, a *mat.Matrix, tau *mat.Vector, c *mat.Matrix, work *mat.Vector, lwork int) (err error) {
	...
	if nb < nbmin || nb >= k {
		...
		for _, i = range genIter(i1, i2, i3) {
			...
			Dlarft('B', 'C', nq-k+i+ib-1, ib, a.Off(0, i-1), tau.Off(i-1), work.Off(iwt-1).Matrix(ldt, opts))
			if left {
			...
			//           Apply H or H**T
			Dlarfb(side, trans, 'B', 'C', mi, ni, ib, a.Off(0, i-1), work.Off(iwt-1).Matrix(ldt, opts), c, work.Matrix(ldwork, opts))
		}
	}
	work.Set(0, float64(lwkopt))
