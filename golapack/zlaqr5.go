	...
	for incol = 3*(1-nbmps) + ktop - 1; incol <= kbot-2; incol += 3*nbmps - 2 {
		...
		for krcol = incol; krcol <= min(incol+3*nbmps-3, kbot-2); krcol++ {
			...
			for m = mtop; m <= mbot; m++ {
				k = krcol + 3*(m-1)
				if k == ktop-1 {
					Zlaqr1(3, h.Off(ktop-1, ktop-1), s.Get(2*m-1-1), s.Get(2*m-1), v.CVector(0, m-1))
					alpha = v.Get(0, m-1)
					alpha, *v.GetPtr(0, m-1) = Zlarfg(3, alpha, v.CVector(1, m-1, 1))
				} else {
					beta = h.Get(k, k-1)
					v.Set(1, m-1, h.Get(k+2-1, k-1))
					v.Set(2, m-1, h.Get(k+3-1, k-1))
					beta, *v.GetPtr(0, m-1) = Zlarfg(3, beta, v.CVector(1, m-1, 1))
			...
			if bmp22 {
				if k == ktop-1 {
					Zlaqr1(2, h.Off(k, k), s.Get(2*m22-1-1), s.Get(2*m22-1), v.CVector(0, m22-1))
					beta = v.Get(0, m22-1)
					beta, *v.GetPtr(0, m22-1) = Zlarfg(2, beta, v.CVector(1, m22-1, 1))
				} else {
					beta = h.Get(k, k-1)
		...
		if accum {
			...
			if (!blk22) || (incol < ktop) || (ndcol > kbot) || (ns <= 2) {
				...
				for jcol = min(ndcol, kbot) + 1; jcol <= jbot; jcol += nh {
					...
					if err = goblas.Zgemm(ConjTrans, NoTrans, nu, jlen, nu, one, u.Off(k1-1, k1-1), h.Off(incol+k1-1, jcol-1), zero, wh); err != nil {
						panic(err)
					}
				...
				for jrow = jtop; jrow <= max(ktop, incol)-1; jrow += nv {
					...
					if err = goblas.Zgemm(NoTrans, NoTrans, jlen, nu, nu, one, h.Off(jrow-1, incol+k1-1), u.Off(k1-1, k1-1), zero, wv); err != nil {
						panic(err)
					}
				...
				if wantz {
					for jrow = iloz; jrow <= ihiz; jrow += nv {
						...
						if err = goblas.Zgemm(NoTrans, NoTrans, jlen, nu, nu, one, z.Off(jrow-1, incol+k1-1), u.Off(k1-1, k1-1), zero, wv); err != nil {
							panic(err)
						}
				...
				for jcol = min(ndcol, kbot) + 1; jcol <= jbot; jcol += nh {
					...
					if err = goblas.Ztrmm(Left, Upper, ConjTrans, NonUnit, knz, jlen, one, u.Off(j2, 1+kzs-1), wh.Off(kzs, 0)); err != nil {
						panic(err)
					}

					//                 ==== Multiply top of H by U11**H ====
					if err = goblas.Zgemm(ConjTrans, NoTrans, i2, jlen, j2, one, u, h.Off(incol, jcol-1), one, wh); err != nil {
						panic(err)
					}
					...
					//                 ==== Multiply by U21**H ====
					if err = goblas.Ztrmm(Left, Lower, ConjTrans, NonUnit, j2, jlen, one, u.Off(0, i2), wh.Off(i2, 0)); err != nil {
						panic(err)
					}

					//                 ==== Multiply by U22 ====
					if err = goblas.Zgemm(ConjTrans, NoTrans, i4-i2, jlen, j4-j2, one, u.Off(j2, i2), h.Off(incol+1+j2-1, jcol-1), one, wh.Off(i2, 0)); err != nil {
						panic(err)
					}
				...
				for jrow = jtop; jrow <= max(incol, ktop)-1; jrow += nv {
					...
					if err = goblas.Ztrmm(Right, Upper, NoTrans, NonUnit, jlen, knz, one, u.Off(j2, 1+kzs-1), wv.Off(0, 1+kzs-1)); err != nil {
						panic(err)
					}

					//                 ==== Multiply by U11 ====
					if err = goblas.Zgemm(NoTrans, NoTrans, jlen, i2, j2, one, h.Off(jrow-1, incol), u, one, wv); err != nil {
						panic(err)
					}
					...
					//                 ==== Multiply by U21 ====
					if err = goblas.Ztrmm(Right, Lower, NoTrans, NonUnit, jlen, i4-i2, one, u.Off(0, i2), wv.Off(0, 1+i2-1)); err != nil {
						panic(err)
					}

					//                 ==== Multiply by U22 ====
					if err = goblas.Zgemm(NoTrans, NoTrans, jlen, i4-i2, j4-j2, one, h.Off(jrow-1, incol+1+j2-1), u.Off(j2, i2), one, wv.Off(0, 1+i2-1)); err != nil {
						panic(err)
					}
				...
				if wantz {
					for jrow = iloz; jrow <= ihiz; jrow += nv {
						...
						//                    ==== Multiply by U11 ====
						if err = goblas.Zgemm(NoTrans, NoTrans, jlen, i2, j2, one, z.Off(jrow-1, incol), u, one, wv); err != nil {
							panic(err)
						}
						...
						//                    ==== Multiply by U22 ====
						if err = goblas.Zgemm(NoTrans, NoTrans, jlen, i4-i2, j4-j2, one, z.Off(jrow-1, incol+1+j2-1), u.Off(j2, i2), one, wv.Off(0, 1+i2-1)); err != nil {
							panic(err)
						}
