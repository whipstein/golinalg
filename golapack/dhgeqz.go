...
func Dhgeqz(job, compq, compz byte, n, ilo, ihi int, h, t *mat.Matrix, alphar, alphai, beta *mat.Vector, q, z *mat.Matrix, work *mat.Vector, lwork int) (info int, err error) {
		...
		for j = ilast - 1; j >= ilo; j-- {
			...
			if math.Abs(t.Get(j-1, j-1)) < btol {
				...
				if !ilazro {
					...
					if temp*(ascale*math.Abs(h.Get(j, j-1))) <= temp2*(ascale*atol) {
						ilazr2 = true
					}
				}
				...
				if ilazro || ilazr2 {
					for jch = j; jch <= ilast-1; jch++ {
						...
						h.Off(jch, jch).Vector().Rot(ilastm-jch, h.Off(jch-1, jch).Vector(), h.Rows, h.Rows, c, s)
						t.Off(jch, jch).Vector().Rot(ilastm-jch, t.Off(jch-1, jch).Vector(), t.Rows, t.Rows, c, s)
						if ilq {
							q.Off(0, jch).Vector().Rot(n, q.Off(0, jch-1).Vector(), 1, 1, c, s)
						}
						if ilazr2 {
							h.Set(jch-1, jch-1-1, h.Get(jch-1, jch-1-1)*c)
						}
						ilazr2 = false
						if math.Abs(t.Get(jch, jch)) >= btol {
							if jch+1 >= ilast {
								goto label80
							} else {
								ifirst = jch + 1
					...
					for jch = j; jch <= ilast-1; jch++ {
						...
						t.Set(jch, jch, zero)
						if jch < ilastm-1 {
							t.Off(jch, jch+2-1).Vector().Rot(ilastm-jch-1, t.Off(jch-1, jch+2-1).Vector(), t.Rows, t.Rows, c, s)
						}
						h.Off(jch, jch-1-1).Vector().Rot(ilastm-jch+2, h.Off(jch-1, jch-1-1).Vector(), h.Rows, h.Rows, c, s)
						if ilq {
							q.Off(0, jch).Vector().Rot(n, q.Off(0, jch-1).Vector(), 1, 1, c, s)
						}
						...
						h.Off(ifrstm-1, jch-1-1).Vector().Rot(jch+1-ifrstm, h.Off(ifrstm-1, jch-1).Vector(), 1, 1, c, s)
						t.Off(ifrstm-1, jch-1-1).Vector().Rot(jch-ifrstm, t.Off(ifrstm-1, jch-1).Vector(), 1, 1, c, s)
						if ilz {
							z.Off(0, jch-1-1).Vector().Rot(n, z.Off(0, jch-1).Vector(), 1, 1, c, s)
						}
					}
					goto label70
				}
	...
	label70:
		...
		h.Set(ilast-1, ilast-1-1, zero)
		h.Off(ifrstm-1, ilast-1-1).Vector().Rot(ilast-ifrstm, h.Off(ifrstm-1, ilast-1).Vector(), 1, 1, c, s)
		t.Off(ifrstm-1, ilast-1-1).Vector().Rot(ilast-ifrstm, t.Off(ifrstm-1, ilast-1).Vector(), 1, 1, c, s)
		if ilz {
			z.Off(0, ilast-1-1).Vector().Rot(n, z.Off(0, ilast-1).Vector(), 1, 1, c, s)
		}
	...
	label110:
		...
		if (iiter/10)*10 == iiter {
			...
			temp = math.Max(s1, safmin*math.Max(one, math.Max(math.Abs(wr), math.Abs(wi))))
			if wi != zero {
				goto label200
			}
		}
		...
		for j = ilast - 1; j >= ifirst+1; j-- {
			...
			if math.Abs((ascale*h.Get(j, j-1))*temp) <= (ascale*atol)*temp2 {
				goto label130
			}
		}
	...
	label130:
		...
		for j = istart; j <= ilast-1; j++ {
			...
			for jc = j; jc <= ilastm; jc++ {
				temp = c*h.Get(j-1, jc-1) + s*h.Get(j, jc-1)
				h.Set(j, jc-1, -s*h.Get(j-1, jc-1)+c*h.Get(j, jc-1))
				h.Set(j-1, jc-1, temp)
				temp2 = c*t.Get(j-1, jc-1) + s*t.Get(j, jc-1)
				t.Set(j, jc-1, -s*t.Get(j-1, jc-1)+c*t.Get(j, jc-1))
			...
			for jr = ifrstm; jr <= min(j+2, ilast); jr++ {
				temp = c*h.Get(jr-1, j) + s*h.Get(jr-1, j-1)
				h.Set(jr-1, j-1, -s*h.Get(jr-1, j)+c*h.Get(jr-1, j-1))
				h.Set(jr-1, j, temp)
			}
			for jr = ifrstm; jr <= j; jr++ {
				temp = c*t.Get(jr-1, j) + s*t.Get(jr-1, j-1)
				t.Set(jr-1, j-1, -s*t.Get(jr-1, j)+c*t.Get(jr-1, j-1))
				t.Set(jr-1, j, temp)
			}
			if ilz {
				for jr = 1; jr <= n; jr++ {
	...
	label200:
		...
		if ifirst+1 == ilast {
			...
			h.Off(ilast-1, ilast-1-1).Vector().Rot(ilastm+1-ifirst, h.Off(ilast-1-1, ilast-1-1).Vector(), h.Rows, h.Rows, cl, sl)
			h.Off(ifrstm-1, ilast-1).Vector().Rot(ilast+1-ifrstm, h.Off(ifrstm-1, ilast-1-1).Vector(), 1, 1, cr, sr)

			if ilast < ilastm {
				t.Off(ilast-1, ilast).Vector().Rot(ilastm-ilast, t.Off(ilast-1-1, ilast).Vector(), t.Rows, t.Rows, cl, sl)
			}
			if ifrstm < ilast-1 {
				t.Off(ifrstm-1, ilast-1).Vector().Rot(ifirst-ifrstm, t.Off(ifrstm-1, ilast-1-1).Vector(), 1, 1, cr, sr)
			}

			if ilq {
				q.Off(0, ilast-1).Vector().Rot(n, q.Off(0, ilast-1-1).Vector(), 1, 1, cl, sl)
			}
			if ilz {
				z.Off(0, ilast-1).Vector().Rot(n, z.Off(0, ilast-1-1).Vector(), 1, 1, cr, sr)
			}
			...
			ad21l = (ascale * h.Get(ifirst, ifirst-1)) / (bscale * t.Get(ifirst-1, ifirst-1))
			ad12l = (ascale * h.Get(ifirst-1, ifirst)) / (bscale * t.Get(ifirst, ifirst))
			ad22l = (ascale * h.Get(ifirst, ifirst)) / (bscale * t.Get(ifirst, ifirst))
			ad32l = (ascale * h.Get(ifirst+2-1, ifirst)) / (bscale * t.Get(ifirst, ifirst))
			u12l = t.Get(ifirst-1, ifirst) / t.Get(ifirst, ifirst)
			...
			*v.GetPtr(0), tau = Dlarfg(3, v.Get(0), v.Off(1), 1)
			...
			for j = istart; j <= ilast-2; j++ {
				...
				for jc = j; jc <= ilastm; jc++ {
					temp = tau * (h.Get(j-1, jc-1) + v.Get(1)*h.Get(j, jc-1) + v.Get(2)*h.Get(j+2-1, jc-1))
					h.Set(j-1, jc-1, h.Get(j-1, jc-1)-temp)
					h.Set(j, jc-1, h.Get(j, jc-1)-temp*v.Get(1))
					h.Set(j+2-1, jc-1, h.Get(j+2-1, jc-1)-temp*v.Get(2))
					temp2 = tau * (t.Get(j-1, jc-1) + v.Get(1)*t.Get(j, jc-1) + v.Get(2)*t.Get(j+2-1, jc-1))
					t.Set(j-1, jc-1, t.Get(j-1, jc-1)-temp2)
					t.Set(j, jc-1, t.Get(j, jc-1)-temp2*v.Get(1))
				...
				//              Swap rows to pivot
				ilpivt = false
				temp = math.Max(math.Abs(t.Get(j, j)), math.Abs(t.Get(j, j+2-1)))
				temp2 = math.Max(math.Abs(t.Get(j+2-1, j)), math.Abs(t.Get(j+2-1, j+2-1)))
			...
			label250:
				...
				for jr = ifrstm; jr <= min(j+3, ilast); jr++ {
					temp = tau * (h.Get(jr-1, j-1) + v.Get(1)*h.Get(jr-1, j) + v.Get(2)*h.Get(jr-1, j+2-1))
					h.Set(jr-1, j-1, h.Get(jr-1, j-1)-temp)
					h.Set(jr-1, j, h.Get(jr-1, j)-temp*v.Get(1))
					h.Set(jr-1, j+2-1, h.Get(jr-1, j+2-1)-temp*v.Get(2))
				}
				for jr = ifrstm; jr <= j+2; jr++ {
					temp = tau * (t.Get(jr-1, j-1) + v.Get(1)*t.Get(jr-1, j) + v.Get(2)*t.Get(jr-1, j+2-1))
					t.Set(jr-1, j-1, t.Get(jr-1, j-1)-temp)
					t.Set(jr-1, j, t.Get(jr-1, j)-temp*v.Get(1))
