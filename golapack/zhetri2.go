...
func Zhetri2(uplo mat.MatUplo, n int, a *mat.CMatrix, ipiv *[]int, work *mat.CVector, lwork int) (info int, err error) {
	...
	lquery = (lwork == -1)
	//     Get blocksize
	nbmax = Ilaenv(1, "Zhetrf", []byte{uplo.Byte()}, n, -1, -1, -1)
	if nbmax >= n {
		minsize = n
	} else {
	...
	if nbmax >= n {
		if info, err = Zhetri(uplo, n, a, ipiv, work); err != nil {
			panic(err)
		}
	} else {
		if info, err = Zhetri2x(uplo, n, a, ipiv, work.CMatrix(nbmax, opts), nbmax); err != nil {
			panic(err)
		}
	}
