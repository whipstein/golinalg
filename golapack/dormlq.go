...
// if SIDE = 'R'.
func Dormlq(side mat.MatSide, trans mat.MatTrans, m, n, k int, a *mat.Matrix, tau *mat.Vector, c *mat.Matrix, work *mat.Vector, lwork int) (err error) {
	...
	if err == nil {
		...
		nb = min(nbmax, Ilaenv(1, "Dormlq", []byte{side.Byte(), trans.Byte()}, m, n, k, -1))
		lwkopt = max(1, nw)*nb + tsize
		work.Set(0, float64(lwkopt))
	}

	if err != nil {
		gltest.Xerbla2("Dormlq", err)
		return
	} else if lquery {
		return
	}
	...
	if nb > 1 && nb < k {
		if lwork < nw*nb+tsize {
			...
			nbmin = max(2, Ilaenv(2, "Dormlq", []byte{side.Byte(), trans.Byte()}, m, n, k, -1))
		}
	}

	if nb < nbmin || nb >= k {
		//        Use unblocked code
		if err = Dorml2(side, trans, m, n, k, a, tau, c, work); err != nil {
			panic(err)
		}
	} else {
		//        Use blocked code
