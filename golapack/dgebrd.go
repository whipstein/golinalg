...
func Dgebrd(m, n *int, a *mat.Matrix, lda *int, d, e, tauq, taup, work *mat.Vector, lwork, info *int) {
	...
	(*info) = 0
	nb = maxint(1, Ilaenv(func() *int { y := 1; return &y }(), []byte("DGEBRD"), []byte{' '}, m, n, toPtr(-1), toPtr(-1)))
	lwkopt = ((*m) + (*n)) * nb
	...
	if nb > 1 && nb < minmn {
		...
		nx = maxint(nb, Ilaenv(func() *int { y := 3; return &y }(), []byte("DGEBRD"), []byte{' '}, m, n, toPtr(-1), toPtr(-1)))
	...
	for i = 1; i <= minmn-nx; i += nb {
		...
		//        part of the matrix
		Dlabrd(toPtr((*m)-i+1), toPtr((*n)-i+1), &nb, a.Off(i-1, i-1), lda, d.Off(i-1), e.Off(i-1), tauq.Off(i-1), taup.Off(i-1), work.Matrix(ldwrkx, opts), &ldwrkx, work.MatrixOff(ldwrkx*nb+1-1, ldwrky, opts), &ldwrky)
