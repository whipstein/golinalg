...
func Zgeqrfp(m, n int, a *mat.CMatrix, tau, work *mat.CVector, lwork int) (err error) {
	var lquery bool
	...
	if nb >= nbmin && nb < k && nx < k {
		...
		for i = 1; i <= k-nx; i += nb {
			...
			//           A(i:m,i:i+ib-1)
			if err = Zgeqr2p(m-i+1, ib, a.Off(i-1, i-1), tau.Off(i-1), work); err != nil {
				panic(err)
			}
			if i+ib <= n {
	...
	if i <= k {
		if err = Zgeqr2p(m-i+1, n-i+1, a.Off(i-1, i-1), tau.Off(i-1), work); err != nil {
			panic(err)
		}
	}
