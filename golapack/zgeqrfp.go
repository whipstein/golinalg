...
func Zgeqrfp(m, n *int, a *mat.CMatrix, lda *int, tau, work *mat.CVector, lwork, info *int) {
	var lquery bool
	...
	if (*info) != 0 {
		gltest.Xerbla([]byte("ZGEQRFP"), -(*info))
		return
	} else if lquery {
		return
	}
	...
	if nb >= nbmin && nb < k && nx < k {
		...
		for i = 1; i <= k-nx; i += nb {
			...
			//           A(i:m,i:i+ib-1)
			Zgeqr2p(toPtr((*m)-i+1), &ib, a.Off(i-1, i-1), lda, tau.Off(i-1), work, &iinfo)
			if i+ib <= (*n) {
	...
	if i <= k {
		Zgeqr2p(toPtr((*m)-i+1), toPtr((*n)-i+1), a.Off(i-1, i-1), lda, tau.Off(i-1), work, &iinfo)
	}
