...
func Zheevx(jobz, _range, uplo byte, n *int, a *mat.CMatrix, lda *int, vl, vu *float64, il, iu *int, abstol *float64, m *int, w *mat.Vector, z *mat.CMatrix, ldz *int, work *mat.CVector, lwork *int, rwork *mat.Vector, iwork, ifail *[]int, info *int) {
	...
	var i, iinfo, imax, indd, inde, indee, indibl, indisp, indiwk, indrwk, indtau, indwrk, iscale, itmp1, j, jj, llwork, lwkmin, lwkopt, nb, nsplit int
	...
	if (*info) != 0 {
		gltest.Xerbla([]byte("ZHEEVX"), -(*info))
		return
	} else if lquery {
		return
	}
	...
	Zhetrd(uplo, n, a, lda, rwork.Off(indd-1), rwork.Off(inde-1), work.Off(indtau-1), work.Off(indwrk-1), &llwork, &iinfo)
