...
func DsytrfAa(uplo byte, n *int, a *mat.Matrix, lda *int, ipiv *[]int, work *mat.Vector, lwork, info *int) {
	...
	if upper {
		...
		goblas.Dcopy(*n, a.Vector(0, 0, *lda), work.Off(0, 1))
	...
	label10:
		...
		//        Panel factorization
		DlasyfAa(uplo, toPtr(2-k1), toPtr((*n)-j), &jb, a.Off(max(1, j)-1, j), lda, toSlice(ipiv, j), work.Matrix(*n, opts), n, work.Off((*n)*nb))
		...
		if j < (*n) {
			...
			if j1 > 1 || jb > 1 {
				...
				alpha = a.Get(j-1, j)
				a.Set(j-1, j, one)
				goblas.Dcopy((*n)-j, a.Vector(j-1-1, j, *lda), work.Off((j+1-j1+1)+jb*(*n)-1, 1))
				...
				for j2 = j + 1; j2 <= (*n); j2 += nb {
					...
					j3 = j2
					for mj = nj - 1; mj >= 1; mj-- {
						err = goblas.Dgemv(NoTrans, mj, jb+1, -one, work.MatrixOff(j3-j1+1+k1*(*n)-1, *n, opts), a.Vector(j1-k2-1, j3-1, 1), one, a.Vector(j3-1, j3-1, *lda))
						j3 = j3 + 1
					}

					//                 Update off-diagonal block of J2-th block row with DGEMM
					err = goblas.Dgemm(Trans, Trans, nj, (*n)-j3+1, jb+1, -one, a.Off(j1-k2-1, j2-1), work.MatrixOff(j3-j1+1+k1*(*n)-1, *n, opts), one, a.Off(j2-1, j3-1))
				}
			...
			goblas.Dcopy((*n)-j, a.Vector(j, j, *lda), work.Off(0, 1))
		}
		goto label10
	} else {
		//        .....................................................
	...
	label11:
		...
		//        Panel factorization
		DlasyfAa(uplo, toPtr(2-k1), toPtr((*n)-j), &jb, a.Off(j, max(1, j)-1), lda, toSlice(ipiv, j), work.Matrix(*n, opts), n, work.Off((*n)*nb))
		...
		if j < (*n) {
			...
			if j1 > 1 || jb > 1 {
				...
				for j2 = j + 1; j2 <= (*n); j2 += nb {
					...
					j3 = j2
					for mj = nj - 1; mj >= 1; mj-- {
						err = goblas.Dgemv(NoTrans, mj, jb+1, -one, work.MatrixOff(j3-j1+1+k1*(*n)-1, *n, opts), a.Vector(j3-1, j1-k2-1, *lda), one, a.Vector(j3-1, j3-1, 1))
						j3 = j3 + 1
					}

					//                 Update off-diagonal block in J2-th block column with DGEMM
					err = goblas.Dgemm(NoTrans, Trans, (*n)-j3+1, nj, jb+1, -one, work.MatrixOff(j3-j1+1+k1*(*n)-1, *n, opts), a.Off(j2-1, j1-k2-1), one, a.Off(j3-1, j2-1))
				}
