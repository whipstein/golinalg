...
func DsytrfAa(uplo mat.MatUplo, n int, a *mat.Matrix, ipiv *[]int, work *mat.Vector, lwork int) (info int, err error) {
	...
	if upper {
		...
		work.Copy(n, a.Off(0, 0).Vector(), a.Rows, 1)
	...
	label10:
		...
		for j2 = j + 2; j2 <= min(n, j+jb+1); j2++ {
			...
			if (j2 != (*ipiv)[j2-1]) && ((j1 - k1) > 2) {
				a.Off(0, (*ipiv)[j2-1]-1).Vector().Swap(j1-k1-2, a.Off(0, j2-1).Vector(), 1, 1)
			}
		}
		j = j + jb
		...
		if j < n {
			...
			if j1 > 1 || jb > 1 {
				...
				alpha = a.Get(j-1, j)
				a.Set(j-1, j, one)
				work.Off((j+1-j1+1)+jb*n-1).Copy(n-j, a.Off(j-1-1, j).Vector(), a.Rows, 1)
				...
				for j2 = j + 1; j2 <= n; j2 += nb {
					...
					j3 = j2
					for mj = nj - 1; mj >= 1; mj-- {
						if err = a.Off(j3-1, j3-1).Vector().Gemv(NoTrans, mj, jb+1, -one, work.Off(j3-j1+1+k1*n-1).Matrix(n, opts), a.Off(j1-k2-1, j3-1).Vector(), 1, one, a.Rows); err != nil {
							panic(err)
						}
						j3 = j3 + 1
					}
					...
					if err = a.Off(j2-1, j3-1).Gemm(Trans, Trans, nj, n-j3+1, jb+1, -one, a.Off(j1-k2-1, j2-1), work.Off(j3-j1+1+k1*n-1).Matrix(n, opts), one); err != nil {
						panic(err)
					}
				}
			...
			work.Copy(n-j, a.Off(j, j).Vector(), a.Rows, 1)
		}
		goto label10
	} else {
		//        .....................................................
		...
		//         (stored in WORK(1:N))
		work.Copy(n, a.Off(0, 0).Vector(), 1, 1)
	...
	label11:
		...
		for j2 = j + 2; j2 <= min(n, j+jb+1); j2++ {
			(*ipiv)[j2-1] = (*ipiv)[j2-1] + j
			if (j2 != (*ipiv)[j2-1]) && ((j1 - k1) > 2) {
				a.Off((*ipiv)[j2-1]-1, 0).Vector().Swap(j1-k1-2, a.Off(j2-1, 0).Vector(), a.Rows, a.Rows)
			}
		}
		j = j + jb
		...
		if j < n {
			...
			if j1 > 1 || jb > 1 {
				...
				alpha = a.Get(j, j-1)
				a.Set(j, j-1, one)
				work.Off((j+1-j1+1)+jb*n-1).Copy(n-j, a.Off(j, j-1-1).Vector(), 1, 1)
				work.Off((j+1-j1+1)+jb*n-1).Scal(n-j, alpha, 1)
				...
				for j2 = j + 1; j2 <= n; j2 += nb {
					...
					j3 = j2
					for mj = nj - 1; mj >= 1; mj-- {
						if err = a.Off(j3-1, j3-1).Vector().Gemv(NoTrans, mj, jb+1, -one, work.Off(j3-j1+1+k1*n-1).Matrix(n, opts), a.Off(j3-1, j1-k2-1).Vector(), a.Rows, one, 1); err != nil {
							panic(err)
						}
						j3 = j3 + 1
					}
					...
					if err = a.Off(j3-1, j2-1).Gemm(NoTrans, Trans, n-j3+1, nj, jb+1, -one, work.Off(j3-j1+1+k1*n-1).Matrix(n, opts), a.Off(j2-1, j1-k2-1), one); err != nil {
						panic(err)
					}
				}
			...
			//           WORK(J+1, 1) stores H(J+1, 1)
			work.Copy(n-j, a.Off(j, j).Vector(), 1, 1)
		}
		goto label11
	}
}
