...
func Dorgql(m, n, k *int, a *mat.Matrix, lda *int, tau, work *mat.Vector, lwork, info *int) {
	var lquery bool
	var zero float64
	...
	if (*info) == 0 {
		if (*n) == 0 {
			...
			nb = Ilaenv(func() *int { y := 1; return &y }(), []byte("DORGQL"), []byte{' '}, m, n, k, toPtr(-1))
			lwkopt = (*n) * nb
		}
	...
	if (*info) != 0 {
		gltest.Xerbla([]byte("DORGQL"), -(*info))
		return
	} else if lquery {
		return
	}
	...
	Dorg2l(toPtr((*m)-kk), toPtr((*n)-kk), toPtr((*k)-kk), a, lda, tau, work, &iinfo)
	...
	if kk > 0 {
		...
		for _, i = range genIter((*k)-kk+1, (*k), nb) {
			...
			Dorg2l(toPtr((*m)-(*k)+i+ib-1), &ib, &ib, a.Off(0, (*n)-(*k)+i-1), lda, tau.Off(i-1), work, &iinfo)
