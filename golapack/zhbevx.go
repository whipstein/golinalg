...
func Zhbevx(jobz, _range, uplo byte, n, kd *int, ab *mat.CMatrix, ldab *int, q *mat.CMatrix, ldq *int, vl, vu *float64, il, iu *int, abstol *float64, m *int, w *mat.Vector, z *mat.CMatrix, ldz *int, work *mat.CVector, rwork *mat.Vector, iwork, ifail *[]int, info *int) {
	...
	var i, iinfo, imax, indd, inde, indee, indibl, indisp, indiwk, indrwk, indwrk, iscale, itmp1, j, jj, nsplit int
	...
	if (*info) != 0 {
		gltest.Xerbla([]byte("ZHBEVX"), -(*info))
		return
	}
	...
	indrwk = inde + (*n)
	indwrk = 1
	Zhbtrd(jobz, uplo, n, kd, ab, ldab, rwork.Off(indd-1), rwork.Off(inde-1), q, ldq, work.Off(indwrk-1), &iinfo)
	...
	if wantz {
		...
		for j = 1; j <= (*m); j++ {
			...
			goblas.Zgemv(NoTrans, n, n, &cone, q, ldq, work, func() *int { y := 1; return &y }(), &czero, z.CVector(0, j-1), func() *int { y := 1; return &y }())
		}
	}
