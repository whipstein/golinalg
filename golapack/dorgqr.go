...
func Dorgqr(m, n, k *int, a *mat.Matrix, lda *int, tau, work *mat.Vector, lwork, info *int) {
	var lquery bool
	var zero float64
	...
	if (*info) != 0 {
		gltest.Xerbla([]byte("DORGQR"), -(*info))
		return
	} else if lquery {
		return
	}
	...
	if kk > 0 {
		...
		for i = ki + 1; i >= 1; i -= nb {
			...
			Dorg2r(toPtr((*m)-i+1), &ib, &ib, a.Off(i-1, i-1), lda, tau.Off(i-1), work, &iinfo)
