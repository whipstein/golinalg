...
func Zsteqr(compz byte, n *int, d, e *mat.Vector, z *mat.CMatrix, ldz *int, work *mat.Vector, info *int) {
	...
	if (*info) != 0 {
		gltest.Xerbla([]byte("ZSTEQR"), -(*info))
		return
	}
...
label30:
	...
	anorm = Dlanst('I', toPtr(lend-l+1), d.Off(l-1), e.Off(l-1))
	iscale = 0
	if anorm == zero {
		goto label10
	}
	...
	label60:
		...
		if icompz > 0 {
			mm = m - l + 1
			Zlasr('R', 'V', 'B', n, &mm, work.Off(l-1), work.Off((*n)-1+l-1), z.Off(0, l-1), ldz)
		}
	...
	label110:
		...
		if m == l-1 {
			if icompz > 0 {
				...
				work.Set(m-1, c)
				work.Set((*n)-1+m-1, s)
				Zlasr('R', 'V', 'F', n, func() *int { y := 2; return &y }(), work.Off(m-1), work.Off((*n)-1+m-1), z.Off(0, l-1-1), ldz)
			} else {
		...
		if icompz > 0 {
			mm = l - m + 1
			Zlasr('R', 'V', 'F', n, &mm, work.Off(m-1), work.Off((*n)-1+m-1), z.Off(0, m-1), ldz)
		}
