...
func Dgeqrfp(m, n int, a *mat.Matrix, tau, work *mat.Vector, lwork int) (err error) {
	var lquery bool
	...
	if nb > 1 && nb < k {
		...
		nx = max(0, Ilaenv(3, "Dgeqrf", []byte{' '}, m, n, -1, -1))
		if nx < k {
			...
			if lwork < iws {
				...
				nb = lwork / ldwork
				nbmin = max(2, Ilaenv(2, "Dgeqrf", []byte{' '}, m, n, -1, -1))
			}
		}
	}
	...
	if nb >= nbmin && nb < k && nx < k {
		...
		for i = 1; i <= k-nx; i += nb {
			...
			//           A(i:m,i:i+ib-1)
			if err = Dgeqr2p(m-i+1, ib, a.Off(i-1, i-1), tau.Off(i-1), work); err != nil {
				panic(err)
			}
			if i+ib <= n {
	...
	if i <= k {
		if err = Dgeqr2p(m-i+1, n-i+1, a.Off(i-1, i-1), tau.Off(i-1), work); err != nil {
			panic(err)
		}
	}
