...
func Ztrrfs(uplo mat.MatUplo, trans mat.MatTrans, diag mat.MatDiag, n, nrhs int, a, b, x *mat.CMatrix, ferr, berr *mat.Vector, work *mat.CVector, rwork *mat.Vector) (err error) {
		...
		*ferr.GetPtr(j - 1), kase = Zlacn2(n, work.Off(n), work, ferr.Get(j-1), kase, &isave)
		if kase != 0 {
			if kase == 1 {
				...
				if err = goblas.Ztrsv(uplo, transt, diag, n, a, work.Off(0, 1)); err != nil {
					panic(err)
				}
				for i = 1; i <= n; i++ {
				...
				if err = goblas.Ztrsv(uplo, transn, diag, n, a, work.Off(0, 1)); err != nil {
					panic(err)
				}
			}
			goto label210
