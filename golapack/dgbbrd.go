...
func Dgbbrd(vect byte, m, n, ncc, kl, ku *int, ab *mat.Matrix, ldab *int, d, e *mat.Vector, q *mat.Matrix, ldq *int, pt *mat.Matrix, ldpt *int, c *mat.Matrix, ldc *int, work *mat.Vector, info *int) {
		...
		for i = 1; i <= minmn; i++ {
			...
			for kk = 1; kk <= kb; kk++ {
				...
				for l = 1; l <= kb; l++ {
					...
					if nrt > 0 {
						Dlartv(&nrt, ab.Vector(klu1-l-1, j1-klm+l-1-1), &inca, ab.Vector(klu1-l+1-1, j1-klm+l-1-1), &inca, work.Off(mn+j1-1), work.Off(j1-1), &kb1)
					}
				}
				...
				if ml > ml0 {
					if ml <= (*m)-i+1 {
						...
						if i < (*n) {
							goblas.Drot(toPtr(minint((*ku)+ml-2, (*n)-i)), ab.Vector((*ku)+ml-2-1, i+1-1), toPtr((*ldab)-1), ab.Vector((*ku)+ml-1-1, i+1-1), toPtr((*ldab)-1), work.GetPtr(mn+i+ml-1-1), work.GetPtr(i+ml-1-1))
						}
					}
					nr = nr + 1
					j1 = j1 - kb1
				...
				if wantq {
					...
					for j = j1; j <= j2; j += kb1 {
						goblas.Drot(m, q.Vector(0, j-1-1), func() *int { y := 1; return &y }(), q.Vector(0, j-1), func() *int { y := 1; return &y }(), work.GetPtr(mn+j-1), work.GetPtr(j-1))
					}
				}
				...
				if wantc {
					...
					for j = j1; j <= j2; j += kb1 {
						goblas.Drot(ncc, c.Vector(j-1-1, 0), ldc, c.Vector(j-1, 0), ldc, work.GetPtr(mn+j-1), work.GetPtr(j-1))
					}
				}
				...
				for l = 1; l <= kb; l++ {
					...
					if nrt > 0 {
						Dlartv(&nrt, ab.Vector(l+1-1, j1+kun-1-1), &inca, ab.Vector(l-1, j1+kun-1), &inca, work.Off(mn+j1+kun-1), work.Off(j1+kun-1), &kb1)
					}
				}
				...
				if ml == ml0 && mu > mu0 {
					if mu <= (*n)-i+1 {
						...
						goblas.Drot(toPtr(minint((*kl)+mu-2, (*m)-i)), ab.Vector((*ku)-mu+4-1, i+mu-2-1), func() *int { y := 1; return &y }(), ab.Vector((*ku)-mu+3-1, i+mu-1-1), func() *int { y := 1; return &y }(), work.GetPtr(mn+i+mu-1-1), work.GetPtr(i+mu-1-1))
					}
					nr = nr + 1
					j1 = j1 - kb1
				}
				...
				if wantpt {
					...
					for j = j1; j <= j2; j += kb1 {
						goblas.Drot(n, pt.Vector(j+kun-1-1, 0), ldpt, pt.Vector(j+kun-1, 0), ldpt, work.GetPtr(mn+j+kun-1), work.GetPtr(j+kun-1))
					}
				}
	...
	if (*ku) == 0 && (*kl) > 0 {
		...
		for i = 1; i <= minint((*m)-1, *n); i++ {
			...
			d.Set(i-1, ra)
			if i < (*n) {
				e.Set(i-1, rs*ab.Get(0, i+1-1))
				ab.Set(0, i+1-1, rc*ab.Get(0, i+1-1))
			}
			if wantq {
				goblas.Drot(m, q.Vector(0, i-1), func() *int { y := 1; return &y }(), q.Vector(0, i+1-1), func() *int { y := 1; return &y }(), &rc, &rs)
			}
			if wantc {
				goblas.Drot(ncc, c.Vector(i-1, 0), ldc, c.Vector(i+1-1, 0), ldc, &rc, &rs)
			}
		}
		if (*m) <= (*n) {
		...
		if (*m) < (*n) {
			...
			for i = (*m); i >= 1; i-- {
				Dlartg(ab.GetPtr((*ku)+1-1, i-1), &rb, &rc, &rs, &ra)
				d.Set(i-1, ra)
				if i > 1 {
				...
				if wantpt {
					goblas.Drot(n, pt.Vector(i-1, 0), ldpt, pt.Vector((*m)+1-1, 0), ldpt, &rc, &rs)
				}
			}
		} else {
			//           Copy off-diagonal elements to E and diagonal elements to D
			for i = 1; i <= minmn-1; i++ {
				e.Set(i-1, ab.Get((*ku)-1, i+1-1))
			}
			for i = 1; i <= minmn; i++ {
				d.Set(i-1, ab.Get((*ku)+1-1, i-1))
			}
		}
	} else {
