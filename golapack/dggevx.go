...
func Dggevx(balanc, jobvl, jobvr, sense byte, n *int, a *mat.Matrix, lda *int, b *mat.Matrix, ldb *int, alphar, alphai, beta *mat.Vector, vl *mat.Matrix, ldvl *int, vr *mat.Matrix, ldvr, ilo, ihi *int, lscale, rscale *mat.Vector, abnrm, bbnrm *float64, rconde, rcondv, work *mat.Vector, lwork *int, iwork *[]int, bwork *[]bool, info *int) {
	...
	if (*info) == 0 {
		if (*n) == 0 {
			...
			if wantsv || wantsb {
				minwrk = maxint(minwrk, 2*(*n)*((*n)+4)+16)
			}
			maxwrk = minwrk
			maxwrk = maxint(maxwrk, (*n)+(*n)*Ilaenv(func() *int { y := 1; return &y }(), []byte("DGEQRF"), []byte{' '}, n, func() *int { y := 1; return &y }(), n, func() *int { y := 0; return &y }()))
			maxwrk = maxint(maxwrk, (*n)+(*n)*Ilaenv(func() *int { y := 1; return &y }(), []byte("DORMQR"), []byte{' '}, n, func() *int { y := 1; return &y }(), n, func() *int { y := 0; return &y }()))
			if ilvl {
				maxwrk = maxint(maxwrk, (*n)+(*n)*Ilaenv(func() *int { y := 1; return &y }(), []byte("DORGQR"), []byte{' '}, n, func() *int { y := 1; return &y }(), n, func() *int { y := 0; return &y }()))
			}
		}
