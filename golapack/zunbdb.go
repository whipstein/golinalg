...
func Zunbdb(trans, signs byte, m, p, q *int, x11 *mat.CMatrix, ldx11 *int, x12 *mat.CMatrix, ldx12 *int, x21 *mat.CMatrix, ldx21 *int, x22 *mat.CMatrix, ldx22 *int, theta, phi *mat.Vector, taup1, taup2, tauq1, tauq2, work *mat.CVector, lwork, info *int) {
	...
	if colmajor {
		...
		for i = 1; i <= (*q); i++ {
			...
			if (*p) > i {
				Zlarfgp(toPtr((*p)-i+1), x11.GetPtr(i-1, i-1), x11.CVector(i, i-1), func() *int { y := 1; return &y }(), taup1.GetPtr(i-1))
			} else if (*p) == i {
				Zlarfgp(toPtr((*p)-i+1), x11.GetPtr(i-1, i-1), x11.CVector(i-1, i-1), func() *int { y := 1; return &y }(), taup1.GetPtr(i-1))
			}
			x11.Set(i-1, i-1, one)
			if (*m)-(*p) > i {
				Zlarfgp(toPtr((*m)-(*p)-i+1), x21.GetPtr(i-1, i-1), x21.CVector(i, i-1), func() *int { y := 1; return &y }(), taup2.GetPtr(i-1))
			} else if (*m)-(*p) == i {
			...
			if (*q) > i {
				Zlarf('L', toPtr((*p)-i+1), toPtr((*q)-i), x11.CVector(i-1, i-1), func() *int { y := 1; return &y }(), toPtrc128(taup1.GetConj(i-1)), x11.Off(i-1, i), ldx11, work)
				Zlarf('L', toPtr((*m)-(*p)-i+1), toPtr((*q)-i), x21.CVector(i-1, i-1), func() *int { y := 1; return &y }(), toPtrc128(taup2.GetConj(i-1)), x21.Off(i-1, i), ldx21, work)
			}
			if (*m)-(*q)+1 > i {
			...
			if i < (*q) {
				goblas.Zscal((*q)-i, complex(-z1*z3*math.Sin(theta.Get(i-1)), 0), x11.CVector(i-1, i, *ldx11))
				goblas.Zaxpy((*q)-i, complex(z2*z3*math.Cos(theta.Get(i-1)), 0), x21.CVector(i-1, i, *ldx21), x11.CVector(i-1, i, *ldx11))
			}
			...
			if i < (*q) {
				phi.Set(i-1, math.Atan2(goblas.Dznrm2((*q)-i, x11.CVector(i-1, i, *ldx11)), goblas.Dznrm2((*m)-(*q)-i+1, x12.CVector(i-1, i-1, *ldx12))))
			}

			if i < (*q) {
				Zlacgv(toPtr((*q)-i), x11.CVector(i-1, i), ldx11)
				if i == (*q)-1 {
					Zlarfgp(toPtr((*q)-i), x11.GetPtr(i-1, i), x11.CVector(i-1, i), ldx11, tauq1.GetPtr(i-1))
				} else {
					Zlarfgp(toPtr((*q)-i), x11.GetPtr(i-1, i), x11.CVector(i-1, i+2-1), ldx11, tauq1.GetPtr(i-1))
				}
				x11.Set(i-1, i, one)
			}
			if (*m)-(*q)+1 > i {
				...
				if (*m)-(*q) == i {
					...
					Zlarfgp(toPtr((*m)-(*q)-i+1), x12.GetPtr(i-1, i-1), x12.CVector(i-1, i), ldx12, tauq2.GetPtr(i-1))
				}
			}
			x12.Set(i-1, i-1, one)

			if i < (*q) {
				Zlarf('R', toPtr((*p)-i), toPtr((*q)-i), x11.CVector(i-1, i), ldx11, tauq1.GetPtr(i-1), x11.Off(i, i), ldx11, work)
				Zlarf('R', toPtr((*m)-(*p)-i), toPtr((*q)-i), x11.CVector(i-1, i), ldx11, tauq1.GetPtr(i-1), x21.Off(i, i), ldx21, work)
			}
			if (*p) > i {
				Zlarf('R', toPtr((*p)-i), toPtr((*m)-(*q)-i+1), x12.CVector(i-1, i-1), ldx12, tauq2.GetPtr(i-1), x12.Off(i, i-1), ldx12, work)
			}
			if (*m)-(*p) > i {
				Zlarf('R', toPtr((*m)-(*p)-i), toPtr((*m)-(*q)-i+1), x12.CVector(i-1, i-1), ldx12, tauq2.GetPtr(i-1), x22.Off(i, i-1), ldx22, work)
			}
		...
		for i = (*q) + 1; i <= (*p); i++ {
			...
			if (*m)-(*p)-(*q) >= 1 {
				Zlarf('R', toPtr((*m)-(*p)-(*q)), toPtr((*m)-(*q)-i+1), x12.CVector(i-1, i-1), ldx12, tauq2.GetPtr(i-1), x22.Off((*q), i-1), ldx22, work)
			}
		...
		for i = 1; i <= (*m)-(*p)-(*q); i++ {
			...
			Zlarfgp(toPtr((*m)-(*p)-(*q)-i+1), x22.GetPtr((*q)+i-1, (*p)+i-1), x22.CVector((*q)+i-1, (*p)+i), ldx22, tauq2.GetPtr((*p)+i-1))
			x22.Set((*q)+i-1, (*p)+i-1, one)
			Zlarf('R', toPtr((*m)-(*p)-(*q)-i), toPtr((*m)-(*p)-(*q)-i+1), x22.CVector((*q)+i-1, (*p)+i-1), ldx22, tauq2.GetPtr((*p)+i-1), x22.Off((*q)+i, (*p)+i-1), ldx22, work)
		...
		for i = 1; i <= (*q); i++ {
			...
			Zlarfgp(toPtr((*p)-i+1), x11.GetPtr(i-1, i-1), x11.CVector(i-1, i), ldx11, taup1.GetPtr(i-1))
			x11.Set(i-1, i-1, one)
			if i == (*m)-(*p) {
				Zlarfgp(toPtr((*m)-(*p)-i+1), x21.GetPtr(i-1, i-1), x21.CVector(i-1, i-1), ldx21, taup2.GetPtr(i-1))
			} else {
				Zlarfgp(toPtr((*m)-(*p)-i+1), x21.GetPtr(i-1, i-1), x21.CVector(i-1, i), ldx21, taup2.GetPtr(i-1))
			}
			x21.Set(i-1, i-1, one)

			Zlarf('R', toPtr((*q)-i), toPtr((*p)-i+1), x11.CVector(i-1, i-1), ldx11, taup1.GetPtr(i-1), x11.Off(i, i-1), ldx11, work)
			Zlarf('R', toPtr((*m)-(*q)-i+1), toPtr((*p)-i+1), x11.CVector(i-1, i-1), ldx11, taup1.GetPtr(i-1), x12.Off(i-1, i-1), ldx12, work)
			Zlarf('R', toPtr((*q)-i), toPtr((*m)-(*p)-i+1), x21.CVector(i-1, i-1), ldx21, taup2.GetPtr(i-1), x21.Off(i, i-1), ldx21, work)
			...
			if i < (*q) {
				Zlarfgp(toPtr((*q)-i), x11.GetPtr(i, i-1), x11.CVector(i+2-1, i-1), func() *int { y := 1; return &y }(), tauq1.GetPtr(i-1))
				x11.Set(i, i-1, one)
			}
			Zlarfgp(toPtr((*m)-(*q)-i+1), x12.GetPtr(i-1, i-1), x12.CVector(i, i-1), func() *int { y := 1; return &y }(), tauq2.GetPtr(i-1))
			x12.Set(i-1, i-1, one)

			if i < (*q) {
				Zlarf('L', toPtr((*q)-i), toPtr((*p)-i), x11.CVector(i, i-1), func() *int { y := 1; return &y }(), toPtrc128(tauq1.GetConj(i-1)), x11.Off(i, i), ldx11, work)
				Zlarf('L', toPtr((*q)-i), toPtr((*m)-(*p)-i), x11.CVector(i, i-1), func() *int { y := 1; return &y }(), toPtrc128(tauq1.GetConj(i-1)), x21.Off(i, i), ldx21, work)
			}
			Zlarf('L', toPtr((*m)-(*q)-i+1), toPtr((*p)-i), x12.CVector(i-1, i-1), func() *int { y := 1; return &y }(), toPtrc128(tauq2.GetConj(i-1)), x12.Off(i-1, i), ldx12, work)
			if (*m)-(*p) > i {
				Zlarf('L', toPtr((*m)-(*q)-i+1), toPtr((*m)-(*p)-i), x12.CVector(i-1, i-1), func() *int { y := 1; return &y }(), toPtrc128(tauq2.GetConj(i-1)), x22.Off(i-1, i), ldx22, work)
			}
		...
		for i = (*q) + 1; i <= (*p); i++ {
			...
			if (*m)-(*p)-(*q) >= 1 {
				Zlarf('L', toPtr((*m)-(*q)-i+1), toPtr((*m)-(*p)-(*q)), x12.CVector(i-1, i-1), func() *int { y := 1; return &y }(), toPtrc128(tauq2.GetConj(i-1)), x22.Off(i-1, (*q)), ldx22, work)
			}
		...
		for i = 1; i <= (*m)-(*p)-(*q); i++ {
			...
			Zlarfgp(toPtr((*m)-(*p)-(*q)-i+1), x22.GetPtr((*p)+i-1, (*q)+i-1), x22.CVector((*p)+i, (*q)+i-1), func() *int { y := 1; return &y }(), tauq2.GetPtr((*p)+i-1))
			x22.Set((*p)+i-1, (*q)+i-1, one)

			if (*m)-(*p)-(*q) != i {
				Zlarf('L', toPtr((*m)-(*p)-(*q)-i+1), toPtr((*m)-(*p)-(*q)-i), x22.CVector((*p)+i-1, (*q)+i-1), func() *int { y := 1; return &y }(), toPtrc128(tauq2.GetConj((*p)+i-1)), x22.Off((*p)+i-1, (*q)+i), ldx22, work)
			}
