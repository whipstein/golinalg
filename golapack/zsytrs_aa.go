...
func Zsytrsaa(uplo byte, n, nrhs *int, a *mat.CMatrix, lda *int, ipiv *[]int, b *mat.CMatrix, ldb *int, work *mat.CVector, lwork, info *int) {
	...
	if upper {
		...
		if (*n) > 1 {
			...
			err = goblas.Ztrsm(Left, Upper, Trans, Unit, (*n)-1, *nrhs, one, a.Off(0, 1), *lda, b.Off(1, 0), *ldb)
		}
		//
		...
		if (*n) > 1 {
			...
			err = goblas.Ztrsm(Left, Lower, Trans, Unit, (*n)-1, *nrhs, one, a.Off(1, 0), *lda, b.Off(1, 0), *ldb)
