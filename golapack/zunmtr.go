...
func Zunmtr(side mat.MatSide, uplo mat.MatUplo, trans mat.MatTrans, m, n int, a *mat.CMatrix, tau *mat.CVector, c *mat.CMatrix, work *mat.CVector, lwork int) (err error) {
	...
	if err == nil {
		if upper {
			if left {
				nb = Ilaenv(1, "Zunmql", []byte{side.Byte(), trans.Byte()}, m-1, n, m-1, -1)
			} else {
				nb = Ilaenv(1, "Zunmql", []byte{side.Byte(), trans.Byte()}, m, n-1, n-1, -1)
			}
		} else {
			if left {
	...
	if upper {
		...
		if err = Zunmql(side, trans, mi, ni, nq-1, a.Off(0, 1), tau, c, work, lwork); err != nil {
			panic(err)
		}
	} else {
