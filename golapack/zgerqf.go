...
// A = R * Q.
func Zgerqf(m, n int, a *mat.CMatrix, tau, work *mat.CVector, lwork int) (err error) {
	var lquery bool
	...
	if nb >= nbmin && nb < k && nx < k {
		...
		for i = k - kk + ki + 1; i >= k-kk+1; i -= nb {
			...
			if err = Zgerq2(ib, n-k+i+ib-1, a.Off(m-k+i-1, 0), tau.Off(i-1), work); err != nil {
				panic(err)
			}
			if m-k+i > 1 {
	...
	if mu > 0 && nu > 0 {
		if err = Zgerq2(mu, nu, a, tau, work); err != nil {
			panic(err)
		}
	}
