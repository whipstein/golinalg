	...
	label30:
		...
		if (*ipiv)[k-1] > 0 {
			...
			if k > 1 {
				...
				if err = Zsymv(uplo, k-1, -cone, a, work.Off(0, 1), czero, a.CVector(0, k-1, 1)); err != nil {
					panic(err)
				}
			...
			if k > 1 {
				...
				if err = Zsymv(uplo, k-1, -cone, a, work.Off(0, 1), czero, a.CVector(0, k, 1)); err != nil {
					panic(err)
				}
	...
	label50:
		...
		if (*ipiv)[k-1] > 0 {
			...
			if k < n {
				...
				if err = Zsymv(uplo, n-k, -cone, a.Off(k, k), work.Off(0, 1), czero, a.CVector(k, k-1, 1)); err != nil {
					panic(err)
				}
			...
			if k < n {
				...
				if err = Zsymv(uplo, n-k, -cone, a.Off(k, k), work.Off(0, 1), czero, a.CVector(k, k-1-1, 1)); err != nil {
					panic(err)
				}
