	...
	label30:
		...
		if (*ipiv)[k-1] > 0 {
			...
			if k > 1 {
				...
				Zsymv(uplo, toPtr(k-1), toPtrc128(-cone), a, lda, work, func() *int { y := 1; return &y }(), &czero, a.CVector(0, k), func() *int { y := 1; return &y }())
		...
		if kstep == 1 {
			...
			if kp != k {
				...
				goblas.Zswap(k-kp-1, a.CVector(kp, k-1, 1), a.CVector(kp-1, kp, *lda))
				temp = a.Get(k-1, k-1)
	...
	label50:
		...
		if (*ipiv)[k-1] > 0 {
			...
			if k < (*n) {
				...
				Zsymv(uplo, toPtr((*n)-k), toPtrc128(-cone), a.Off(k, k), lda, work, func() *int { y := 1; return &y }(), &czero, a.CVector(k, k-1), func() *int { y := 1; return &y }())
			...
			if k < (*n) {
				...
				Zsymv(uplo, toPtr((*n)-k), toPtrc128(-cone), a.Off(k, k), lda, work, func() *int { y := 1; return &y }(), &czero, a.CVector(k, k-1-1), func() *int { y := 1; return &y }())
		...
		if kstep == 1 {
			...
			if kp != k {
				...
				goblas.Zswap(kp-k-1, a.CVector(k, k-1, 1), a.CVector(kp-1, k, *lda))
				temp = a.Get(k-1, k-1)
				a.Set(k-1, k-1, a.Get(kp-1, kp-1))
				a.Set(kp-1, kp-1, temp)
			}
