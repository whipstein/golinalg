...
func Zhb2stkernels(uplo byte, wantz bool, ttype, st, ed, sweep, n, nb, ib *int, a *mat.CMatrix, lda *int, v, tau *mat.CVector, ldvt *int, work *mat.CVector) {
	...
	if upper {
		...
		if (*ttype) == 1 {
			...
			Zlarfg(&lm, &ctmp, v.Off(vpos), func() *int { y := 1; return &y }(), tau.GetPtr(taupos-1))
		...
		if (*ttype) == 2 {
			...
			if lm > 0 {
				...
				Zlarfx('R', toPtr(ln-1), &lm, v.Off(vpos-1), tau.GetPtr(taupos-1), a.Off(dpos-(*nb), j1-1).UpdateRows((*lda)-1), toPtr((*lda)-1), work)
			}
		}
		...
		if (*ttype) == 1 {
			...
			Zlarfg(&lm, a.GetPtr(ofdpos-1, (*st)-1-1), v.Off(vpos), func() *int { y := 1; return &y }(), tau.GetPtr(taupos-1))
		...
		if (*ttype) == 2 {
			...
			if lm > 0 {
				...
				Zlarfg(&lm, a.GetPtr(dpos+(*nb)-1, (*st)-1), v.Off(vpos), func() *int { y := 1; return &y }(), tau.GetPtr(taupos-1))

				Zlarfx('L', &lm, toPtr(ln-1), v.Off(vpos-1), toPtrc128(tau.GetConj(taupos-1)), a.Off(dpos+(*nb)-1-1, (*st)).UpdateRows((*lda)-1), toPtr((*lda)-1), work)
			}
		}
	}
}
