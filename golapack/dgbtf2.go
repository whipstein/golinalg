...
func Dgbtf2(m, n, kl, ku *int, ab *mat.Matrix, ldab *int, ipiv *[]int, info *int) {
	...
	for j = 1; j <= min(*m, *n); j++ {
		...
		if ab.Get(kv+jp-1, j-1) != zero {
			...
			if jp != 1 {
				goblas.Dswap(ju-j+1, ab.Vector(kv+jp-1, j-1, (*ldab)-1), ab.Vector(kv, j-1, (*ldab)-1))
			}
			...
			if km > 0 {
				...
				if ju > j {
					err = goblas.Dger(km, ju-j, -one, ab.Vector(kv+2-1, j-1, 1), ab.Vector(kv-1, j, (*ldab)-1), ab.Off(kv, j).UpdateRows((*ldab)-1))
				}
			}
		} else {
