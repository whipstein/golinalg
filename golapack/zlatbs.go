...
func Zlatbs(uplo, trans, diag, normin byte, n, kd *int, ab *mat.CMatrix, ldab *int, x *mat.CVector, scale *float64, cnorm *mat.Vector, info *int) {
	...
	if (*info) != 0 {
		gltest.Xerbla([]byte("ZLATBS"), -(*info))
		return
	}
	...
	if (grow * tscal) > smlnum {
		...
		err = goblas.Ztbsv(mat.UploByte(uplo), mat.TransByte(trans), mat.DiagByte(diag), *n, *kd, ab, x.Off(0, 1))
	} else {
		...
		if notran {
			...
			label110:
				...
				if upper {
					...
					jlen = min(*kd, (*n)-j)
					if jlen > 0 {
						goblas.Zaxpy(jlen, -x.Get(j-1)*complex(tscal, 0), ab.CVector(1, j-1, 1), x.Off(j, 1))
					}
			...
			for _, j = range genIter(jfirst, jlast, jinc) {
				...
				if uscal == complex(one, 0) {
					...
					if upper {
						...
						jlen = min(*kd, (*n)-j)
						if jlen > 1 {
							csumj = goblas.Zdotu(jlen, ab.CVector(1, j-1, 1), x.Off(j, 1))
						}
					}
				} else {
			...
			for _, j = range genIter(jfirst, jlast, jinc) {
				...
				if uscal == complex(one, 0) {
					...
					if upper {
						...
						jlen = min(*kd, (*n)-j)
						if jlen > 1 {
							csumj = goblas.Zdotc(jlen, ab.CVector(1, j-1, 1), x.Off(j, 1))
						}
					}
				} else {
					//                 Otherwise, use in-line code for the dot product.
