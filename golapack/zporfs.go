...
// solution.
func Zporfs(uplo byte, n, nrhs *int, a *mat.CMatrix, lda *int, af *mat.CMatrix, ldaf *int, b *mat.CMatrix, ldb *int, x *mat.CMatrix, ldx *int, ferr, berr *mat.Vector, work *mat.CVector, rwork *mat.Vector, info *int) {
	var upper bool
	var one complex128
	...
	if (*info) != 0 {
		gltest.Xerbla([]byte("ZPORFS"), -(*info))
		return
	}
	...
	label20:
		...
		err = goblas.Zhemv(mat.UploByte(uplo), *n, -one, a, x.CVector(0, j-1, 1), one, work.Off(0, 1))
		...
		if berr.Get(j-1) > eps && two*berr.Get(j-1) <= lstres && count <= itmax {
			...
			Zpotrs(uplo, n, func() *int { y := 1; return &y }(), af, ldaf, work.CMatrix(*n, opts), n, info)
