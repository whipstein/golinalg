	...
	label30:
		...
		if (*ipiv)[k-1] > 0 {
			...
			if k > 1 {
				...
				err = goblas.Zhpmv(mat.UploByte(uplo), k-1, -cone, ap, work.Off(0, 1), zero, ap.Off(kc-1, 1))
			...
			if k > 1 {
				...
				err = goblas.Zhpmv(mat.UploByte(uplo), k-1, -cone, ap, work.Off(0, 1), zero, ap.Off(kcnext-1, 1))
	...
	label60:
		...
		if (*ipiv)[k-1] > 0 {
			...
			if k < (*n) {
				...
				err = goblas.Zhpmv(mat.UploByte(uplo), (*n)-k, -cone, ap.Off(kc+(*n)-k), work.Off(0, 1), zero, ap.Off(kc, 1))
			...
			if k < (*n) {
				goblas.Zcopy((*n)-k, ap.Off(kc, 1), work.Off(0, 1))
				err = goblas.Zhpmv(mat.UploByte(uplo), (*n)-k, -cone, ap.Off(kc+((*n)-k+1)-1), work.Off(0, 1), zero, ap.Off(kc, 1))
				ap.Set(kc-1, ap.Get(kc-1)-complex(real(goblas.Zdotc((*n)-k, work.Off(0, 1), ap.Off(kc, 1))), 0))
				...
				err = goblas.Zhpmv(mat.UploByte(uplo), (*n)-k, -cone, ap.Off(kc+((*n)-k+1)-1), work.Off(0, 1), zero, ap.Off(kcnext+2-1, 1))
