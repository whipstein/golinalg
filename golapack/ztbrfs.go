	...
	label210:
		...
		if kase != 0 {
			if kase == 1 {
				...
				err = goblas.Ztbsv(mat.UploByte(uplo), mat.TransByte(transt), mat.DiagByte(diag), *n, *kd, ab, *ldab, work, 1)
				for i = 1; i <= (*n); i++ {
				...
				err = goblas.Ztbsv(mat.UploByte(uplo), mat.TransByte(transn), mat.DiagByte(diag), *n, *kd, ab, *ldab, work, 1)
			}
			goto label210
		}
